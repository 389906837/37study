...
public class AntboxTable extends JFrame {
    ...
    private String[] columnName = {"序号", "标签", "RSSI", "已绑定商品", "是否绑定"};
    ...
    public void showBody(UserDto dto, List<MerchantDto> merchantList, MerchantDto merchant) {
        ...
        mer = merchant;
        welcomeText.setText("【"+ merchant.getMerchantName() +"】欢迎您 " + dto.getUsername());
    }
    ...
    private void createMenuBar() {
        ...
        JMenu operationMenu = new JMenu("返回操作");

        JMenuItem loginItem = new JMenuItem("登录页");
        JMenuItem selectMerchantItem = new JMenuItem("选择域名页");
    ...
    private void createTopPanel() {
        ...
        productId.setBounds(105, 20, 70, 25);
        productIdValue.setBounds(160, 20, 150, 20);
        productName.setBounds(350, 20, 70, 25);
        productNameValue.setBounds(420, 20, 150, 20);
        searchButton.setBounds(595, 20, 65, 20);
    ...
    private void showProductPanel() {
        // JLabel
        JLabel sourceLabel = new JLabel("待绑定标签的商品：");
        sourceLabel.setBounds(40, 60, 140, 25);
        con.add(sourceLabel);
        // JList
    ...
    private void showProductBatchPanel() {
        JLabel batchLabel = new JLabel("商品所属批次号：");
    ...
    private void createOperationPanel() {
        JLabel machineModel = new JLabel("一体机机型:");
        comboBox = new JComboBox();
        JButton scanButton = new JButton("扫描标签");
        JButton bindingButton = new JButton("确认无误并绑定标签");
        JButton removeButton = new JButton("商品解绑");

        machineModel.setBounds(157, 180, 140, 25);
        comboBox.setBounds(230, 180, 140, 25);
        scanButton.setBounds(525, 180, 140, 25);

        bindingButton.setBounds(152, 220, 160, 25);
        ...
        bindingButton.addActionListener(e -> {
            ...
            if (selectProduct == null) {
                JOptionPane.showMessageDialog(null, "没有选中要绑定的商品");
                return;
            }
            ...
            if (uidList1 == null) {
                JOptionPane.showMessageDialog(null, "没有标签，无法绑定");
                return;
            }

            if (uidList1.size() == 0) {
                JOptionPane.showMessageDialog(null, "标签已绑定产品，先解绑，再绑定");
                return;
            }

            if (hasRssiUnqualified()) {
                JOptionPane.showMessageDialog(null, "表格中含有不合格标签");
                return;
            }
        ...
        removeButton.addActionListener(e -> {
            ...
            List<String> uidList1 = getUidList();
            if (uidList1 == null) {
                JOptionPane.showMessageDialog(null, "没有已绑定的标签");
                return;
            }
            //解绑接口调用
            CommodityRfidDto dto = new CommodityRfidDto();
    ...
    private void createTablePanel() {
        ...
        analysisText.setText("备注：表格中，红色底行表示不合格标签；选中行底色为蓝色底");
        // 创建表格
...
//        rfidText.setText("共 " + 30 + " 标签；已绑定: " + 15 + "；未绑定: " + 15);
        ...
        analysisText.setBounds(25, 280, 500, 25);
    ...
    private void createBottomPanel() {
        welcomeText.setText("意为所有局域网可访问");
    ...
    private void isDataValid() {
        if (userDto == null) {
            JOptionPane.showMessageDialog(null, "登录信息有误,请退出重新登录");
            return;
        }
    }
    ...
    void openReader() {
        ...
        if (portNames == null || portNames.length == 0) {
            ...
            if (!antboxClient.isOpened()) {
                JOptionPane.showMessageDialog(null, "检查不到端口，请确认驱动安装正确或者USB连接正常");
            } else {
                return;
            }
        }
...
//        scanGoodsNum.setText(statInfo);
    ...
    public String setLabelData(List<String> rfidList) {
        ...
        return "共 " + rfidList.size() + " 标签；已绑定: " + bindCount + "；未绑定: " + (rfidList.size() - bindCount);
    }
