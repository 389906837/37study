...
public class AntboxClient implements AntBoxService, SerialPortEventListener {
    ...
    public synchronized void close() {
        try {
            ...
            if (log.isErrorEnabled()) {
                log.error("[" + commAddress + "] 关闭串口失败: " + ex.getClass().getSimpleName() + ": "
                        + ex.getMessage());
            }
        }
    }
    ...
    public synchronized void open() {
        try {
            ...
            if (log.isErrorEnabled()) {
                log.error("[" + commAddress + "] 打开串口失败: " + ex.getClass().getSimpleName() + ": "
                        + ex.getMessage());
            }
        }
    }
    ...
    private void sendCommand(final short command, final short state, final ByteBuf data) {
        ...
        try {
            ...
            log.error("[" + commAddress + "] 发送命令失败: " + e.getClass().getSimpleName() + ": "
                    + e.getMessage());
        }
        // 发命令给读写器
...
//        }
        try {
            ...
            // 等待结果
            if (!awaitResult()) {
                log.info("等待结果:读写器通讯超时");
            }
        } catch (SerialPortException ex) {
            if (log.isErrorEnabled()) {
                log.error("[" + commAddress + "] 发送命令失败: " + ex.getClass().getSimpleName() + ": "
                        + ex.getMessage());
            }
        }
    }
