...
public class MemberInfoServiceImpl extends GenericServiceImpl<MemberInfo, String> implements MemberInfoService {
	 ...
	 */
	@Override
	public MemberInfo selectByMobile(String mobile) {
		return memberInfoDao.selectByMobile(mobile);
	}
	 ...
	 */
	@Override
	public List<MemberInfo> selectMemberInfoByRecommedCode(String recommedCode) {
		return memberInfoDao.selectMemberInfoByRecommedCode(recommedCode);
	}
	/**
	 * 根据会员ID 查找会员信息
	 * @param memberId 会员ID
	 ...
	 */
	@Override
	public MemberInfo selectMemberInfoByLoginName(String loginName) {
		return memberInfoDao.selectMemberInfoByLoginName(loginName);
	}
	...
	public MemberInfo registerMember(RegisterVo regVo, HttpServletRequest request, boolean isPwd) throws ServiceException, Exception {
		// 注册会员信息
		MemberInfo registerMember = this.selectMemberInfoByLoginName(regVo.getMobileNumber());
		...
		if (regVo.getItype().intValue() == 10) {//微信
			...
			tempList = thirdAuthoriseService.selectThirdAuthoriseByOpenId(wechatUser.getOpenid(),10);
			...
			tempList = thirdAuthoriseService.selectThirdAuthoriseByOpenId(userInfo.getUserId(),20);
			if (null != tempList && tempList.size() > 0) {
		...
		if (null == registerMember) {
			...
			if (StringUtil.isNotBlank(regVo.getRecommonCode())) {// 会员推荐
				recommendMember = this.selectMemberInfoByRecommonCode(regVo.getRecommonCode());
			...
			String json = (String) iCached.hget(RedisConst.MERCHANT_INFO, RedisConst.MERCHANT_INFO_DETAILS + merchantCode);
		...
		if (isSend) {
			// 注册成功发送短信
			sendUserMsg(registerMember.getSmobile(), regVo.getLoginPwd(), isPwd);
		}
	 ...
	 */
	@Override
	public MemberInfo selectMemberInfoByBindType(String mobileNumber, Integer bindType) {
	 ...
	 */
	@Override
	public MemberInfo selectMemberInfoByRecommonCode(String recommonCode) {
		return memberInfoDao.selectMemberInfoByRecommonCode(recommonCode);
	}
	 ...
	 * @param isPwd 密码是否在自动生成
	 */
	private void sendUserMsg(final String mobile, final String password, final boolean isPwd) {
