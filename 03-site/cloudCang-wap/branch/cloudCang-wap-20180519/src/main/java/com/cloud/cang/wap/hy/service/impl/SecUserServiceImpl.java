...
public class SecUserServiceImpl implements SecUserService {
	...
	public AuthorizationVO getUserByUserName(CaptchaUsernamePasswordToken loginObj) {
		// 将登录信息放入缓存
		MemberInfo memberInfo = memberInfoService.selectMemberInfoByLoginName(loginObj.getUsername());
		if (memberInfo == null)
			return null;
				...
				try {
					...
					if (ipAreaService != null) {
						...
						if (ipArea == null) {
							...
							ResponseEntity<AreaInfo> areaInfoRes = restTemplate.getForEntity(requestAddress, AreaInfo.class);
							if (areaInfoRes != null) {
								areaInfo = areaInfoRes.getBody();
								IpArea insertIpArea = new IpArea();
								insertIpArea.setCountry(areaInfo.getCountry());
								insertIpArea.setIstatus(1);
								insertIpArea.setSprovince(areaInfo.getProvince());
							...
							areaInfo.setProvince(ipArea.getSprovince());
						}
					}
					...
					loginLog.setSprovince(areaInfo.getProvince());
