...
public class RefundAuditServiceImpl extends GenericServiceImpl<RefundAudit, String> implements
    ...
    public ResponseVo<String> generateRefundOrder(RefundOrderVo refundOrderVo, HttpServletRequest request) throws Exception {
        ...
        refundAudit.setIactualPayIpoint(record.getIpoint());//积分
        ...
        BigDecimal totalRefundPoint = BigDecimal.ZERO;//本次退回总积分
        BigDecimal totalAmount = BigDecimal.ZERO;//本次退款订单总额
        BigDecimal totalPoint = BigDecimal.ZERO;//本次退回订单积分
        BigDecimal tempAmount = BigDecimal.ZERO;//临时退款金额
        BigDecimal tempPoint = BigDecimal.ZERO;//临时退回积分
            ...
            if (StringUtil.isNotBlank(sid)) {//ID不为空
                ...
                if (updateVo.getFrefundCount() >= orderCommodity.getForderCount()) {//两者相等  本次退款数量+已退款数量 >= 订单商品数量
                    ...
                    if (orderCommodity.getIpoint() != null) {
                        tempPoint = orderCommodity.getIpoint().subtract(orderCommodity.getFrefundPoint());//本次退款金额
                        totalRefundPoint = totalRefundPoint.add(tempPoint);
                        updateVo.setFrefundPoint(orderCommodity.getIpoint());
                    }
                } else {
                    ...
                    if (orderCommodity.getIpoint() != null) {
                        tempPoint = orderCommodity.getIpoint().divide(new BigDecimal(orderCommodity.getForderCount()), 2, BigDecimal.ROUND_DOWN).multiply(new BigDecimal(tempNum));
                        totalRefundPoint = totalRefundPoint.add(tempPoint);
                        updateVo.setFrefundPoint(orderCommodity.getFrefundPoint().add(tempPoint));
                    }
                }
                ...
                refundCommodity.setIpoint(orderCommodity.getIpoint());
                totalAmount = totalAmount.add(refundCommodity.getFcommodityAmount());
                totalPoint = totalPoint.add(refundCommodity.getIpoint());
                refundCommodity.setFrefundAmount(tempAmount);
                refundCommodity.setFrefundPoint(tempPoint);
                ...
                if (refundCommodity.getFdiscountAmount().doubleValue() > 0) {//优惠大于0
                    ...
                    }else if (orderCommodity.getFpointDiscountAmount().doubleValue() > 0) {
                        refundCommodity.setFpointDiscountAmount(refundCommodity.getFdiscountAmount());
                    }
                } else {
                    refundCommodity.setFfirstDiscountAmount(BigDecimal.ZERO);
                    refundCommodity.setFpromotionDiscountAmount(BigDecimal.ZERO);
        ...
        if (record.getIpayType()!=90 && totalRefundAmount.doubleValue() <= 0) {
        ...
        updateRefund.setIactualPayIpoint(totalRefundPoint);//订单实付积分
        updateRefund.setFapplyRefundAmount(totalRefundAmount);//申请退款总额
        updateRefund.setIapplyRefundIpoint(totalRefundPoint);//申请退款总额
        updateRefund.setFactualRefundAmount(totalRefundAmount);//实际退款总额  =  申请退款总额 - 手续费
        updateRefund.setIactualRefundIpoint(totalRefundPoint);//实际退回积分总额  =  申请退回积分总额 - 手续费
        ...
        updateRecord.setIactualRefundPont(record.getIactualRefundPont().add(totalRefundPoint));
