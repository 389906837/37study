...
<div class="refdetails_container">
    ...
    <div class="refdetails_main m-b-07">
        ...
        <div class="refdetails_priceinfo">
            ...
            <c:if test="${(!empty orderRecord.fotherDiscountAmount and orderRecord.fotherDiscountAmount > 0)}">
                <div class="refdetails_discounts">
                    其他优惠<span>-￥<f:formatNumber pattern=",##0.00"
                                                value="${orderRecord.fotherDiscountAmount}"/>
                </span>
                </div>
            </c:if>
            <c:if test="${(!empty orderRecord.fpointDiscountAmount and orderRecord.fpointDiscountAmount > 0)}">
                <div class="refdetails_discounts">
                    积分优惠<span>-￥<f:formatNumber pattern=",##0.00"
                                                value="${orderRecord.fpointDiscountAmount}"/></span>
                </div>
            </c:if>
            ...
            <c:if test="${!empty orderRecord.iactualRefundPont and orderRecord.iactualRefundPont > 0}">
                <div class="shop_money">
                    实际退回积分<span> <i class="totalprice">${orderRecord.iactualRefundPont}</i></span>
                </div>
            </c:if>
        </div>
    </div>
