...
<div class="reg_container">
    ...
    <c:choose>
        <c:when test="${isupportPayWay eq 10}">
            <c:if test="${signPageType eq 20}">
                <div class="reg_main md">
                    <a href="javascript:void(0);" onclick="wechatPoint('${signPage}');" class="reg_btn">开通微信支付分</a>
                    <div class="m-t-1" style="font-size: .9rem;">
                        开通后可自动结算扣款体验无感消费。<a href="${ctx}/index/main.html"
                                            style="color:#3080e1;text-decoration: underline;">暂不开通</a>
                    </div>
                </div>
            </c:if>
            <c:if test="${signPageType eq 10}">
                <div class="reg_main md">
        ...
        <c:when test="${isupportPayWay eq 20}">
            <c:if test="${signPageType eq 20}">
                <div class="reg_main md">
                    <a href="javascript:void(0);" onclick="wechatPoint('${signPage}');" class="reg_btn">开通微信支付分</a>
                </div>
        ...
        <c:when test="${isupportPayWay eq 30}">
            <div class="reg_main md">
