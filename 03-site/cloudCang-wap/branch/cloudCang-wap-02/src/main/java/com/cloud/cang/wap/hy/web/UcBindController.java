...
public class UcBindController {
     ...
     * @return
     */
    @RequestMapping(value = "/authTrash", method = RequestMethod.POST)
    @ResponseBody
    public ResponseVo<String> authTrash(RegisterVo regVo, HttpServletRequest request) {
        String errorMsg = "";
        try {
            ...
            //验证注册基础参数
            validationAuthParam(regVo, request, authVo, deviceInfo);
            //授权智能垃圾箱
            return trashAuthoriseService.authTrash(request, authVo, deviceInfo);
        } catch (ServiceException e) {
            errorMsg = e.getMessage();
            logger.error("系统异常:{}", e.getMessage());
        } catch (Exception e) {
     ...
     * @return
     */
    @RequestMapping(value = "/cancelAuthTrash", method = RequestMethod.POST)
    @ResponseBody
    public ResponseVo<String> cancelAuthTrash(String authoriseTrashId, HttpServletRequest request) {
        ResponseVo<String> responseVo = ResponseVo.getSuccessResponse();
        String errorMsg = "";
        try {
            AuthorizationVO authVo = SessionUserUtils.getSessionAttributeForUserDtl();//获取用户数据
            ...
            //validationAuthParam(regVo, request, authVo, deviceInfo);
            //授权智能垃圾箱
            trashAuthoriseService.cancelAuthTrash(authoriseTrashId, authVo, deviceInfo);
            //responseVo.setData("/personal/index");
            return responseVo;
        } catch (ServiceException e) {
            errorMsg = e.getMessage();
            logger.error("系统异常:{}", e.getMessage());
        ...
        if (WapUtils.isWXRequest(request)) {
         /*   String json = (String) iCached.hget(RedisConst.MERCHANT_INFO, RedisConst.SH_WECHAT_CONFIG + authVo.getSmerchantCode());
          ...
          MerchantConf conf = JSONObject.parseObject(json, MerchantConf.class);*/
            MerchantConf conf = indexService.getWechatMerchantConf(authVo.getSmerchantCode(), 2);
            // map.put("signPage", "/personal/wechatSign.html");
            if (conf.getIwechatWithholdType() == null || conf.getIwechatWithholdType() == 10) {
                map.put("signPage", "/personal/wechatSign.html");
                map.put("signPageType", 10);
            } else if (conf.getIwechatWithholdType() != null && conf.getIwechatWithholdType() == 20) {
                map.put("signPage", "/personal/wechatOpen.html");
                map = indexService.getPayConfig(map, request);
                map.put("signPageType", 20);
            }
        } else if (WapUtils.isAlipayRequest(request)) {
     ...
     * @param request
     */
    private void validationAuthParam(RegisterVo regVo, HttpServletRequest request, AuthorizationVO authVo, DeviceInfo deviceInfo) {
        ...
        if (null != deviceInfo.getIisBindTrash() && deviceInfo.getIisBindTrash() == 1) {
            ...
            if (trashAuthorise != null && 10 == trashAuthorise.getIstatus()) {
                throw new ServiceException("用户已经授权");
            }
        } else {
            throw new ServiceException("设备不是智能垃圾箱设备,无法授权");
        }
        // 短信验证码不正确
        int msgErrorCode = checkAuthMessageCode(regVo.getMsgCode(),
                regVo.getMobileNumber());
        if (msgErrorCode != 1) {
            if (msgErrorCode == -1) {
                throw new ServiceException("短信验证码已超时");
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("会员授权垃圾箱基础校验成功！会员手机：{}", regVo.getMobileNumber());
        }
    }
     ...
     * @return
     */
    @RequestMapping("/checkAuthMessageCode")
    @ResponseBody
    public Integer checkAuthMessageCode(String messageCode, String mobile) {
        try {
            if (org.apache.commons.lang.StringUtils.isNotBlank(messageCode)) {
                String authCodeByCached = (String) iCached.get(WapConstants.RedisConst.MEMBER_BIND_AUTH_TRASH_CODE + mobile);
                if (StringUtil.isNull(authCodeByCached)) {
                    return -1;//短信验证码已超时
                } else if (!authCodeByCached.equalsIgnoreCase(messageCode)) {
                    return -2;//短信验证码错误
