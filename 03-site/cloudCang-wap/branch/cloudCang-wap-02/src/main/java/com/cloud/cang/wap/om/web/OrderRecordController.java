...
public class OrderRecordController {
    ...
    ResponseVo<PayBackDto> orderPayment(String orderCode, HttpServletRequest request) {
        ...
        try {
            ...
            if (null != trashAuthorise) {
                ...
                ResponseVo<CreatOrderResult>  payPointRes = (ResponseVo<CreatOrderResult> ) invoke.invoke();
                if (null != payPointRes && payPointRes.isSuccess()) {
                    logger.info("wap商户代扣积分成功,订单编号：{}", orderRecord.getSorderCode());
                    PayBackDto payBackDto = new PayBackDto();
                    payBackDto.setIpayType(BizTypeDefinitionEnum.PayType.PAY_POINT.getCode());
                    payBackDto.setIpayWay(BizTypeDefinitionEnum.PayWay.POINT_ALL_DEDUCTION.getCode());
    ...
    public String applyRefund(@PathVariable String orderCode, ModelMap modelMap, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            Integer ipayType = orderRecord.getIpayType();
            if (90 != ipayType) {
               ...
               /* if (orderRecord.getFactualPayAmount().doubleValue() <= 0) {
                ...
                if (orderRecord.getIpoint().compareTo(orderRecord.getIactualRefundPont()) <= 0) {
                    modelMap.put("resVo", new SiteResponseVo(false, -1002, "订单已申请退款，请不要重复操作", -1));
                    return "error/error";
                }
            }
