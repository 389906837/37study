...
public class PhotoProxy {
	...
	public static boolean upLoadPic(File file, String serverPath,
			String fileName) {
		boolean b = FtpUtils.uploadToFtpServer(file, serverPath, fileName,
				getFtpUser());
	...
	public static FtpUser getFtpUser() {
		...
		Set<ParameterGroupDetail> ParameterGroupDetailList = GrpParaUtil
				.get(GrpParaUtil.FTP_CONFIG);
		Iterator<ParameterGroupDetail> iterator = ParameterGroupDetailList
				.iterator();
	...
	private static void resizeImage(File srcFile, boolean keepRatio,
		...
		try {
			...
			int[] size = calculateImgSize(srcImage, keepRatio,
	...
	public static String resizePhoto(BufferedInputStream bis, String fileName, int width, int height, HttpServletRequest request){
	 ...
	 */
	private static boolean string2Image(String imgStr, String imgFilePath) {
