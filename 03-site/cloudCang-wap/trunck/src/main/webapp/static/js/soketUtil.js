    ...
    socketObj.on('socketEvent', function (data) {
        if(data == "success") {
            ...
            if(data.types == 10 || data.types == 20) {
                ...
                if(!flag_shopping) {
                    jumpUrl(ctxRoot + "/order/shoppingCloudRealTimeOrderEmpty");
                    flag_shopping = true;
                    //关闭开门定时器
                    clearInterval(openTimer);
                    openTimeOut = 5;
                    ...
                    if (!Util.isEmpty(data.data)) {
                        $.ajax({
                            ...
                            success: function(msg){
                                childWindow.showCart(msg);
                            }
                        });
                    } else {
                        childWindow.hideCart();
                    }
                }
            } else if (data.types == 80) {
                if(!flag_shopping) {
                    jumpUrl(ctxRoot + "/order/replenishmentCloudRealTimeOrderEmpty");
                    flag_shopping = true;
                    //关闭开门定时器
                    clearInterval(openTimer);
                    openTimeOut = 5;
                    ...
                    if (!Util.isEmpty(data.data)) {
                        $.ajax({
                            ...
                            dataType: "json",
                            success: function(msg){
                                childWindow.showAddCart(msg);
                                childWindow.showSubCart(msg);
                            }
                        });
                    } else {
                        childWindow.hideRepCart(0);
                    }
                }
            }
        } else {
