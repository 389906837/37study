...
public class MsgTemplateController {
    ...
    ResponseVo<MsgTemplate> save(MsgTemplate msgTemplate) {
        ...
        if (StringUtils.isBlank(msgTemplate.getStemplateContent())) {
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("模板内容不能为空");
        }
        if (msgTemplate.getIisRealtime().intValue() == 2) {//短信非实时发送
            if (StringUtils.isBlank(startTime)) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("发送开始时间不能为空");
            }
            if (StringUtils.isBlank(endTime)) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("发送结束时间不能为空");
            }
            if (StringUtils.isNotBlank(startTime) && StringUtils.isNotBlank(endTime)) {
                if (endTime.compareTo(startTime) < 0) {
                    return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("发送的开始时间不能大于结束时间");
                }
            }
        }

        if (StringUtils.isBlank(msgTemplate.getSsupplierCode())) {
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("供应商编号不能为空");
        }
        ...
        if (StringUtils.isBlank(msgTemplate.getId())) {
            ...
            // 操作日志
            String logText = MessageFormat.format("新增消息/协议  模板从表名称{0},标题{1}", msgTemplate.getStemplateName(), msgTemplate.getStemplateTitle());
            ...
            // 操作日志
            String logText = MessageFormat.format("修改消息/协议模板从表名称{0},标题{1}", msgTemplate.getStemplateName(), msgTemplate.getStemplateTitle());
    ...
    ResponseVo<String> delete(String[] checkboxId) {
        try {
            ...
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("消息协议模板启用中请勿删除！");
        }
    }
        ...
        if (supplierInfo != null) {
            for (String id : checkboxId) {
                ...
                if (msgTemplate != null) {
                    ...
                    // 操作日志
                    String logText = MessageFormat.format("修改模板【{0}】的短信供应商为【{1}】", msgTemplate.getStemplateName(), supplierInfo.getSname());
                    LogUtil.addOPLog(LogUtil.AC_EDIT, logText, null);
                }
            }
            return ResponseVo.getSuccessResponse();
        } else {
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("供应商不存在！");
        }
    }
...
//            }
            ...
            // 操作日志
            String logText = MessageFormat.format("短信供应商启用", msgTemplateNew.getStemplateTitle());
            LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
            return ResponseVo.getSuccessResponse();
        } catch (Exception e) {
            log.error("消息/协议模板启禁用异常：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("消息/协议模板启用失败");
        }
    }
}
