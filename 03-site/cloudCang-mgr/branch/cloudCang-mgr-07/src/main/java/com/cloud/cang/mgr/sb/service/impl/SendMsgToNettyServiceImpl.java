...
public class SendMsgToNettyServiceImpl implements SendMsgToNettyService {
    ...
    public ResponseVo<String> offline(String id) {
        ResponseVo<String> responseVo = ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("设备离线失败！");
        ...
        try {
            ...
            if (responseVo.isSuccess()) {
                ...
                //操作日志
                String logText = MessageFormat.format("向设备发送操作消息，操作类型{0}", NettyConst.OFFLINE);
    ...
    public TcpResult selectUnRegistered() {
        ...
        ResponseVo<TcpResult> responseVo = ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("调用查询未注册长连接信息接口失败");
        try {
    ...
    public TcpResult selectRegistered() {
        ...
        ResponseVo<TcpResult> responseVo = ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("调用查询已注册长连接信息接口失败");
        try {
            TcpDto tcpDto = new TcpDto();
            // 创建Rest服务客户端
    ...
    public ResponseVo<String> disconnetTcp(String channelId) {
        ResponseVo<String> responseVo = ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("调用断开未注册TCP连接服务失败");
    ...
    public ResponseVo<String> disconnectRegisterTcp(String deviceId) {
        ResponseVo<String> responseVo = ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("调用断开已注册TCP连接服务失败");
    ...
    private ResponseVo<String> sendAdToDevice(String deviceIds, String userId, String position, String merchantCode) {
        ...
        ResponseVo<String> responseVo = ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("调用发送广告服务接口失败！");
        // 封装参数
        ...
        try {
            ...
            if (responseVo.isSuccess()) {
                //操作日志
                String logText = MessageFormat.format("向设备发送广告消息:{0}", "");
                LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
                return responseVo;
            }
        } catch (Exception e) {
    ...
    private ResponseVo<String> sendMsgToDevice(Object deviceIds, String method, String data) {
        ...
        ResponseVo<String> responseVo = ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("向设备发送信息失败！");
        //调用长连接服务前，对DTO对象进行赋值
        ...
        try {
            ...
            if (responseVo.isSuccess()) {
                ...
                //操作日志
                String logText = MessageFormat.format("向设备发送操作消息，操作类型{0}", method);
                LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
                return responseVo;
            }
        } catch (Exception e) {
