 ...
 /* ----------4.商品批次管理开始 ----------*/
    ...
    public ResponseVo<CommodityBatch> addBatch(CommodityBatch commodityBatch ) {
        ...
        try {
            ...
            if (StringUtils.isBlank(sbatchNo)) {
                ...
                return new ResponseVo<>(false, 111, "添加商品批次信息出错");
            }

            //插入数据到商品批次信息表
            commodityBatchService.insertCommodityBatch(commodityBatch,sbatchNo);
            //操作日志
            String logText = MessageFormat.format("新增商品批次信息，批次编号{0}", sbatchNo);
    ...
    public String toEditBatch(String sid, ModelMap map) {
        try {
            if (StringUtils.isNotBlank(sid)) {
                ...
                String logText = MessageFormat.format("编辑商品批次信息按钮，批次编号{0}", commodityBatch.getSbatchNo());
    ...
    public ResponseVo<CommodityBatch> editBatch(CommodityBatch commodityBatch ) {
        ...
        try {
            ...
            if (StringUtils.isBlank(sid)) {
                logger.error("商品批次信息不存在");
                return new ResponseVo<>(false, 111, "商品批次信息不存在");
            }
            // 10待上架 20部分上架 30全部上架
            ...
            String logText = MessageFormat.format("编辑商品批次编号，批次编号{0}", commodityBatch1.getSbatchNo());
            LogUtil.addOPLog(LogUtil.AC_ADD, logText, null);
        ...
        return new ResponseVo<>(false, 111, "编辑商品批次信息出错");
    }
        ...
        try {
            if (StringUtils.isNotBlank(sid)) {
                ...
                if (null != commodityBatch) {
                    ...
                    map.put("merchantInfo", merchantInfo);
                    map.put("commodityInfo", commodityInfo);
                    String logText4 = MessageFormat.format("查看商品批次信息，批次编号{0}", commodityBatch.getSbatchNo());
    ...
    ResponseVo<String> addToCommodityBatch(String commodityIds,CommodityBatch commodityBatch) {
        try {
            if (StringUtil.isNotBlank(commodityIds)) { //商品ID
                ...
                //操作日志
                String logText = MessageFormat.format("批量添加商品批次，批次编号{0}", commodityBatch.getSbatchNo());
                LogUtil.addOPLog(LogUtil.AC_ADD, logText, null);
