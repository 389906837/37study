...
RestSuccess = function(data) {
    if(data.success) {
        ...
        alertDel("Failed to reset password!");
    }
};
//成功操作
...
function createDatePicker(element,callback,inparams) {
    var params = {
        ...
        ranges: {
            ...
            'Nearly seven days': [moment().subtract('days', 6), moment()],
            'This month': [moment().startOf('month'), moment()],
            'Nearly thirty days': [moment().subtract('days', 29), moment()]
        }
    };
    if (inparams && typeof (inparams) == 'object') {
        for ( var key in inparams) {
