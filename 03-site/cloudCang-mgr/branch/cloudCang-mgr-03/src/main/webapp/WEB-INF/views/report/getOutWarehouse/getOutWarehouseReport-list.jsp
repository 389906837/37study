...
<title><spring:message code="report.outgoing.summary.report" /></title>
        ...
        <div class="col-sm-12">
            <div class="ibox ">
                <div class="ibox-title">
                    <h5><spring:message code="report.outgoing.summary.report" /></h5>
                </div>
                    ...
                    <div class="layui-form" id="searchForm">
                        <div class="layui-form-item">
                            ...
                            <div class="layui-inline" style="width: 210px">
                                <input type="text" name="queryTimeCondition" id="queryTimeCondition" placeholder='<spring:message code="report.depot.time" />'
                                       class="layui-input">
                            </div>
...
<script type="text/javascript">
    var colModel = [
        {name: 'commodityCode', index: 'SCOMMODITY_CODE', label: '<spring:message code="report.commodity.number" />', width: 180},
        {name: 'commodityFullName', index: 'commodityFullName', label: '<spring:message code="report.commodity.name" />', width: 360, sortable: false},
        {name: 'orderCount', index: 'FORDER_COUNT', label: '<spring:message code="report.out-of-stock.quantity" />', width: 200},
        {name: 'commodityPrice', index: 'FCOMMODITY_PRICE', label: '<spring:message code="report.unit.price.of.commodity.yuan" />', width: 240},
        {name: 'commodityAmount', index: 'FCOMMODITY_AMOUNT', label: '<spring:message code="report.total.merchandise.yuan" />', width: 220},
        {name: 'merchantName', index: 'merchantName', label: '<spring:message code="report.subordinate.merchants" />', sortable: false, width: 400}
    ];
    //表格加载完成后回调
    ...
    $(function () {
        ...
        initTable(ctx + "/getOutWarehouseReport/queryData", colModel, function () {
            <shiro:hasPermission name="GETOUTWAREHOUSEREPORT_DOWNLOADEXCEL">
            if (isExport) {//判断是否导出
                exportFileCallback1('<spring:message code="report.outgoing.summary.report" />');
                isExport = false;
            }
            </shiro:hasPermission>
        }, config,loadCompleteFn);
        <shiro:hasPermission name="GETOUTWAREHOUSEREPORT_DOWNLOADEXCEL">
        addExportBtn1('<spring:message code="report.outgoing.summary.report" />');
        </shiro:hasPermission>
    });
    ...
    function getPageFooterTotal(totalRow) {//处理合计
        ...
        $.ajax({
            url: ctx + "/getOutWarehouseReport/queryTotalData",
            type: "post",
            dataType: "json",
            data: sdata,
