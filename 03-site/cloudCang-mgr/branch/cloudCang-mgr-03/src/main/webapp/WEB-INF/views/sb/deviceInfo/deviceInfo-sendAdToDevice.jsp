...
<body class="gray-bg">
<form class="layui-form" action="${ctx}/device/saveNewsUpdate" method="post" id="myForm">
    <div class="col-sm-12">
        <div class="ibox-content m-t">
            <div class="layui-form-item">
                <div class="layui-col-md6">
                    <label class="layui-form-label"><spring:message code="ad.news.type"/></label>
                    <div class="layui-input-inline">
                        <input type="radio" name="newsType"  value="10"  checked title="<spring:message code='sb.device.ad'/> "/>
                        <%--<input type="radio" name="newsType"  value="20"  title="系统公告"/>--%>
                    </div>
                </div>
                <div class="layui-col-md6">
                    <label class="layui-form-label"><spring:message code='sb.news.operation'/></label>
                    <div class="layui-input-inline">
                        <select lay-verify="required" lay-search="" name="sregionCode" datatype="*" nullmsg="<spring:message code='sb.new.operation.empty'/>"
                                id="sregionCode" class="state" lay-filter="sregionCode">
                            <option value=""><spring:message code='sb.new.operation.empty'/></option>
                            <c:forEach items="${regionList }" var="item">
                                <option value="${item.scode}">${item.sregionName}</option>
                            </c:forEach>
                        </select>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    layui.use(['form', 'layedit', 'upload', 'laydate'], function () {
        form = layui.form;
        form.on('select(sregionCode)', function (data) {
            if (!isEmpty(data.value)) {
                $("#sregionCode").parent().find("span").hide();
            } else {
                $("#sregionCode").parent().find("span").show();
                if (!$("#sregionCode").parent().find("span").hasClass("Validform_wrong")) {
                    $("#sregionCode").parent().find("span").html($("#sregionCode").attr("nullmsg"));
                    $("#sregionCode").parent().find("span").removeClass("Validform_right");
                    $("#sregionCode").parent().find("span").addClass("Validform_wrong");
                }
            }
        });
        //设备单选
    ...
    $(function () {
        ...
        $("#selectDevice").click(function() {
            ...
            if (!isEmpty(deviceIds)) {
                showLayerMax("<spring:message code='sb.select.device.information'/>",ctx+"/common/selectDevice?queryDeviceType=100&deviceIds="+deviceIds+"&deviceCodes="+deviceCodes);
            } else {
                showLayerMax("<spring:message code='sb.select.device.information'/>",ctx+"/common/selectDevice?queryDeviceType=100");
            }
        });
