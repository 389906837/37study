...
<div class="ibox-content">
    <form class="layui-form" action="${ctx}/commodity/category/add" method="post" id="myForm" enctype="multipart/form-data">
        ...
        <div class="layui-form-item">
            <div class="layui-col-md6">
                <label class="layui-form-label"><spring:message code="sp.category.parent.class.name" /></label>
                <div class="layui-input-inline">
                    <select name="sparentId" id="sparentId" class="state" lay-filter="sparentId">
            ...
            <div class="layui-col-md6" id="mainCategoryDiv">
                <label class="layui-form-label"><spring:message code="sp.category.main.category.name" /></label>
                <div class="layui-input-inline">
                    <select name="scategoryName" id="scategoryName" class="state" lay-filter="scategoryName">
                        <option value=""><spring:message code="main.please.choose" /></option>
                        <c:forEach items="${dic}" var="vo">
                            <option value="${vo.svalue}_${vo.sname}">${vo.sname}</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    layui.use(['form', 'layedit', 'laydate', 'upload','layer'], function () {
        ...
        //监听select选择,选择商品大类
        form.on('select(sparentId)', function (data) {
...
//                    data: {pid: categoryId},
                    ...
                    success: function (resp) {
                        ...
                        $("#scategoryName").append(option1);
                        $.each(resp.data, function (key, val) {
                            var option1 = $("<option>").val(val.svalue).text(val.sname);
                            $("#scategoryName").append(option1);
                            form.render('select');
                        });
                    }
