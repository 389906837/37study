...
<style>
    div.tree-multiselect span.section-name{float:none;}
    .section-name{padding-left:10px;}
</style>
</head>
        ...
        <c:forEach items="${purviewList}" var="item" varStatus="L">
            <c:if test="${item.bisroot eq '1'}">
                ...
                <c:forEach varStatus="K" var="purItem" items="${item.nodes}">
                    <option value="${item.id}#//#${purItem.id}" data-section='<spring:message code="sh.menu.permission.configuration" />/${item.menuName}/${purItem.menuName}' <c:if test="${purItem.isSelect eq 1}"> selected</c:if>>${purItem.text}</option>
                </c:forEach>
            </c:if>
