...
<div class="ibox-content">
    <select id="purviewSelect" multiple="multiple">
        <c:forEach items="${roleList}" var="item" varStatus="L">
            <c:if test="${item.bisroot eq '1'}">
                <%--<option value="${item.id}" data-section="<spring:message code='sh.authority.management' />/${item.menuName}" <c:if test="${item.isSelect eq 1}"> selected</c:if>>${item.text}</option>--%>
                <c:forEach varStatus="K" var="purItem" items="${item.nodes}">
                    <option value="${item.id}#//#${purItem.id}" data-section="<spring:message code='sh.rights.profile' />/<spring:message code='${item.menuName}' />/<spring:message code='${purItem.menuName}' />" <c:if test="${purItem.isSelect eq 1}"> selected</c:if>>${purItem.text}</option>
                </c:forEach>
            </c:if>
        ...
        <%--<c:forEach items="${roleList}" var="item" varStatus="L">
            <c:if test="${item.bisroot eq '1'}">
                ...
                <c:forEach varStatus="K" var="purItem" items="${item.nodes}">
                    <option value="${purItem.id}" data-section="${item.menuName}/${purItem.menuName}" <c:if test="${purItem.isSelect eq 1}"> selected</c:if>>${purItem.text}</option>
                </c:forEach>
            </c:if>
            <c:if test="${item.bisroot ne '1'}">
    ...
    <form class="layui-form" action="${ctx}/role/updateRoleAuth" method="post" id="myForm">
