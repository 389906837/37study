...
<div class="ibox-content">
    <select id="brandSelect" multiple="multiple">
        <c:forEach items="${brandList}" var="item" varStatus="L">
            <option value="${item.id}#//#${item.text}" data-section='<spring:message code="sp.brand.product.brand" />' <c:if test="${fn:indexOf(brandIds,item.id) != -1}"> selected</c:if>>${item.text}</option>
        </c:forEach>
    </select>
