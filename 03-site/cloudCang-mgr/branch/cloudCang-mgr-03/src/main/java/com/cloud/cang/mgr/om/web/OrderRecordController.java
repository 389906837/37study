...
public class OrderRecordController {
    ...
    public ResponseVo save(OrderRecord orderRecord, HttpServletRequest request, String commodityIds) {
        try {
            ...
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("om.order.edit.error"));
        }
    }
    ...
    ResponseVo rechargeAgain(String checkboxId) {
        try {
            if (StringUtils.isBlank(checkboxId)) {
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("common.data.error"));
            }
            OrderRecord orderRecord = orderRecordService.selectByPrimaryKey(checkboxId);
            if (null == orderRecord) {
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("om.order.no.exist"));
            }
            if (null == orderRecord.getIstatus() ||
                ...
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("om.recharge.again.not.support"));
            }
            // 组装数据
            ...
            try {
                ...
                if (null != responseVo && responseVo.isSuccess()) {
                    ...
                    String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("log.pay.again.success"), orderRecord.getSorderCode());
                    LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
                    return ResponseVo.getSuccessResponse(MessageSourceUtils.getMessageByKey("om.recharge.again.success"));
                } else {
                    logger.error("调用订单支付补处理接口服务失败{}", JSON.toJSONString(responseVo));
                    return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("hy.recharge.again.fail"));
                }
            } catch (Exception e) {
    ...
    public void downloadExcel(HttpServletRequest request, HttpServletResponse response, OrderRecordDomain orderRecordDomain, ParamPage paramPage) {
        ...
        String[] names = new String[]{
                ...
                MessageSourceUtils.getMessageByKey("sq.trade.serial.number"),
                ...
                MessageSourceUtils.getMessageByKey("spcon.commodity.short.name"),
                MessageSourceUtils.getMessageByKey("om.commodity.cost.yuan"),
                MessageSourceUtils.getMessageByKey("om.commodity.sales.unit.price.yuan"),
                MessageSourceUtils.getMessageByKey("sp.commodityBatch.product.number"),
                MessageSourceUtils.getMessageByKey("om.total.amount.yuan"),
                MessageSourceUtils.getMessageByKey("om.total.amount.of.the.offer.yuan"),
                MessageSourceUtils.getMessageByKey("om.first.order.discount.amount.yuan"),
                MessageSourceUtils.getMessageByKey("om.promotional.discount.amount.yuan"),
                MessageSourceUtils.getMessageByKey("om.voucher.deduction.amount.yuan"),
                MessageSourceUtils.getMessageByKey("om.total.amount.paid.yuan"),
                MessageSourceUtils.getMessageByKey("om.refund.amount"),
                MessageSourceUtils.getMessageByKey("sq.total.refund.yuan"),
                MessageSourceUtils.getMessageByKey("con.ac.mu"),
                MessageSourceUtils.getMessageByKey("sq.total.order.amount.yuan"),
                MessageSourceUtils.getMessageByKey("order.discount.amount"),
                MessageSourceUtils.getMessageByKey("excel.order.first.amount"),
                MessageSourceUtils.getMessageByKey("excel.order.promotion.amount"),
                MessageSourceUtils.getMessageByKey("excel.order.coupon.deduction.amount"),
                MessageSourceUtils.getMessageByKey("excel.total.amount.paid.order"),
                MessageSourceUtils.getMessageByKey("excel.total.amount.order.refund"),
                MessageSourceUtils.getMessageByKey("tb.order.time"),
                MessageSourceUtils.getMessageByKey("om.deduction.method"),
                MessageSourceUtils.getMessageByKey("om.payment.types"),
                MessageSourceUtils.getMessageByKey("main.state"),
                MessageSourceUtils.getMessageByKey("excel.reason.order.payment.failure"),
                MessageSourceUtils.getMessageByKey("excel.whether.split.the.order"),
                MessageSourceUtils.getMessageByKey("excel.split.order.number"),
                MessageSourceUtils.getMessageByKey("excel.split.type")};
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if ("10".equals(ichargebackWay)) {
                e.setCell(31, MessageSourceUtils.getMessageByKey("sh.merchants.withholding"));
            } else {
                e.setCell(31, MessageSourceUtils.getMessageByKey("om.active.payment"));
            }
            if ("30".equals(ipayType)) {
                e.setCell(32, MessageSourceUtils.getMessageByKey("sh.wechat.pay"));
            } else if ("40".equals(ipayType)) {
                e.setCell(32, MessageSourceUtils.getMessageByKey("sh.pay.with.ali.pay"));
            } else {
                e.setCell(32, "");
            }
            if ("10".equals(istatus)) {
                e.setCell(33, MessageSourceUtils.getMessageByKey("om.successful.payment"));
            } else if ("20".equals(istatus)) {
                e.setCell(33,MessageSourceUtils.getMessageByKey("om.payment.fail"));
            } else if ("30".equals(istatus)) {
                e.setCell(33, MessageSourceUtils.getMessageByKey("om.order.exception"));
            } else if ("100".equals(istatus)) {
                e.setCell(33, MessageSourceUtils.getMessageByKey("om.ending.payment"));
            } else if ("110".equals(istatus)) {
                e.setCell(33,MessageSourceUtils.getMessageByKey("om.payment.processing"));
            }
            ...
            if ("0".equals(iisDismantling)) {
                e.setCell(36, MessageSourceUtils.getMessageByKey("excel.false"));
            } else if ("1".equals(iisDismantling)) {
                e.setCell(36, MessageSourceUtils.getMessageByKey("excel.true"));
            }
            ...
            if ("10".equals(idismantlingType)) {
                e.setCell(38, MessageSourceUtils.getMessageByKey("excel.automatic"));
            } else if ("20".equals(idismantlingType)) {
                e.setCell(38, MessageSourceUtils.getMessageByKey("excel.manual"));
            }
        }
        e.downloadExcel(request, response, MessageSourceUtils.getMessageByKey("menu.order.list")+"-" + DateUtils.getCurrentDTimeNumber() + ".xls");
    }
    ...
    public ResponseVo delete(String checkboxId) {
        try {
            ...
            OrderRecord orderRecord = orderRecordService.selectByPrimaryKey(checkboxId);
            if (null == orderRecord) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("om.order.no.exist"));
            }
            if (null != orderRecord.getIstatus() && (20 == orderRecord.getIstatus() || 100 == orderRecord.getIstatus())) {
                ...
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("log.deletw.order.success"), orderRecord.getSorderCode());
                LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
                return ResponseVo.getSuccessResponse();
            } else {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("om.order.status.edit.error="));
            }
        } catch (Exception e) {
    ...
    @ResponseBody
    public ResponseVo cancel(String checkboxId) {
        try {
            return orderRecordService.cancelOrder(checkboxId);
        } catch (Exception e) {
            logger.error("撤销订单异常,订单ID:{}", checkboxId);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("撤销订单异常！");
        }
    }
}
