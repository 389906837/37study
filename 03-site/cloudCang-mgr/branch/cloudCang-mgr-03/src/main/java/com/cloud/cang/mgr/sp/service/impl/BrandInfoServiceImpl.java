...
//		CommodityInfo commodityInfo = null;
		...
		for (String brandId : checkboxId) {
			...
			if (CollectionUtils.isNotEmpty(commoditySkuList)) {
				throw new ServiceException(MessageSourceUtils.getMessageByKey("spcon.delete.brand.exception",null));
			}
			...
			if (CollectionUtils.isNotEmpty(commodityInfoList)) {
				throw new ServiceException(MessageSourceUtils.getMessageByKey("spcon.delete.brand.status.error",null));
			}
