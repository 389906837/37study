        ...
        try {
            ...
            if (StringUtils.isNotBlank(sid)) {
                ...
                if (deviceCommodityDomain != null) {
                    ...
                    //操作日志
                    String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("log.query.device.commodity"), deviceCommodityDomain.getScommodityCode());
    ...
    ResponseVo<String> dropOffDeviceCommodity(String[] checkboxId) {
        try {
            ...
            if (ArrayUtils.isNotEmpty(checkboxId)) {
                ...
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("log.drop.device.commodity"),"");
                LogUtil.addOPLog(LogUtil.AC_VIEW, logText, null);
                return ResponseVo.getSuccessResponse(MessageSourceUtils.getMessageByKey("sb.device.remove.success"));
            }
        ...
        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("sb.device.remove.fail"));
    }
    /* ----------4.设备商品信息结束 ----------*/
