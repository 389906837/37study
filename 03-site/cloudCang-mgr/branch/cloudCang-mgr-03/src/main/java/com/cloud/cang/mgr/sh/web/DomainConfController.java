...
public class DomainConfController {
    ...
    ResponseVo<DomainConf> save(DomainConf domainConf) {
        try {
            if (StringUtils.isBlank(domainConf.getSmerchantId())) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("shcon.add.domain.conf.sh.empty",null));
            }
            if (StringUtils.isBlank(domainConf.getSdomainUrl())) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("shcon.add.domain.conf.name.empty",null));
            }
            // 执行新增
            if (StringUtils.isBlank(domainConf.getId())) {

                if (domainConfService.isUrlExist(domainConf.getSdomainUrl())) {
                    return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo(MessageSourceUtils.getMessageByKey("shcon.add.domain.conf.exist",null));
                }
                ...
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("shcon.add.domain.conf.log",null), domainConf.getId());
                ...
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("shcon.edit.domain.conf.log",null), domainConf.getId());
                LogUtil.addOPLog(LogUtil.AC_EDIT, logText, null);
                return ResponseVo.getSuccessResponse(domainConf);
            }
        } catch (Exception ex) {
            log.error("修改增加域名异常：{}", ex);
            return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo(MessageSourceUtils.getMessageByKey("shcon.save.domain.conf.error",null));
        }
    ...
    public ResponseVo cache() {
        ...
        try {
            ...
            return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo(MessageSourceUtils.getMessageByKey("shcon.sync.cache.domain.conf.error",null));
        }
            ...
            public void run() {
                if (StringUtils.isNotBlank(domainConf.getSdomainUrl())) {
                    try {
                        ...
                        throw new ServiceException(MessageSourceUtils.getMessageByKey("shcon.save.domain.conf.exception",null));
                    }
                }
            }
        });
    ...
    ResponseVo<String> delete(String[] checkboxId) {
        try {
            ...
            //操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("shcon.delete.domain.conf.log",null), checkboxId);
    ...
    ResponseVo<DomainConf> saveExamine(DomainConf domainConf, String up) {
        try {
            if (null != domainConf && 30 == domainConf.getIstatus() && StringUtils.isBlank(domainConf.getSauditOpinion())) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("shcon.sudit.domain.conf.reason.empty",null));
            }
            ...
            //操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("shcon.modify.domain.conf.log",null), domainConf.getId());
            LogUtil.addOPLog(LogUtil.AC_EDIT, logText, null);
            return ResponseVo.getSuccessResponse(domainConf);
            ...
            return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo(MessageSourceUtils.getMessageByKey("shcon.modify.domain.conf.error",null));
        }
    ...
    ResponseVo<DomainConf> selectDomain(DomainConf domainConf) {
        try {
            ...
            return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo(MessageSourceUtils.getMessageByKey("shcon.view.domain.conf.error",null));
        }
    ...
    ResponseVo<String> enable(String checkboxId, Integer type) {
        try {
            if (StringUtils.isBlank(checkboxId)) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("shcon.enable.domain.conf.error",null));
            }
            ...
            domainConfService.updateBySelective(domainConf);
            //操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("shcon.enable.domain.conf.log",null), domainConf.getId());
            LogUtil.addOPLog(LogUtil.AC_EDIT, logText, null);
            return ResponseVo.getSuccessResponse();
        } catch (Exception e) {
            log.error("商户域名启停用异常：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("shcon.enable.domain.conf.exception",null));
        }
    }
