...
public class OperatorServiceImpl extends GenericServiceImpl<Operator, String> implements
    ...
    public void delete(String[] ids) {
        if (ids != null && ids.length > 0) {
            ...
            for (String id : ids) {
                ...
                if (null == operator) {
                    throw new ServiceException(MessageSourceUtils.getMessageByKey("syscon.delete.operator.exception",null));
                }
                if (null != operator.getBisFreeze() && 1 == operator.getBisFreeze()) {
                    throw new ServiceException(MessageSourceUtils.getMessageByKey("syscon.delete.operator.delete",null));
                }
                // 执行逻辑删除
    ...
    public void loginSuccess(String userId, String host) {
        ...
        String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.login.log",null), operator.getSrealName(), new Date(), host);
    ...
    public void syncData(MerchantInfo merchantInfo, String merchantUserName) {
        ...
        if (this.isUsernameExist(merchantUserName)) {
            throw new ServiceException(MessageSourceUtils.getMessageByKey("syscon.sync.system.error",null));
        }
        try {
            ...
            throw new ServiceException(MessageSourceUtils.getMessageByKey("syscon.sync.system.fail",null));
        }
    }
