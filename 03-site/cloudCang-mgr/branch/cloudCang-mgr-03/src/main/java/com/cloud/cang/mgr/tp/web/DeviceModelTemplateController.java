...
public class DeviceModelTemplateController extends GenericController {
    ...
    public ResponseVo<DeviceModelTemplate> commodityAdd(DeviceModelTemplate deviceModelTemplate) {
        try {
            ...
            if (BooleanUtils.isTrue(responseVo.isSuccess())) {
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("tpcon.add.device.details.template",null)+"，"+MessageSourceUtils.getMessageByKey("main.name",null)+"{0}", sname);
    ...
    public ResponseVo<String> delete(String[] checkboxId) {
        ...
        try {
            if (ArrayUtils.isNotEmpty(checkboxId)) {
                ...
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("tpcon.delete.device.details.template",null), "");
        ...
        try {
            if (StringUtils.isNotBlank(sid)) {
                ...
                if (null != deviceModelTemplate) {
                    map.put("deviceModelTemplate", deviceModelTemplate);
                    String logText4 = MessageFormat.format(MessageSourceUtils.getMessageByKey("tpcon.edit.device.details.template",null)+"{0}", deviceModelTemplate.getScode());
    ...
    public ResponseVo<DeviceModelTemplate> edit(DeviceModelTemplate deviceModelTemplate) {
        ...
        try {
            ...
            //操作日志
            String logText1 = MessageFormat.format(MessageSourceUtils.getMessageByKey("tpcon.modify.device.details.template",null)+"，"+ MessageSourceUtils.getMessageByKey("main.code",null)+"{0}", responseVo.getData().getScode());
        ...
        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("tpcon.modify.device.details.template.error",null));
    }
    ...
    public String brandInfoView(String sid, ModelMap map) {
        try {
            if (StringUtils.isNotBlank(sid)) {
                ...
                if (null != deviceModelTemplate) {
                    map.put("deviceModelTemplate", deviceModelTemplate);
                    String logText4 = MessageFormat.format(MessageSourceUtils.getMessageByKey("tpcon.view.device.details.template",null)+"，"+MessageSourceUtils.getMessageByKey("main.name",null)+"{0}", deviceModelTemplate.getScode());
                    LogUtil.addOPLog(LogUtil.AC_VIEW, logText4, null);
