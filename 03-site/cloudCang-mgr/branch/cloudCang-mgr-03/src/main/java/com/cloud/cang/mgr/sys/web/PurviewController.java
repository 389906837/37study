...
public class PurviewController extends GenericController {
    ...
    ResponseVo<String> delete(String[] checkboxId) {
        try {
            ...
            // 操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.delete.purview.log",null), checkboxId);
    ...
    ResponseVo<String> syncPurview(String[] checkboxId) {
        try {
            ...
            // 操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.sync.purview.log",null), checkboxId);
            LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
            return responseVo;
        } catch (Exception e) {
            log.error("权限码同步到商户异常：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("syscon.sync.purview.error",null));
        }
    }
    ...
    ResponseVo<Purview> save(Purview purview) {
        try {
            if (StringUtils.isBlank(purview.getSparentId())) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("syscon.purview.menu.id.empty",null));
            }
            ...
            if (StringUtils.isBlank(purview.getId())) {
                if (purviewService.isNameExist(purview)) {
                    return ErrorCodeEnum.ERROR_COMMON_UNIQUE.getResponseVo(MessageSourceUtils.getMessageByKey("syscon.purview.exist",null));
                }
                ...
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.add.purview.log",null)+"，"+ MessageSourceUtils.getMessageByKey("main.code",null)+"{0}", purview.getSpurNo());
                ...
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.modify.purview.log",null)+"，"+MessageSourceUtils.getMessageByKey("main.code",null)+"{0}", presitPurview.getSpurNo());
                LogUtil.addOPLog(LogUtil.AC_EDIT, logText, null);
            }

        } catch (Exception ex) {
            log.error("保存/修改权限码异常：{}", ex);
            return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo(MessageSourceUtils.getMessageByKey("syscon.save.purview.error",null));
        }
