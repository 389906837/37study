...
public class DeviceStockController extends GenericController {
    ...
    ResponseVo<DeviceStock> updateComFsalePrice(DeviceStock deviceStock) {
        try {
            if (StringUtil.isBlank(deviceStock.getId())) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("smcon.modify.stock.product.exception",null));
            }
            if (null == deviceStock.getFsalePrice()) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("smcon.modify.stock.sale.exception",null));
            }
            if (null == deviceStock.getIstock()) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("smcon.modify.stock.num.exception",null));
            }
            // 20181129 修改库存逻辑--zzx
            ...
            return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo(MessageSourceUtils.getMessageByKey("smcon.modify.stock.exception",null));
        }
    }
    ...
    ResponseVo<String> syncComFsalePrice(String[] checkboxId) {
        try {
            ...
            responseVo.setMsg(MessageSourceUtils.getMessageByKey("smcon.sync.stock.success",null));
            // 操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("smcon.sync.stock.log",null), checkboxId);
            LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
            return responseVo;
        } catch (Exception e) {
            log.error("同步商品单价到库存表异常：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("smcon.sync.stock.exception",null));
        }
    }
    ...
    public ResponseVo saveUndercarriage(DeviceStock deviceStock, Integer unType, Integer unNum) {
        try {
            if (null == unType) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("smcon.obtained.stock.product.type.error",null));
            }
            //判断是否为整数
           /* Pattern pattern = Pattern.compile("^//d+$");
            if(!pattern.matcher(unNum.toString()).matches()){
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("smcon.obtained.stock.num.error",null));
            }*/
            ...
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("smcon.obtained.stock.error",null));
        }
    }
}
