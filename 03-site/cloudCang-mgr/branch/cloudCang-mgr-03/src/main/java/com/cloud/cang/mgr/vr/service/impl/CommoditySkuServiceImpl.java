...
public class CommoditySkuServiceImpl extends GenericServiceImpl<CommoditySku, String> implements
            ...
            if (CollectionUtils.isNotEmpty(commodityInfoList)) {
                for (CommodityInfo commodityInfo1 : commodityInfoList) {
                    ...
                    if (commodityStatus != null && commodityStatus == 10) {
                        throw new ServiceException(MessageSourceUtils.getMessageByKey("vrcon.delete.visual.goods.exception",null));
                    }
                }
            }
        }
    ...
    public ResponseVo<String> updateVrCommodity(CommoditySku commoditySku, MultipartFile scommodityImgfile) {
        ...
        if (StringUtils.isBlank(sbrandId) || StringUtils.isBlank(sbrandName)) {
            throw new ServiceException(MessageSourceUtils.getMessageByKey("vrcon.brandinfo.empty",null));
        }
    ...
    public ResponseVo<String> updateCommodityStatus(String[] checkboxId, String status) {
                ...
                if (CollectionUtils.isNotEmpty(commodityInfoList)) {
                    for (CommodityInfo commodityInfo1 : commodityInfoList) {
                        ...
                        if (commodityIstatus == 10) {
                            throw new ServiceException(MessageSourceUtils.getMessageByKey("vrcon.disable.visual.goods.exception",null));
                        }
                    }
                }
        ...
        return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("vrcon.modify.visual.goods.status.error",null));
    }
                ...
                if (CollectionUtils.isNotEmpty(commodityInfoList)) {
                    for (CommodityInfo commodityInfo1 : commodityInfoList) {
                        ...
                        if (commodityInfo1Istatus == 10) {    // 正常状态
                            throw new ServiceException(MessageSourceUtils.getMessageByKey("vrcon.modify.visual.goods.status.exception",null));
                        }
                    }
                }
    ...
    public ResponseVo<String> batchUploadVrComInfo(MultipartFile file) throws Exception {
        ...
        if (null == workbook) {
            logger.error("工作薄不存在");
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("vr.batch.upload.workbook.empty"));
        }
        ...
        if (CollectionUtils.isEmpty(vrCommodityList)) {
            ...
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("vr.batch.upload.data.empty"));
        }
