			...
			if (msgTemplateMain != null){
				...
				for (MsgTemplate msgTemplate : msgTemplateList){
					if (msgTemplate.getIisEnable().equals(1)){
						throw new ServiceException(MessageSourceUtils.getMessageByKey("xxcon.delete.template.error",null)+"【"+msgTemplate.getStemplateName()+"】，"+MessageSourceUtils.getMessageByKey("main.delete.error",null)+"！");
					}
...
//				msgTemplateMain.setIdelFlag(1);
				...
				// 操作日志
				String logText= MessageFormat.format(MessageSourceUtils.getMessageByKey("xxcon.delete.template",null)+" "+ MessageSourceUtils.getMessageByKey("main.name",null)+"{0},"+MessageSourceUtils.getMessageByKey("main.code",null)+"{1}",msgTemplateMain.getSmsgName(),msgTemplateMain.getScode() );
