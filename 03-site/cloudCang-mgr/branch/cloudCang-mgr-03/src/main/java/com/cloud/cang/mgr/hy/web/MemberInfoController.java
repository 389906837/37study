...
public class MemberInfoController {
    ...
    ResponseVo<MemberInfo> save(MemberInfo memberInfo) {
        ...
        try {
            ...
            if (StringUtils.isBlank(memberInfo.getId()) && StringUtils.isNotBlank(merchantId)) {
                ...
                if (StringUtils.isNotEmpty(merchantCode)) {
                    ...
                    // 操作日志
                    String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.hy.amutn", null) + MessageSourceUtils.getMessageByKey("main.code", null) + "{1}", memberInfo.getSrealName(), memberInfo.getScode());
                ...
                // 操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.hy.mtmutn", null) + MessageSourceUtils.getMessageByKey("main.code", null) + "{1}", memberInfo.getSrealName(), memberInfo.getScode());
        ...
        return new ResponseVo<>(false, 111, MessageSourceUtils.getMessageByKey("con.hy.miieacba", null));
    }
    ...
    ResponseVo<String> delete(String[] checkboxId) {
        try {
            if (ArrayUtils.isNotEmpty(checkboxId)) {
                ...
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.hy.ddadri", null), checkboxId);
                LogUtil.addOPLog(LogUtil.AC_DELETE, logText, null);
                return ResponseVo.getSuccessResponse();
            }
        } catch (Exception e) {
            logger.error("系统异常删除设备与设备注册失败！", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("con.hy.tsiadatdria", null));
        }
        return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("con.hy.sadodadrf", null));
    }
    ...
    ResponseVo<String> resetPass(String sid) {
        ...
        try {
            //操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.hy.rupmui", null), sid);
    ...
    ResponseVo<String> enableRecoaward(String checkboxId, Integer type) {
        ...
        try {
            ...
            // 操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.hy.imr", null) + MessageSourceUtils.getMessageByKey("main.code", null) + "{0}", memberInfo.getScode());
            LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
            memberInfoService.updateBySelective(memberInfo);
            return ResponseVo.getSuccessResponse();
        } catch (Exception e) {
            logger.error("推荐奖励是否启用/禁用异常：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("con.hy.drmrf", null));
        }
    }
    ...
    ResponseVo<String> freezeAccount(String checkboxId, Integer type) {
        ...
        try {
            ...
            // 操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.hy.ft", null) + MessageSourceUtils.getMessageByKey("main.code", null) + "{0}", memberInfo.getScode());
            LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
            memberInfoService.updateBySelective(memberInfo);
            return ResponseVo.getSuccessResponse();
        } catch (Exception e) {
            logger.error("冻结/解冻异常：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("con.hy.ftf", null));
        }
    }
    ...
    ResponseVo<String> blackList(String checkboxId, Integer type) {
        try {
            ...
            memberInfoService.updateBySelective(memberInfo);
            // 操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.hy.abs", null), memberInfo.getIstatus());
            LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
            return ResponseVo.getSuccessResponse();
        } catch (Exception e) {
            logger.error("加入黑名单异常：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("con.hy.ftjtb", null));
        }
    }
    ...
    public void downloadExcel(HttpServletRequest request, HttpServletResponse response, MemberInfoVo memberInfoVo) {
        ...
        String[] names = new String[]{
                ...
                MessageSourceUtils.getMessageByKey("con.hy.mm"),
                MessageSourceUtils.getMessageByKey("con.hy.mu"),
                MessageSourceUtils.getMessageByKey("main.merchant.name"),
                MessageSourceUtils.getMessageByKey("con.hy.gender"),
                MessageSourceUtils.getMessageByKey("con.hy.mpn"),
                MessageSourceUtils.getMessageByKey("con.hy.rt"),
                MessageSourceUtils.getMessageByKey("con.hy.ms"),
                MessageSourceUtils.getMessageByKey("con.hy.tom"),
                MessageSourceUtils.getMessageByKey("hy.membership.level"),
                MessageSourceUtils.getMessageByKey("hy.registered.device.number"),
                MessageSourceUtils.getMessageByKey("hy.registration.platform"),
                MessageSourceUtils.getMessageByKey("xx.is.it.the.first"),
                MessageSourceUtils.getMessageByKey("hy.real.name.certification"),
                MessageSourceUtils.getMessageByKey("hy.is.the.first.time.tied.to.the.card"),
                MessageSourceUtils.getMessageByKey("hy.recent.shopping.time")};
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (sex.equals("0")) {
                e.setCell(4, MessageSourceUtils.getMessageByKey("main.female"));
            } else {
                e.setCell(4, MessageSourceUtils.getMessageByKey("main.male"));
            }
            e.setCell(5, String.valueOf(obj.get("SMOBILE")));
            e.setCell(6, String.valueOf(obj.get("TREGISTER_TIME")));
            if (status.equals("1")) {
                e.setCell(7, String.valueOf(MessageSourceUtils.getMessageByKey("main.normal", null)));
            }
            if (status.equals("2")) {
                e.setCell(7, MessageSourceUtils.getMessageByKey("xx.logout.disabled"));
            }
            if (status.equals("3")) {
                e.setCell(7, MessageSourceUtils.getMessageByKey("xx.system.blacklist"));
            }
            if (status.equals("4")) {
                e.setCell(7, MessageSourceUtils.getMessageByKey("xx.freeze"));
            }
            if (type.equals("10")) {
                e.setCell(8, MessageSourceUtils.getMessageByKey("sl.shopping.member"));
            }
            if (type.equals("20")) {
                e.setCell(8, MessageSourceUtils.getMessageByKey("sl.replenishment.member"));
            }
            if (level.equals("10")) {
                e.setCell(9, MessageSourceUtils.getMessageByKey("hy.public.member"));
            }
            if (level.equals("20")) {
                e.setCell(9, MessageSourceUtils.getMessageByKey("hy.gold.member"));
            }
            if (level.equals("30")) {
                e.setCell(9, MessageSourceUtils.getMessageByKey("hy.platinum.member"));
            }
            if (level.equals("40")) {
                e.setCell(9,MessageSourceUtils.getMessageByKey("hy.diamond.membership"));
            }
            if (code.equals("null")) {
            ...
            if (ctype.equals("10")) {
                e.setCell(11, MessageSourceUtils.getMessageByKey("main.wechat.pay"));
            }
            if (ctype.equals("20")) {
                e.setCell(11, MessageSourceUtils.getMessageByKey("main.alipay"));
            }
            if (ctype.equals("30")) {
                e.setCell(11, MessageSourceUtils.getMessageByKey("main.wechat.public.number"));
            }
            if (ctype.equals("40")) {
                e.setCell(11, MessageSourceUtils.getMessageByKey("main.alipay.life"));
            }
            if (ctype.equals("50")) {
            ...
            if (order.equals("0")) {
                e.setCell(12, MessageSourceUtils.getMessageByKey("excel.false"));
            }
            if (order.equals("1")) {
                e.setCell(12,  MessageSourceUtils.getMessageByKey("excel.true"));
            }
            if (verified.equals("0")) {
                e.setCell(13, MessageSourceUtils.getMessageByKey("excel.false"));
            }
            if (verified.equals("1")) {
                e.setCell(13, MessageSourceUtils.getMessageByKey("excel.true"));
            }
            if (card.equals("0")) {
                e.setCell(14, MessageSourceUtils.getMessageByKey("excel.false"));
            }
            if (card.equals("1")) {
                e.setCell(14, MessageSourceUtils.getMessageByKey("excel.true"));
            }
        ...
        e.downloadExcel(request, response, MessageSourceUtils.getMessageByKey("menu.membership.list")+"-" + DateUtils.getCurrentDTimeNumber() + ".xls");
    }
    ...
    ResponseVo<String> rechargeAgain(String checkboxId, Integer type, HttpServletRequest request) {
        ...
        if (StringUtils.isBlank(checkboxId) || null == type) {
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("hy.recharge.again.param.error"));
        }
        try {
            ...
            if (0 == type) {
                ...
                if (null != responseVo && responseVo.isSuccess() && null != responseVo.getData()) {
                   ...
                   /* if ("NORMAL".equals(responseVo.getData().getStatus())) {*/
                    ...
                    try {
                        ResponseVo responseVo1 = memberInfoService.rechargeAgainUpData(responseVo.getData(), request, memberInfo.getSmerchantCode());
                        if (responseVo1.isSuccess()) {
                            logger.info("支付宝免密支付补处理成功！");
                            return ResponseVo.getSuccessResponse(MessageSourceUtils.getMessageByKey("hy.recharge.again.alipay.free.secret.success"));

                        } else {
                            logger.info("支付宝补处理失败，参数{}" + JSON.toJSONString(responseVo1));
                            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("hy.recharge.again.alipay.free.secret.fail"));
                        }
                    } catch (Exception e) {
                        logger.error("支付宝补处理异常", e);
                        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("hy.recharge.again.alipay.error"));
                    }
                    ...
                    return ResponseVo.getSuccessResponse(MessageSourceUtils.getMessageByKey("hy.recharge.again.alipay.success"));
                }
            } else if (1 == type) {
                QuerySignDto querySignDto = new QuerySignDto();
                querySignDto.setSmemberId(checkboxId);
                ...
                if (null != responseVo && responseVo.isSuccess() && null != responseVo.getData()) {
                  ...
                  /*  if ("0".equals(responseVo.getData().getContract_state())) {*/
                    ...
                    try {
                        ResponseVo responseVo1 = memberInfoService.WechantRechargeAgainUpData(responseVo.getData(), request, memberInfo.getSmerchantCode());
                        if (responseVo1.isSuccess()) {
                            logger.info("微信免密支付补处理成功！");
                            return ResponseVo.getSuccessResponse(MessageSourceUtils.getMessageByKey("hy.recharge.again.weixin.free.secret.success"));

                        } else {
                            logger.info("微信免密支付补处理失败，参数{}" + JSON.toJSONString(responseVo1));
                            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("hy.recharge.again.weixin.free.secret.fail"));
                        }
                    } catch (Exception e) {
                        logger.error("微信免密支付补处理异常，参数{}" + e);
                        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("hy.recharge.again.weixin.free.secret.error"));
                    }
                 /*   }*/
                    ...
                    updata.setUpateUser(SessionUserUtils.getSessionAttributeForUserDtl().getRealName());
                    memberInfoService.updateBySelective(updata);
                    return ResponseVo.getSuccessResponse(MessageSourceUtils.getMessageByKey("hy.recharge.again.weixin.success"));
                }
            } else if (type == 2) {
                QueryUserAvaiDto queryUserAvaiDto = new QueryUserAvaiDto();
                queryUserAvaiDto.setSmemberId(memberInfo.getId());
                queryUserAvaiDto.setSmerchantCode(memberInfo.getSmerchantCode());
                // 创建Rest服务客户端
                RestServiceInvoker invoke = RestServiceInvokeBuilder.newBuilder().newInvoker(FreeServicesDefine.WECHAT_POINT_QUERY_USER_AVAILABILITY);
                invoke.setRequestObj(queryUserAvaiDto);
                // 返回对象中含有泛型，则需要设置返回类型，否则无需设置
                invoke.setParameterizedTypeReference(new ParameterizedTypeReference<ResponseVo<QueryUserAvaiResult>>() {
                });
                ResponseVo<QueryUserAvaiResult> responseVo = (ResponseVo<QueryUserAvaiResult>) invoke.invoke();
                if (null != responseVo && responseVo.isSuccess() && null != responseVo.getData()) {
                    String status = responseVo.getData().getUse_service_state();
                /*    UNAVAILABLE - 用户未开启服务；
                    AVAILABLE - 用户已开启服务*/
                    logger.info("开始支付宝免密支付补处理！");
                    try {
                        ResponseVo responseVo1 = memberInfoService.wechatPointRechargeAgainUpData(responseVo.getData(), request, memberInfo.getSmerchantCode());
                        if (responseVo1.isSuccess()) {
                            logger.info("微信支付分补处理成功！");
                            return ResponseVo.getSuccessResponse("微信支付分补处理成功！");

                        } else {
                            logger.info("微信支付分补处理失败，参数{}" + JSON.toJSONString(responseVo1));
                            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("微信支付分补处理失败！");
                        }
                    } catch (Exception e) {
                        logger.error("微信支付分补处理异常", e);
                        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("微信支付分补处理异常！");
                    }
                } else if (null != responseVo && null == responseVo.getData() && 1 == memberInfo.getIwechatPointOpen()) {
                    MemberInfo updata = new MemberInfo();
                    updata.setId(memberInfo.getId());
                    updata.setIwechatPointOpen(0);
                    updata.setUpdateTime(DateUtils.getCurrentDateTime());
                    updata.setUpateUser(SessionUserUtils.getSessionAttributeForUserDtl().getRealName());
                    memberInfoService.updateBySelective(updata);
                    return ResponseVo.getSuccessResponse("微信支付分补处理成功！");
                ...
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("hy.recharge.again.type.error"));
            }
        } catch (Exception e) {
            logger.error("支付宝补处理异常，参数{}" + e);
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("hy.recharge.again.error"));
        }
        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("hy.recharge.again.free.secret.fail"));
    }
     ...
     * @return
     */
    @RequestMapping("/manualTermination")
    @ResponseBody
    public ResponseVo manualTermination(String checkboxId, HttpServletRequest request) {
        try {
            if (StringUtils.isBlank(checkboxId)) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("hy.manualTermination.select.member"));
            }
            MemberInfo memberInfo = memberInfoService.selectByPrimaryKey(checkboxId);
            if (null == memberInfo) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("hy.manualTermination.not.exist"));
            }
            return memberInfoService.manualTermination(memberInfo, request);
        } catch (Exception e) {
            logger.error("用户支付宝手动解约异常：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("hy.manualTermination.fail"));
        }
    }
}
