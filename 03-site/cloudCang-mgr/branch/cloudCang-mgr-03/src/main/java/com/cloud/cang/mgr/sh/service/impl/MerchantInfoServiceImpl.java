...
public class MerchantInfoServiceImpl extends GenericServiceImpl<MerchantInfo, String> implements
        ...
        if (ids != null && ids.length > 0) {
            for (String id : ids) {
                ...
                if (null == merchantInfo) {
                    log.info(MessageSourceUtils.getMessageByKey("shcon.update.merchant.empty",null));
                    throw new ServiceException(MessageSourceUtils.getMessageByKey("shcon.update.merchant.empty",null));
                }
                //1.绑定了设备的商户不能删除
                List<DeviceInfo> list = deviceInfoService.selectAllDeviceInfoByMerchantId(id);
                if (null != list && list.size() > 0) {
                    log.info("已有设备绑定了该商户，"+MessageSourceUtils.getMessageByKey("main.delete.error",null)+" ，"+MessageSourceUtils.getMessageByKey("main.merchant.number",null)+"：{}", merchantInfo.getScode());
                    throw new ServiceException(MessageSourceUtils.getMessageByKey("shcon.add.merchant.exception",null)+","+MessageSourceUtils.getMessageByKey("main.delete.error",null)+"！");
                }
                //2.有通过审核域名的商户不能删除
                String url = domainConfService.selectThroughAuditByMerchantId(id);
                if (StringUtils.isNotBlank(url)) {
                    log.info(MessageSourceUtils.getMessageByKey("shcon.add.merchant.error",null)+"，"+MessageSourceUtils.getMessageByKey("main.delete.error",null)+" ，"+MessageSourceUtils.getMessageByKey("main.merchant.number",null)+"：{}", merchantInfo.getScode());
                    throw new ServiceException(MessageSourceUtils.getMessageByKey("shcon.add.merchant.error",null)+","+MessageSourceUtils.getMessageByKey("main.delete.error",null)+"！");
                }
                //3.签约通过的商户不能删除
                if (BizTypeDefinitionEnum.IcompanyStatus.ALREADYSIGNED.getCode() == merchantInfo.getIstatus()) {
                    log.info("该商户已签约，"+MessageSourceUtils.getMessageByKey("main.delete.error",null)+" ，"+MessageSourceUtils.getMessageByKey("main.merchant.number",null)+"：{}", merchantInfo.getScode());
                    throw new ServiceException(MessageSourceUtils.getMessageByKey("shcon.add.merchant.sign.error",null)+","+MessageSourceUtils.getMessageByKey("main.delete.error",null)+"！");
                }
    ...
    private boolean selectMax(String sparentId, int max) {
        ...
        if (!"0".equals(merchantInfo.getSrootId())) {
            ...
            if (max > Integer.valueOf(SysParaUtil.getValue("merchantInfoMax"))) {
                throw new ServiceException(MessageFormat.format(MessageSourceUtils.getMessageByKey("shcon.add.merchant.max.level.error",null), SysParaUtil.getValue("merchantInfoMax")));
            }
    ...
    public ResponseVo insertNewMerchant(MultipartFile merLogofile, MultipartFile loginLogofile, MerchantInfo merchantInfo, String merchantUserName, MerchantDetail merchantDetail, MerchantClientConf merchantClientConf, String KFscontactMobile) {
        if (this.isExistName(merchantInfo.getSname())) {
            throw new ServiceException(MessageSourceUtils.getMessageByKey("shcon.add.merchant.name.error",null));
        }
        ...
        if (null != merchantInfo.getIdbWap() && 20 == merchantInfo.getIdbWap()) {
            ...
            if (null == operator) {
                throw new ServiceException(MessageSourceUtils.getMessageByKey("shcon.update.merchant.bd.error",null));
            }
        ...
        //操作日志
        String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("log.add.merchant"), merchantInfo.getId());
    ...
    public ResponseVo upMerchant(MultipartFile merLogofile, MultipartFile loginLogofile, MerchantInfo merchantInfo, MerchantDetail merchantDetail, MerchantClientConf merchantClientConf, String merchantDetailId, String KFscontactMobile) {
        ...
        if (!merchantInfo.getSname().equals(merchantInfo1.getSname())) {
            if (this.isExistName(merchantInfo.getSname())) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("shcon.add.merchant.name.error",null));
            }
        }
        merchantInfo.setTupdateTime(DateUtils.getCurrentDateTime());
        merchantInfo.setSupdateUser(SessionUserUtils.getSessionAttributeForUserDtl().getRealName());
        ...
        //操作日志
        String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("shcon.edit.merchant.conf.log",null)+"，"+ MessageSourceUtils.getMessageByKey("main.code",null)+"{0}", merchantInfo.getScode());
