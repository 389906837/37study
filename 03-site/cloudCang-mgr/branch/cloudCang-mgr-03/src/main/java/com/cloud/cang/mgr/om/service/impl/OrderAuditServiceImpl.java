...
public class OrderAuditServiceImpl extends GenericServiceImpl<OrderAudit, String> implements
    ...
    public ResponseVo payOrder(String checkboxId) {
        ...
        try {
            ...
            if (null != responseVo && responseVo.isSuccess() && null != responseVo.getData()) {
                ...
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("om.order.pay.fail"));
            }
        } catch (Exception e) {
        ...
        //操作日志
        String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("log.pay.abnormal.order.success"), orderAudit.getSorderCode());
        LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
        //异步支付异常订单
        payErrorOrder(responseVo, orderAudit);

        return ResponseVo.getSuccessResponse(MessageSourceUtils.getMessageByKey("om.abnormal.order.pay.success"));
    }
     ...
     * @param
     */
    private void payErrorOrder(final ResponseVo<GeneratingOrderResults> responseVo,final OrderAudit orderAudit) {
            ...
            public void run() {
                try {
                    ...
                    for (CreatOrderResult creatOrderResult : creatOrderResultList) {
                        ...
                        freePaymentDto.setIpayWay(BizTypeDefinitionEnum.PayWay.WITHHOLDING.getCode());//代扣
                        freePaymentDto.setIisIgnoreStatus(0);

                        Map<String,String> map = JSONObject.parseObject(orderAudit.getSext(), Map.class);
                        freePaymentDto.setUserAuthToken(map.get("appUserAuthToken"));
                        freePaymentDto.setSessionID(map.get("appSessionID"));
                        freePaymentDto.setAppDeviceId(map.get("appDeviceId"));
                        freePaymentDto.setDeviceCode(orderRecord.getSdeviceCode());
                        freePaymentDto.setDeviceId(orderRecord.getSdeviceId());
                        freePaymentDto.setSremark("dealwith");

                        RestServiceInvoker invoke = RestServiceInvokeBuilder.newBuilder().newInvoker(FreeServicesDefine.VENDSTOP_PAYMENT);// 服务名称
                        invoke.setRequestObj(freePaymentDto);
    ...
    public ResponseVo saveOrderAudit(OrderAudit orderAudit, HttpServletRequest request, String commodityIds) throws Exception {
        ...
        for (int i = 0; i < arr.length; i++) {
            ...
            if (null == orderCommodityEditVo) {
                ...
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("sm.saveorder.not.exist.item"));
