    ...
    ResponseVo<String> saveOperatorRole(String[] operatorRoleIds) {
        ...
        /*    if (operatorRoleIds == null || operatorRoleIds.length <= 0) {
            ...
            if (null != operatorRoleIds && operatorRoleIds.length > maxNum) {
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.add.fast.menu.error",null),maxNum));
            }
            fastMenuService.saveFastMenu(operatorRoleIds, SessionUserUtils.getSessionAttributeForUserDtl().getId());
            // 操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.add.fast.menu",null)+"{0}", operatorRoleIds);
