		...
		for (String id:checkboxId) {
			...
			if (supplierInfo != null){
				...
				if (list != null && list.size() > 0){
					throw new ServiceException("协议/模板启用状态不能删除！");
				}
				if (supplierInfo.getIisDisable().equals(0)){
					throw new ServiceException("短信供应商为启用状态不能删除！");
				}
				supplierInfo.setIdelFlag(1);
				supplierInfoDao.updateByPrimaryKey(supplierInfo);
				//操作日志
				String logText= MessageFormat.format("删除短信供应商 名称{0},编号{1}",supplierInfo.getSname(),supplierInfo.getScode() );
