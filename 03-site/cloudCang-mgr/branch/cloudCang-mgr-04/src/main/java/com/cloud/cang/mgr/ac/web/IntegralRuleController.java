    ...
    ResponseVo<IntegralRule> save(IntegralRule integralRule) {
        try {
            ...
            if (null == integralRule.getIgiveMethod()) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("请选择赠送方式！");
            }
            if (integralRule.getIgiveMethod() == 10) {
                if (null == integralRule.getFminValue()) {
                    return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("最小积分值不能为空！");
                }
                if (null == integralRule.getFmaxValue()) {
                    return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("最大积分值不能为空！");
                }
            }
            boolean flag = true;
            ...
            if (flag) {// 执行新增
                //操作日志
                String logText = MessageFormat.format("新增场景活动送积分规则，编号{0}", integralRule.getScode());
                LogUtil.addOPLog(LogUtil.AC_ADD, logText, null);
            } else {// 执行修改
                //操作日志
                String logText = MessageFormat.format("编辑场景活动送积分规则，编号{0}", integralRule.getScode());
                LogUtil.addOPLog(LogUtil.AC_EDIT, logText, null);
            }
            return ResponseVo.getSuccessResponse(integralRule);
        } catch (Exception ex) {
            logger.error("保存券规则异常：{}", ex);
            return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo("保存送积分规则失败");
        }
    }
