    ...
    <select id="purviewSelect" multiple="multiple">
        <c:forEach items="${roleList}" var="item" varStatus="L">
            <c:if test="${item.bisroot eq '1'}">
                <%--<option value="${item.id}" data-section="权限管理/${item.menuName}" <c:if test="${item.isSelect eq 1}"> selected</c:if>>${item.text}</option>--%>
                <c:forEach varStatus="K" var="purItem" items="${item.nodes}">
                    <option value="${item.id}#//#${purItem.id}" data-section="权限配置/${item.menuName}/${purItem.menuName}" <c:if test="${purItem.isSelect eq 1}"> selected</c:if>>${purItem.text}</option>
                </c:forEach>
            </c:if>
