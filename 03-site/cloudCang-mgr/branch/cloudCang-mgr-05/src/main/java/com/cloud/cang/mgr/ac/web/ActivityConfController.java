...
public class ActivityConfController {
    ...
    ResponseVo<ActivityConf> saveScenes(ScenesVo scenesVo) {
        try {
            ...
            if (StringUtils.isBlank(scenesVo.getSconfName())) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.ancbe", null));
            }
            if (null == scenesVo.getIscenesType()) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.pstest", null));
            }
            ...
            if (null == scenesVo.getIallCount()) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.ttnopldtecbe", null));
            }
            if (null == scenesVo.getIcountType()) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.tnotltcbe", null));
            }
            if (null == scenesVo.getIuserAllCount()) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.tnotlttpcbe", null));
            }
            String limitStr = MessageSourceUtils.getMessageByKey("con.ac.user", null);
            if (scenesVo.getIuserAllCount().intValue() == BizTypeDefinitionEnum.ActivityCountType.DURING_ACTIVITY_EQUIPMENT.getCode()) {
                limitStr = MessageSourceUtils.getMessageByKey("con.ac.device", null);
            }
            if (scenesVo.getIallCount().intValue() > 0 && scenesVo.getIuserAllCount().intValue() > scenesVo.getIallCount().intValue()) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.ttnopdtambgt", null) + limitStr + MessageSourceUtils.getMessageByKey("con.ac.tnop", null));
            }
            if (null == scenesVo.getIuserDayCount()) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.tnotltsdpcbe", null));
            }
            if (scenesVo.getIuserAllCount().intValue() > 0 && scenesVo.getIuserDayCount().intValue() > scenesVo.getIuserAllCount().intValue()) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(limitStr + MessageSourceUtils.getMessageByKey("con.ac.ttnopmbgt", null) + limitStr + MessageSourceUtils.getMessageByKey("con.ac.sdp", null));
            }
            if (null == scenesVo.getTactiveStartTime()) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.astcbe", null));
            }
            if (null == scenesVo.getTactiveEndTime()) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.eetcbe", null));
            }
            if (scenesVo.getTactiveEndTime().before(scenesVo.getTactiveStartTime())) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.eetmbatest", null));
            }
            ...
            if (scenesVo.getIscenesType().intValue() != 30 && scenesVo.getIscenesType().intValue() != 40) {
                if (null == scenesVo.getIrangeDevice() ||
                        (scenesVo.getIrangeDevice().intValue() == 1 && (null == scenesVo.getDeviceIds() || scenesVo.getDeviceIds().length <= 0))) {
                    return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.apde", null));
                }
            } else {
            ...
            if (StringUtils.isNotBlank(scenesVo.getId())) {//编辑排除自己
                ...
                if (null == temp) {
                    return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.diasf", null));
                }
            ...
            if (null != tempAc && tempAc.size() > 0) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.tsaaeitms", null));
            }
            ...
            if (StringUtils.isBlank(scenesVo.getId())) {
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.ac.nsan", null), activityConf.getSconfCode());
                LogUtil.addOPLog(LogUtil.AC_ADD, logText, null);
            } else {// 执行修改
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.ac.esan", null), activityConf.getSconfCode());
                LogUtil.addOPLog(LogUtil.AC_EDIT, logText, null);
            }
            return ResponseVo.getSuccessResponse(activityConf);
        } catch (Exception ex) {
            logger.error("新增和修改 场景活动异常：{}", ex);
            return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.saf", null));
        }
    }
    ...
    ResponseVo<ActivityConf> savePromotions(PromotionsVo promotionsVo) {
        try {
            ...
            if (null == promotionsVo.getIdiscountWay()) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.psaeo", null));
            }
            if (promotionsVo.getIdiscountWay() % 10 != 0 || promotionsVo.getIdiscountWay() < 10 || promotionsVo.getIdiscountWay() > 50) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.admia", null));
            }
            if (promotionsVo.getIdiscountWay().intValue() == 10) {//首单
                if (null == promotionsVo.getFirstTargetMoney()) {
                    return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.tfomtcacbe", null));
                }
                if (null == promotionsVo.getFirstDiscountAmount()) {
                    return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.tfcacbe", null));
                }
            } else if (promotionsVo.getIdiscountWay().intValue() == 20) {//折扣
                if (null == promotionsVo.getDiscountSelect()) {
                    return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.psaedm", null));
                }
                if (promotionsVo.getDiscountSelect().intValue() == 30 || promotionsVo.getDiscountSelect().intValue() == 40) {
                    if (null == promotionsVo.getDiscountTargetNum() || promotionsVo.getDiscountTargetNum() <= 0) {
                        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.tnodcnbe", null));
                    }
                }
                if (promotionsVo.getDiscountSelect().intValue() == 20 || promotionsVo.getDiscountSelect().intValue() == 40) {
                    if (null == promotionsVo.getDiscountTargetMoney()) {
                        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.tdmtcatacbe", null));
                    }
                }
                if (null == promotionsVo.getFdiscount() || promotionsVo.getFdiscount().doubleValue() <= 0) {
                    return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.dcbe", null));
                }
            } else if (promotionsVo.getIdiscountWay().intValue() == 30) {//满减
                if (null == promotionsVo.getFullReductionSelect()) {
                    return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.pstafrm", null));
                }
                if (promotionsVo.getFullReductionSelect().intValue() == 70
                    ...
                    if (null == promotionsVo.getFullReductionTargetNum() || promotionsVo.getFullReductionTargetNum() <= 0) {
                        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.tnofrccbe", null));
                    }
                }
                if (promotionsVo.getFullReductionSelect().intValue() == 50
                    ...
                    if (null == promotionsVo.getFullReductionTargetMoney()) {
                        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.tfrcacbe", null));
                    }
                    if (null == promotionsVo.getFullReductionDiscountMoney() || promotionsVo.getFullReductionDiscountMoney().doubleValue() <= 0) {
                        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.tfracbe", null));
                    }
                }
                if (promotionsVo.getFullReductionSelect().intValue() == 60) {
                    if (null == promotionsVo.getFdiscountLimit()) {
                        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.tfrlcbe", null));
                    }
                }
            } else if (promotionsVo.getIdiscountWay().intValue() == 30) {//返现
                if (null == promotionsVo.getCashBackTargetMoney()) {
                    return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.cmcacbe", null));
                }
                if (null == promotionsVo.getFcashBackMoney() || promotionsVo.getFcashBackMoney().doubleValue() <= 0) {
                    return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.cbacbe", null));
                }
            }
            ...
            if (null == promotionsVo.getIrangeDevice() || null == promotionsVo.getIrangeCommodity() ||
                ...
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.apeoga", null));
            }
            ...
            List<ActivityConf> tempAc = activityConfService.isActivityExistByMap(paramMap);
            if (null != tempAc && tempAc.size() > 0) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.tpaeatst", null));
            }
            ActivityConf activityConf = activityConfService.saveOrUpdatePromotionsVo(promotionsVo);
            if (null == activityConf) {
                return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.diasf", null));
            }
            // 执行新增
            if (StringUtils.isBlank(promotionsVo.getId())) {
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.ac.aanpn", null), activityConf.getSconfCode());
                LogUtil.addOPLog(LogUtil.AC_ADD, logText, null);
            } else {// 执行修改
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.ac.epn", null), activityConf.getSconfCode());
                LogUtil.addOPLog(LogUtil.AC_EDIT, logText, null);
            }
            return ResponseVo.getSuccessResponse(activityConf);
        } catch (Exception ex) {
    ...
    ResponseVo<String> delete(String[] checkboxId) {
        try {
            ...
            //操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.ac.dedic", null), checkboxId);
            ...
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.sedf", null));
        }
    }
    ...
    ResponseVo<String> enabled(String sid) {
        try {
            ...
            //操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.ac.ean", null), activityConf.getSconfCode());
            LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
            return ResponseVo.getSuccessResponse();
        } catch (Exception e) {
            logger.error("启用活动异常：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.sefto", null));
        }
    }
    ...
    ResponseVo<String> closed(String sid) {
        try {
            ...
            //操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.ac.cen", null), activityConf.getSconfCode());
            LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
            return ResponseVo.getSuccessResponse();
        } catch (Exception e) {
    ...
    private ResponseVo<String> verifyActivityData(ActivityConf activityConf, int type) {
        if (null == activityConf) {
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.adne", null));
        }
        if (type == 2) {
            if (activityConf.getIisAvailable().intValue() != 1) {
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.aino", null));
            }
        }
        if (type == 1) {
            if (activityConf.getTactiveEndTime().before(new Date())) {
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.aetmbact", null));
            }
            ...
            if (null == useRanges || useRanges.size() <= 0) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.taciipcttaasceaiv", null));
            }
            //判断活动同类型活动是否存在 时间段
            Map<String, Object> paramMap = new HashMap<String, Object>();
            ...
            List<ActivityConf> tempAc = activityConfService.isActivityExistByMap(paramMap);
            if (null != tempAc && tempAc.size() > 0) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.taaeatst", null));
            }
            if (activityConf.getItype().intValue() == BizTypeDefinitionEnum.ActivityType.SCENES_ACTIVITY.getCode()) {
                //场景活动
                //查询券规则
                ...
                if (null == couponRules || couponRules.size() <= 0) {
                    ...
                    if (null == integralRules || integralRules.size() <= 0) {
                        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.taciipcttvrotireaiv", null));
                    }
                }
            } else {//促销活动
                //获取活动优惠信息
                if (activityConf.getIdiscountWay().intValue() != 40) {
                    ...
                    if (null == list || list.size() <= 0) {
                        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.acedia", null));
                    }
                } else {
                    //查询券规则
                    CouponRule couponRule = new CouponRule();
                    ...
                    List<CouponRule> couponRules = couponRuleService.selectByEntityWhere(couponRule);
                    if (null == couponRules || couponRules.size() <= 0) {
                        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.ac.taciipcttvreaiv", null));
                    }
                }
            }
        }
