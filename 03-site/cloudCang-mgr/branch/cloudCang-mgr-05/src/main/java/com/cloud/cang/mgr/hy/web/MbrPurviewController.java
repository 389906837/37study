...
public class MbrPurviewController {
    ...
    public @ResponseBody
    PageListVo<List<MbrPurview>> queryDataPurview(ParamPage paramPage, MbrPurviewVo mbrPurviewVo) {
    ...
    public @ResponseBody
    ResponseVo<List<Map<String, String>>> queryAllDataByRole(ParamPage paramPage, MbrPurviewVo mbrPurviewVo) {
    ...
    public @ResponseBody
    ResponseVo<MbrPurview> queryById(String id) {
    ...
    @RequestMapping("/save")
    public @ResponseBody
    ResponseVo<MbrPurview> save(MbrPurview mbrPurview) {
        // 判断名称,URL,权限码不能重复
        MbrPurview mbr = mbrPurviewService.selectByExist(mbrPurview);
        if (mbr != null) {
            if (mbr.getSpurCode().equals(mbrPurview.getSpurCode())) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.hy.acae", null));
            }
            if (mbr.getSpurName().equals(mbrPurview.getSpurName())) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.hy.tpnae", null));
            }
            if (mbr.getSurlPath().equals(mbrPurview.getSurlPath())) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("con.hy.apae", null));
            }
        }
        ...
        if (StringUtils.isBlank(mbrPurview.getId())) {
            ...
            // 操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.hy.ampc", null) + MessageSourceUtils.getMessageByKey("main.name", null) + "{0}," + MessageSourceUtils.getMessageByKey("main.code", null) + "{1}", mbrPurview.getSpurName(), mbrPurview.getSpurNo());
            ...
            // 操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("con.hy.mmpc", null) + MessageSourceUtils.getMessageByKey("main.name", null) + "{0}," + MessageSourceUtils.getMessageByKey("main.code", null) + "{1}", mbrPurview.getSpurName(), mbrPurview.getSpurNo());
