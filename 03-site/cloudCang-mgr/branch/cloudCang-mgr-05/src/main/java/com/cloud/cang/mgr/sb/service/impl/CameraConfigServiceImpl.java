...
public class CameraConfigServiceImpl extends GenericServiceImpl<CameraConfig, String> implements
	...
	public ResponseVo<CameraConfig> insertCameraConfig(CameraConfig cameraConfig) {
		...
		if (CollectionUtils.isNotEmpty(cameraConfigList)) {
			return  ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("sb.camera.config.save.exist.ipandport"));
		}
		...
		return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("sb.camera.device.code.not.exist"));
	}
