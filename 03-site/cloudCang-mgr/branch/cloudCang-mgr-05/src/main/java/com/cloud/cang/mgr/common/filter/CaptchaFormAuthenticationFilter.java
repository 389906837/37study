...
public class CaptchaFormAuthenticationFilter extends FormAuthenticationFilter
{
            ...
            if (LOGIN_IP_STATUS!=null){
	            if(3==LOGIN_IP_STATUS){
                    ...
                    if ( captcha == null || !captcha.equalsIgnoreCase(token.getCaptcha())) {
                        throw new IncorrectCaptchaException(MessageSourceUtils.getMessageByKey("syscon.login.yzm.error",null));
                    }
                }else{
                    throw new IncorrectCaptchaException(MessageSourceUtils.getMessageByKey("syscon.login.yzm.expired",null));
                }
            } else {
            	throw new IncorrectCaptchaException(MessageSourceUtils.getMessageByKey("syscon.login.yzm.expired",null));
            }
    ...
    protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
        ...
        try {
            //检查用户是不是被禁用
             ...
             if (null == operator) {
                 throw new IncorrectCaptchaException(MessageSourceUtils.getMessageByKey("syscon.login.username.exist",null));
             }
            if(operator.getBisFreeze() == 0 || operator.getBisDelete()==1){
                //禁用
                throw new IncorrectCaptchaException(MessageSourceUtils.getMessageByKey("syscon.login.username.disabled",null));
            }
        ...
        ServletResponse response)
    {
        ...
        if(exceptionClassName==null){
        	message= MessageSourceUtils.getMessageByKey("syscon.login.username.pwd.error",null);
        }
        ...
        if (exceptionClassName.indexOf("IncorrectCredentialsException")!=-1) {
        	...
        	message= MessageSourceUtils.getMessageByKey("syscon.login.username.pwd.locked",null);
        }  else if (exceptionClassName.indexOf("LockedAccountException")!=-1) {
        	message= MessageSourceUtils.getMessageByKey("syscon.login.username.pwd.error",null);
