...
public class CameraConfigController extends GenericController {
    ...
    public ResponseVo<CameraConfig> add(CameraConfig cameraConfig) {
        try {
            ...
            if (StringUtils.isBlank(sip)) {
                logger.error("IP为空");
                return ErrorCodeEnum.ERROR_APP_PARAM_EXCEPTION.getResponseVo(MessageSourceUtils.getMessageByKey("sb.add.camera.ip.empty"));
            }
            if (StringUtils.isBlank(sport)) {
                logger.error("端口号为空");
                return ErrorCodeEnum.ERROR_APP_PARAM_EXCEPTION.getResponseVo(MessageSourceUtils.getMessageByKey("sb.add.camera.port.empty"));
            }
            if (StringUtils.isBlank(sserialNumber)) {
                logger.error("序列号为空");
                return ErrorCodeEnum.ERROR_APP_PARAM_EXCEPTION.getResponseVo(MessageSourceUtils.getMessageByKey("sb.add.camera.serialnumber.empty"));
            }
            if (StringUtils.isBlank(sdeviceId)) {
                logger.error("设备信息为空");
                return ErrorCodeEnum.ERROR_APP_PARAM_EXCEPTION.getResponseVo(MessageSourceUtils.getMessageByKey("sb.add.camera.deviceid.empty"));
            }
            ...
            //操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("log.add.webcam"), config);
        ...
        return ErrorCodeEnum.ERROR_APP_PARAM_EXCEPTION.getResponseVo(MessageSourceUtils.getMessageByKey("sb.add.camera.fail"));
    }
    ...
    public ResponseVo<CameraConfig> edit(CameraConfig cameraConfig) {
        try {
            ...
            String config = "IP：" + cameraConfig.getSip() + "端口号" + cameraConfig.getSport() + "序列号：" + cameraConfig.getSserialNumber();
            //操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("log.edit.webcam"), config);
        ...
        return ErrorCodeEnum.ERROR_APP_PARAM_EXCEPTION.getResponseVo(MessageSourceUtils.getMessageByKey("sb.edit.camera.fail"));
    }
    ...
    public ResponseVo<String> delete(String[] checkboxId) {
        try {
            if (ArrayUtils.isNotEmpty(checkboxId)) {
                ...
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("log.delete.camera.config"), "");
                LogUtil.addOPLog(LogUtil.AC_DELETE, logText, null);
                return ResponseVo.getSuccessResponse();
            }
        } catch (ServiceException e) {
            logger.error("删除摄像头信息失败：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("sb.delete.camera.fail"));
        } catch (Exception e) {
            logger.error("删除摄像头信息失败：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("sb.delete.camera.fail"));
        }
