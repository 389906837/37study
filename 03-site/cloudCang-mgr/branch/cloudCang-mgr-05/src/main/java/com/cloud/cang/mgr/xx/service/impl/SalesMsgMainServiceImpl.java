...
public class SalesMsgMainServiceImpl extends GenericServiceImpl<SalesMsgMain, String> implements
        ...
        for (String id : checkboxId) {
            ...
            if (salesMsgMain != null) {
                ...
                if (list != null && list.size() > 0) {
                    throw new ServiceException(MessageSourceUtils.getMessageByKey("xxcon.delete.sms.provider.error",null)+"【" + salesMsgMain.getScode() + "】，"+MessageSourceUtils.getMessageByKey("main.delete.error",null)+"！");
                }
                salesMsgMainDao.deleteByPrimaryKey(salesMsgMain.getId());
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("xxcon.delete.sms.provider",null)+" "+MessageSourceUtils.getMessageByKey("main.name",null)+"{0},"+MessageSourceUtils.getMessageByKey("main.code",null)+"{1}", salesMsgMain.getScode());
    ...
    public ResponseVo saveSalesMsgMain(String ssupplierCode, String msgContent, String testMobile, String sendMsgIds) {
        ...
        salesMsgMain.setRemark(MessageSourceUtils.getMessageByKey("xxcon.marketing.sms",null));
        ...
        // 操作日志
        String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("xxcon.add.marketing.provider.name",null)+"{0},"+ MessageSourceUtils.getMessageByKey("main.code",null)+"{1}", salesMsgMain.getScode());
        ...
        ExecutorManager.getInstance().execute(new Runnable() {
            ...
            public void run() {
                ...
                /* 是否实时发送 1:实时 2：非实时 */
                    ...
                    throw new ServiceException(MessageSourceUtils.getMessageByKey("xxcon.calling.send.service.failed",null));
                }
            }
        });
    }
