...
public class RemindMessageController {
    ...
    ResponseVo<RemindMessage> save(RemindMessage remindMessage) {
        try {
            ...
            if (StringUtils.isBlank(remindMessage.getId())) {
                ...
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.add.remind.log",null)+"，"+MessageSourceUtils.getMessageByKey("main.code",null)+"{0}", remindMessage.getScode());
                ...
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.modify.remind.log",null)+"，"+ MessageSourceUtils.getMessageByKey("main.code",null)+"{0}", remindMessage.getScode());
                LogUtil.addOPLog(LogUtil.AC_EDIT, logText, null);
            }
            return ResponseVo.getSuccessResponse(remindMessage);
        } catch (Exception ex) {
            log.error("保存或修改消息提醒配置列表数据异常：{}", ex);
            return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo(MessageSourceUtils.getMessageByKey("syscon.save.remind.error",null));
        }
    }
    ...
    ResponseVo<String> delete(String[] checkboxId) {
        try {
            ...
            //操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.delete.remind.log",null), checkboxId);
