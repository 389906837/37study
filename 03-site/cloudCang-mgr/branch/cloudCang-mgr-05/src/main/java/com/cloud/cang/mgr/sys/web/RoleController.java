...
import com.cloud.cang.mgr.sys.service.*;
import com.cloud.cang.mgr.sys.vo.CheckBoxTreeVo;
...
public class RoleController extends GenericController {
    ...
    ResponseVo<Role> save(Role role) {
        try {
            if (StringUtil.isBlank(role.getSroleName())) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(MessageSourceUtils.getMessageByKey("syscon.add.role.name.empty",null));
            }
            // 执行新增
            if (StringUtils.isBlank(role.getId())) {
               ...
               /* role.setSmerchantName(authorizationVO.getSmerchantName());*/
                ...
                role.setSsystemSource("10");
                //操作日志
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.add.role.log",null), role.getSroleName());
            ...
            } else// 执行修改
            {
                ...
                String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.modify.role.log",null), role.getId());
                LogUtil.addOPLog(LogUtil.AC_EDIT, logText, null);
            }
        } catch (Exception ex) {
            log.error("修改或增加角色异常：{}", ex);
            return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo(MessageSourceUtils.getMessageByKey("syscon.save.role.error",null));
        }
    ...
    ResponseVo<String> delete(String[] checkboxId) {
        try {
            ...
            //操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.delete.role.log",null), checkboxId);
    ...
    ResponseVo<String> saveRolePurview(String checkPurviewId, String roleId, String id) {
        try {
            if (StringUtils.isBlank(roleId)) {
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("syscon.config.purview.role.error",null));
            }
    ...
    ResponseVo<String> updateRoleAuth(String roleId, String[] purviewIds) {
        try {
            ...
            if (role == null) {
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("syscon.save.role.exist",null));
            }
            if (!SessionUserUtils.getSessionAttributeForUserDtl().getSmerchantId().equals(role.getSmerchantId())) {
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("main.system.abnormal",null)+"，系统角色不属于自己商户！");
            }
            ...
            // 操作日志
            String logText = MessageFormat.format(MessageSourceUtils.getMessageByKey("syscon.config.purview.role.log",null), purviewIds);
