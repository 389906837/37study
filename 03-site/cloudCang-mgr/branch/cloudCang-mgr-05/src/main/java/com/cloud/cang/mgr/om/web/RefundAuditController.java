...
public class RefundAuditController {
    ...
    ResponseVo saveAudit(RefundAuditDomain refundAuditDomain, HttpServletRequest request) {
        try {
            ...
            if (null == refundAuditDomain.getIauditStatus()) {
                ...
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("om.save.audit.status.not.empty"));
            }
            if (BizTypeDefinitionEnum.DomainStatus.AUDIT_FAILURE.getCode() == refundAuditDomain.getIauditStatus() && StringUtils.isBlank(refundAuditDomain.getSauditRefuseReason())) {
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("shcon.sudit.domain.conf.reason.empty",null));
            }
           ...
           /* String logText = MessageFormat.format("审核退款订单,订单Id{}", refundAuditDomain.getId());
            ...
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("om.save.audit.fail"));
        }
    }
    ...
    ResponseVo refund(String checkboxId, HttpServletRequest request) {
        ...
        if (null == refundAudit) {
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("om.order.refund.no.exist"));
        }
        try {
            ...
            logger.error("退款异常！", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("om.order.refund.error"));
        }
   /*     OrderRecord orderRecord = orderRecordService.selectByPrimaryKey(refundAudit.getSorderId());
    ...
    public ResponseVo dismissal(String checkboxId, HttpServletRequest request) {
        try {
            ...
            refundOperlog.setSip(SessionUserUtils.getIpAddr(request));
            refundOperlogService.insert(refundOperlog);
            return ResponseVo.getSuccessResponse(MessageSourceUtils.getMessageByKey("common.success"));
        } catch (Exception e) {
            logger.error("退款申请驳回失败：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(MessageSourceUtils.getMessageByKey("common.fail"));
        }
    }
    ...
    public void downloadExcel(HttpServletRequest request, HttpServletResponse response) {
        ...
        String[] names = new String[]{
                ...
                MessageSourceUtils.getMessageByKey("sq.refund.number"),
                MessageSourceUtils.getMessageByKey("sq.trade.serial.number"),
                MessageSourceUtils.getMessageByKey("tb.order.number"),
                ...
                MessageSourceUtils.getMessageByKey("sb.commodity.unit.price.yuan"),
                MessageSourceUtils.getMessageByKey("om.refund.amount"),
                MessageSourceUtils.getMessageByKey("report.total.merchandise.yuan"),
                MessageSourceUtils.getMessageByKey("om.refund.amount.yuan"),
                ...
                MessageSourceUtils.getMessageByKey("om.request.for.refund.yuan"),
                MessageSourceUtils.getMessageByKey("om.refund.status"),
                MessageSourceUtils.getMessageByKey("om.refund.type"),
                MessageSourceUtils.getMessageByKey("om.application.time"),
                MessageSourceUtils.getMessageByKey("sb.audit.status"),
                MessageSourceUtils.getMessageByKey("main.reviewer"),
                MessageSourceUtils.getMessageByKey("sb.audit.time")};
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if ("10".equals(irefundStatus)) {
                e.setCell(20, MessageSourceUtils.getMessageByKey("om.pending.refund"));
            } else if ("20".equals(irefundStatus)) {
                e.setCell(20, MessageSourceUtils.getMessageByKey("sq.refund.failure"));
            } else if ("30".equals(irefundStatus)) {
                e.setCell(20, MessageSourceUtils.getMessageByKey("sq.refund.success"));
            } else {
                e.setCell(20, "");
            }
            if ("30".equals(ipayType)) {
                e.setCell(21,MessageSourceUtils.getMessageByKey("main.wechat"));
            } else if ("40".equals(ipayType)) {
                e.setCell(21, MessageSourceUtils.getMessageByKey("main.alipay"));
            } else {
                e.setCell(21, "");
            }
            ...
            if ("10".equals(iauditStatus)) {
                e.setCell(23, MessageSourceUtils.getMessageByKey("main.audited"));
            } else if ("20".equals(iauditStatus)) {
                e.setCell(23, MessageSourceUtils.getMessageByKey("main.approval"));
            } else if ("30".equals(iauditStatus)) {
                e.setCell(23, MessageSourceUtils.getMessageByKey("sh.review.the.rejected"));
            } else {
                e.setCell(23, "");
            }
        ...
        e.downloadExcel(request, response, MessageSourceUtils.getMessageByKey("excel.refund.audit.list")+"-" + DateUtils.getCurrentDTimeNumber() + ".xls");
    }
