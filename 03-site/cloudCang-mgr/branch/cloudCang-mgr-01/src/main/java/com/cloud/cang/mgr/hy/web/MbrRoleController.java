...
public class MbrRoleController {
    ...
    ResponseVo<MbrRole> save(MbrRole mbrRole) {
        ...
        if (m != null) {
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("角色名称已存在！");
        }
        // 如果id为空就就添加
        if (StringUtils.isBlank(mbrRole.getId())) {
            ...
            //操作日志
            String logText = MessageFormat.format("新增会员角色,名称{0}", mbrRole.getSroleName());
            ...
            //操作日志
            String logText = MessageFormat.format("修改会员角色,名称{0}", mbrRole.getSroleName());
    ...
    ResponseVo<String> saveMbrRole(String[] roleIds, String mid) {
        try {
            ...
            // 操作日志
            String logText = MessageFormat.format("用户角色分配，角色ID集合{0}", roleIds);
            LogUtil.addOPLog(LogUtil.AC_ADD, logText, null);
            return ResponseVo.getSuccessResponse();
        } catch (Exception e) {
