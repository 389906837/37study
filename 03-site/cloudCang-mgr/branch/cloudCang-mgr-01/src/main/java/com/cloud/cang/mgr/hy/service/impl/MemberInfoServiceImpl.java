...
//        }
     ...
     */
    @Override
    public ResponseVo rechargeAgainUpData(SignResult signResult, HttpServletRequest request) throws Exception {
        logger.debug("支付宝免密支付补处理,获得的参数=" + signResult);
        //获取商户配置信息
       /* MerchantConf conf = rechargeAgainService.getDefaultAlipayMerchantConf();*/
        if (this.dealwithAlipaySign(/*conf,*/ signResult, request)) {
     ...
     */
    @Override
    public ResponseVo WechantRechargeAgainUpData(WechatSignResult wechatSignResult, HttpServletRequest request) throws Exception {
        logger.info("微信免密支付补处理,参数：{}", wechatSignResult);
       /* MerchantConf conf = rechargeAgainService.getDefaultWechatMerchantConf();*/
        //获取用户的支付宝免密数据
        if (this.wechantDealwithAlipaySign(wechatSignResult, request)) {
            return ResponseVo.getSuccessResponse();
        }
        return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("用户支付宝免密支付补处理失败！");
    }
     ...
     * @return boolean
     */
    private Boolean wechantDealwithAlipaySign(WechatSignResult wechatSignResult, HttpServletRequest request) throws Exception {
     /*   if (!conf.getSplanId().equals(wechatSignResult.getPlan_id())) {
        ...
        WechatFreeData freeData = wechatFreeDataService.selectByContractCode(contractCode, SessionUserUtils.getSessionAttributeForUserDtl().getSmerchantCode());
        if (freeData == null) {
     ...
     * @return boolean
     */
    private Boolean dealwithAlipaySign(/*MerchantConf conf,*/ SignResult signResult, HttpServletRequest request) throws Exception {
        ...
        FreeData freeData = freeDataService.selectByExternalAgreementNo(externalAgreementNo, SessionUserUtils.getSessionAttributeForUserDtl().getSmerchantCode());
        if (freeData == null) {
