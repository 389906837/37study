...
public class OrderAuditController {
    ...
    PageListVo<List<OrderAuditVo>> queryAuditData(OrderAuditDomain orderAuditDomain, ParamPage paramPage) {
     ...
     /*   List<OrderAuditVo> list = page.getResult();
        //用户没有该权限,用户名脱敏处理
        if (!hasPurCode("ORDER_AUDIT_MEMBER_NAME_DESENSITIZE")) {
            for (OrderAuditVo orderAuditVo : list) {
                orderAuditVo.setSmemberName(DesensitizeUtil.mobilePhone(orderAuditVo.getSmemberName()));
            }
        }*/
        if (page != null) {
    ...
    ResponseVo<String> ignoreOrder(String checkboxId) {
        try {
            ...
            //操作日志
            String logText = MessageFormat.format("忽略异常订单,订单Id", checkboxId);
            LogUtil.addOPLog(LogUtil.AC_EDIT, logText, null);
            return ResponseVo.getSuccessResponse();
        } catch (Exception e) {
            logger.error("忽略异常订单异常：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("忽略异常订单失败！");
        }
    }
    ...
    ResponseVo<String> payOrder(String checkboxId) {
        ...
        try {
            ...
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("异常订单扣款异常！");
        }
    }
    ...
    public void downloadExcel(HttpServletRequest request, HttpServletResponse response, OrderAuditDomain orderAuditDomain) {
        ...
        e.createRow(0);
        String[] names = new String[]{"序号", "商户编号", "商户名称", "订单编号", "设备编号", "设备名称", "设备地址", "商品名", "商品单价", "商品数量",
                "商品总额(元)", "审核类型", "会员用户名", "订单总额(元)", "状态", "异常时间", "处理人", "处理时间", "处理备注"};
        ...
        for (int i = 0; i < downloadOrderAuditVoList.size(); i++) {
            ...
            if (10 == itype) {
                e.setCell(11, String.valueOf("盘点异常"));
            } else if (20 == itype) {
                e.setCell(11, String.valueOf("已售补扣"));
            } else if (30 == itype) {
                e.setCell(11, String.valueOf("创建订单异常"));
            }
            ...
            if (10 == istatus) {
                e.setCell(14, String.valueOf("待处理"));
            } else if (20 == istatus) {
                e.setCell(14, String.valueOf("异常处理成功"));
            } else if (30 == istatus) {
                e.setCell(14, String.valueOf("异常处理忽略"));
            }
        ...
        e.downloadExcel(request, response, "异常订单列表-" + DateUtils.getCurrentDTimeNumber() + ".xls");
    }
    ...
    public ResponseVo save(OrderAudit orderAudit, HttpServletRequest request, String commodityIds) {
        try {
            ...
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("编辑审核订单异常！");
        }
    }
