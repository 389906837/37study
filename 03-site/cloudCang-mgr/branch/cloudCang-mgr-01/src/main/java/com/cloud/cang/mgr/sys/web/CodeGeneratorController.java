...
public class CodeGeneratorController {
    ...
    public @ResponseBody ResponseVo delete(String[] checkboxId) {
		...
		// 操作日志
		String logText = MessageFormat.format("删除编号生成配置，删除ID集合{0}", checkboxId);
    ...
    public @ResponseBody ResponseVo insert(CodeGenerator codeGeneratorConfig) {
    	...
    	if(list!=null&&list.size()>0){
    		...
    		if(DEL_FALSE==generatorConfig.getIdelFlag()){
    			return ErrorCodeEnum.ERROR_COMMON_UNIQUE.getResponseVo("编号已经存在!");
    		}else{
		...
		//操作日志
		String logText = MessageFormat.format("新增编号生成配置，编号{0}", codeGeneratorConfig.getScodeType());
    ...
    public @ResponseBody ResponseVo update(CodeGenerator codeGeneratorConfig) {
		...
		//操作日志
		String logText = MessageFormat.format("修改编号生成配置，编号{0}", codeGeneratorConfig.getScodeType());
