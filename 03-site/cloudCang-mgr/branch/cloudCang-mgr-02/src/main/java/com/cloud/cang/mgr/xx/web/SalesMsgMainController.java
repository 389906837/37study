...
public class SalesMsgMainController {
    ...
    ResponseVo<SalesMsgMain> save(String ssupplierCode, String msgContent, String testMobile, String sendMsgIds) {
        if (StringUtils.isBlank(ssupplierCode)) {
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("请选择供应商！");
        }
        if (StringUtils.isBlank(msgContent)) {
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("请填写发送内容！");
        }
        if (StringUtils.isBlank(testMobile)) {
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("请填写测试发送短信手机号！");
        }
        if (StringUtils.isBlank(sendMsgIds)) {
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("请填写发送短信会员！");
        }
