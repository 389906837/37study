...
public class SupplierInfoController {
    ...
    ResponseVo<SupplierInfo> save(SupplierInfo supplierInfo) {
        ...
        if (StringUtils.isBlank(supplierInfo.getId())) {
            ...
            // 操作日志
            String logText = MessageFormat.format("新增短信供应商名称{0},编号{1}", supplierInfo.getSname(), supplierInfo.getScode());
...
//            supplierInfo.setSmerchantCode(oldSupplierInfo.getSmerchantCode());
            ...
            // 操作日志
            String logText = MessageFormat.format("修改短信供应商 名称{0},编号{1}", supplierInfo.getSname(), supplierInfo.getScode());
    ...
    ResponseVo delete(String[] checkboxId) {
        ...
        return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("短信供应商状态为启用时不能删除！");
    }
        ...
        try {
            ...
            for (MsgTemplate msgTemplate : msgTemplateList) {
                if (msgTemplate.getIisEnable().equals(1)) {
                    return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("消息模板正在使用请勿禁用!");
                }
            }
            ...
            // 操作日志
            String logText = MessageFormat.format("短信供应商启用", supplierInfo.getScode());
            LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
            return ResponseVo.getSuccessResponse();
        } catch (Exception e) {
            log.error("短信供应商启用/禁用异常：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("短信供应商启用失败");
        }
    }
}
