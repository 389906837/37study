...
//		}
		...
		if (StringUtils.isBlank(announcement.getId())) {
			...
			//操作日志
			String logText = MessageFormat.format("新增系统公告区域，标题{0}", announcement.getStitle());
			...
			//操作日志
			String logText = MessageFormat.format("修改系统公告区域，标题{0}", announcement.getStitle());
		...
		for (String id : checkboxId) {
			...
			if (announcement != null) {
				if (announcement.getIstatus().equals(20) && DateUtils.getCurrentDateTime().compareTo(announcement.getTvalidDate())<0){
					throw new ServiceException("系统公告为已发布状态且还未到期不能删除【"+announcement.getStitle()+"】，删除失败！");
				}
			...
			// 操作日志
			String logText = MessageFormat.format("删除系统公告区域,标题{0}", announcement.getStitle());
