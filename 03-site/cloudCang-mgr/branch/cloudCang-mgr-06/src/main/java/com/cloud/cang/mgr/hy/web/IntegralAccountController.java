...
public class IntegralAccountController {
    ...
    ResponseVo<IntegralAccount> integralAdjustment(IntegralAccount integralAccount, IntegralAccountVo integralAccountVo) {
        ...
        try {
            ...
            if (flag) {// 执行新增
                //操作日志
                String logText = MessageFormat.format("新增总积分和可用积分，编号{0}", integralAccount.getItotalPoints(), integralAccount.getIusablePoints());
                LogUtil.addOPLog(LogUtil.AC_ADD, logText, null);
            } else {// 执行修改
                //操作日志
                String logText = MessageFormat.format("减少总积分和可用积分，编号{0}", integralAccount.getIusablePoints(), integralAccount.getIusedPoints());
                LogUtil.addOPLog(LogUtil.AC_EDIT, logText, null);
            }
            return ResponseVo.getSuccessResponse(integralAccount);
        } catch (Exception ex) {
            logger.error("积分调整异常:{}", ex);
            return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo("增加积分值失败");
        }
    }
