...
<script type="text/javascript">
    var colModel = [{name: 'id', index: 'id',hidden:true},{name: 'idiscountWay',hidden:true},
        ...
        {name: 'sconfName',index: 'sconf_name',label : "活动名称",width:180},
        <c:if test="${SESSION_KEY_ROOT_MERCHANT eq 1}">
        ...
        {name: 'discountWay',index: 'idiscount_way',label : "活动类型"},
        ...
        {name: 'irangeType',index: 'irange_type',label : "参与活动商品",sortable : false,formatter:function(value,rowObject){
            return "<a href='javascript:void(0);' onclick=\"viewCommodity('"+rowObject.rowId+"')\">查看商品</a>";}},
        ...
        {name: 'sdescription',index: 'sdescription',label : "备注",sortable : false},
    ...
    function viewMethod(sid) {
        showLayerMin("查看促销活动详情", ctx+'/activityConf/promotionsView?sid='+sid);
    }
    //查看活动设备
    function viewDevice(sid) {
        showLayerMedium("查看促销活动设备信息", ctx+'/activityConf/viewDevice?sid='+sid);
    }
    //查看活动商品
    function viewCommodity(sid) {
        showLayerMedium("查看促销活动商品信息", ctx+'/activityConf/viewCommodity?sid='+sid);
    }
    function editMethod(sid) {
        showLayerMedium("编辑促销活动信息", ctx+'/activityConf/editPromotions?sid='+sid);
    }
    //删除数据
