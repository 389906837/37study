            ...
            public void run() {
                ...
                for (CreatOrderResult creatOrderResult : creatOrderResultList) {
                    ...
                    try {
                        if (20 == type && 10 == orderRecord.getIchargebackWay()) {
                         ...
                         /*   freePaymentDto.setIpayWay(orderRecord.getIpayWay());*/
                        ...
                        } else if (type == 10 && 40 == orderRecord.getIchargebackWay()) {
                            //创建支付
                            ResponseVo<CreatSmartretailOrderDto> responseVo = new ResponseVo<CreatSmartretailOrderDto>();
                            ...
                            OrderCommodityDomain temp = new OrderCommodityDomain();
                            temp.setId(orderRecord.getId());
                            List<OrderCommodityVo> orderCommodities = orderCommodityService.selectOrderDetail(temp);
                            ...
                            RestServiceInvoker invoke = RestServiceInvokeBuilder.newBuilder().newInvoker(url);// 服务名称
                            // 返回对象中含有泛型，则需要设置返回类型，否则无需设置
                            invoke.setParameterizedTypeReference(new ParameterizedTypeReference<ResponseVo<FreePaymentResult>>() {
                            });
                            invoke.setRequestObj(creatSmartretailOrderDto); // post 参数
                            responseVo = (ResponseVo<CreatSmartretailOrderDto>) invoke.invoke();
                            if (null != responseVo && responseVo.isSuccess()) {
                                logger.info("微信支付分代扣成功:{}", responseVo.getData());
                            } else {
                                logger.error("微信支付分代扣成功异常:{}", responseVo);
                            }
                        } else {
                            //未开通免密发送短信通知已生成订单
                            if ("1".equals(isSendMessage)) {
