...
public class RequestIntercepter implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object arg2) throws Exception {
        ...
        if (currentUrl.indexOf("/rec/unauthorized") != -1 || currentUrl.indexOf("/init/cached") != -1) {
            return true;
        }
        //获取域名
        ...
        if (domainConf == null || domainConf.getIstatus().intValue() != BizTypeDefinitionEnum.DomainStatus.AUDIT_SUCCESS.getCode()) {
            //商户域名配置错误
            response.sendRedirect(request.getContextPath() + "/rec/unauthorized");
            return true;
        }
        ...
        if (merchantInfo.getSparentId().equals("0")) {//根商户
