...
import com.cloud.cang.rec.sys.vo.TreeVo;
...
public class OperatorController {
    ...
    public PageListVo<List<OperatorDomain>> queryData(OperatorVo operatorVo, ParamPage paramPage, String selectBd) {
        ...
        if (StringUtil.isNotBlank(selectBd) && "true".equals(selectBd)) {
            ...
            //生成查询条件
            MerchantInfo merchantInfo = merchantInfoService.selectByPrimaryKey(SessionUserUtils.getSessionAttributeForUserDtl().getSmerchantId());
            if (!"0".equals(merchantInfo.getSparentId())) {
                operatorVo.setQueryCondition(SessionUserUtils.getSessionAttributeForUserDtl().getSmerchantCode());
            }
        }*/
        operatorVo.setSmerchantId(SessionUserUtils.getSessionAttributeForUserDtl().getSmerchantId());
