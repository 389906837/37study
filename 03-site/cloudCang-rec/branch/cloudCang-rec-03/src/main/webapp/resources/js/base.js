...
function alertSuccessold(msg, inparams, success) {
    var params = {
        icon: 1,
        time: 3000, //3s后自动关闭,
        end: function (index) {
...
//成功提示框 30s后自动关闭
function alertSuccessnew(msg, inparams, success) {
    var params = {
        icon: 1,
        time: 3000, //3s后自动关闭,
        end: function (index) {
...
//弹出框父窗口成功并刷新
function alertParentMsgAndReload(msg, inparams) {
    if (isEmpty(msg)) {
        msg = "操作成功";
    }
    var params = {
...
//操作父窗口确认提示框
function confirmOperParentTip(msg, url, param) {
    layer.confirm(msg, {
        btn: ['取消', '确定'], //按钮
        cancel: function (index) {
            closeLayer(index);
        ...
        operParentDealwith(url, param);
    });
}

//处理父窗口操作事件
function operParentDealwith(url, param) {
    $.ajax({
        type: "post",
        url: url,
        data: param,
        dataType: "json",
        success: function (data) {
            dealwithParentSuccess(data);
        }
    });
}
//操作完父窗口回调处理
dealwithParentSuccess = function (data) {
    if (data.success) {
        alertParentMsgAndReload(data.msg);
    } else {
        alertDel(data.msg);
    }
};
