...
public class InterfaceAccountController {
    ...
    @ResponseBody
    public ResponseVo close(String checkboxId, Integer type) {
        try {
            ...
            interfaceAccount.setIstatus(type);
            interfaceAccountService.updateBySelective(interfaceAccount);
            //操作日志
            String logText = MessageFormat.format("关闭/开通 接口账户，接口账户编号{0}", temp.getScode());
            LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
            return ResponseVo.getSuccessResponse();
        } catch (Exception e) {
            log.error("关闭/开通 接口账户异常：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("关闭/开通接口账户失败！");
        }
    }
     ...
     * @return
     */
    @RequestMapping("/restrictionNum")
    public @ResponseBody
    ResponseVo<String> restrictionNum(String checkboxId, Integer type) {
        try {
            InterfaceAccount temp = new InterfaceAccount();
            if (type == 1) {
           ...
           /*     String itransferNum = BizParaUtil.get("interface_account_default_itransfer_num");
                temp.setFbalance(Integer.valueOf(itransferNum));*/
                temp.setIisUnlimitedNumber(type);
            } else {
                temp.setId(checkboxId);
                temp.setUpdateTime(DateUtils.getCurrentDateTime());
               /* temp.setFbalance(-1);*/
            ...
            interfaceAccountService.updateBySelective(temp);
            // 操作日志
            String logText = MessageFormat.format("开启/关闭 次数限制，状态{0}", type);
            LogUtil.addOPLog(LogUtil.AC_OTHER, logText, null);
            return ResponseVo.getSuccessResponse();
        } catch (Exception e) {
            log.error("开启/关闭 次数限制异常：{}", e);
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("修改次数限制状态异常！");
        }
    }
}
