...
public class GroupCommonwealController {
	
	@Autowired
    ArticleService articleService;
	
	private static final String GROUPCOMMONWEAL_NAV_ID="583f92d74d7c8d5d37191e1d";
	
	 @RequestMapping("/groupCommonweal")
     public String groupCommonweal(ModelMap map,ParamPage paramPage) {
		
		 Query topQuquery = new Query(Criteria.where("snavigationId").is(GROUPCOMMONWEAL_NAV_ID));
		 ...
		 Query articleQuery = new Query(Criteria.where("snavigationId").is(GROUPCOMMONWEAL_NAV_ID));
		 if (null != topArt){
		 ...
		 map.put("page", page);
		 return "front/cn/duty/groupCommonweal";
	 }
