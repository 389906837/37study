...
public class PartyBuildingController {
	...
	private static final String PARTI_BUILD_NAV_ID="583cfc824d7c0bb5568964b6";
	
	 @RequestMapping("/partyBuilding")
     public String partyBuilding(ModelMap map,ParamPage paramPage) {
		 Query topQuquery = new Query(Criteria.where("snavigationId").is(PARTI_BUILD_NAV_ID));
		 ...
		 Query articleQuery = new Query(Criteria.where("snavigationId").is(PARTI_BUILD_NAV_ID));
		 if (null != topArt){
		 ...
		 map.put("page", page);
		 return "front/cn/duty/partyBuilding";
	 }
