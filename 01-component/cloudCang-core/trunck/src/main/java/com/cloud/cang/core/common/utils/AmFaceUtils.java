...
public class AmFaceUtils {
     ...
     * 雨森人脸图片分组
     */
    public static String YS_GROUP_ID = GrpParaUtil.getValue(GrpParaUtil.AI_FACE_CONFIG, "YuSenFaceGroup");    //数据字典配置
     ...
     * 只有一个用户时对比人脸库
     *
     * @param imgUrl  图片的 URL地址
     * @param userId  用户ID
     * @param groupId 分组ID
     * @return
     */
    public static ResponseAip findUserByUrl(String imgUrl, String userId, String groupId) {
        ...
        // 人脸查询
        JSONObject res = amClient.search(imgUrl, imageType, groupId, options);
        ResponseAip responseAip = com.alibaba.fastjson.JSONObject.parseObject(res.toString(), ResponseAip.class);
        return responseAip;
    }
     ...
     * @return
     */
    public static ResponseAip registerByUrl(String userId, String imgUrl, String groupId, String userInfo) {
        String imageType = "URL";
        HashMap<String, String> options = new HashMap<String, String>();
        options.put(BaiduAiFaceConfig.QUALITY_CONTROL, "NORMAL");  // 图片质量控制（选填）
        options.put(BaiduAiFaceConfig.LIVENESS_CONTROL, "LOW");    // 活体检测控制（选填）
        if (StringUtil.isNotBlank(userInfo)) {
            options.put(BaiduAiFaceConfig.USER_INFO, userInfo);                        // 用户资料，长度限制256B
        }
        // 调用百度人脸注册
        JSONObject res = amClient.addUser(imgUrl, imageType, groupId, userId, options);
        ResponseAip responseAip = com.alibaba.fastjson.JSONObject.parseObject(res.toString(), ResponseAip.class);
        return responseAip;
    }
