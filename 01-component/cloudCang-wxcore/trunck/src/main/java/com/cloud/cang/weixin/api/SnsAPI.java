...
public class SnsAPI extends BaseAPI{
	...
	public static String connectOauth2Alipay(String appid,String redirect_uri,boolean auth_user,String state){
		try {
			StringBuilder sb = new StringBuilder();
			sb.append(ALIPAY_OPEN_URI + "/oauth2/publicAppAuthorize.htm?")
					.append("app_id=").append(appid)
					.append("&redirect_uri=").append(URLEncoder.encode(redirect_uri, "utf-8"))
					.append("&scope=").append(auth_user?"auth_user":"auth_base")
					.append("&state=").append(state==null?"":state);
			return sb.toString();
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
