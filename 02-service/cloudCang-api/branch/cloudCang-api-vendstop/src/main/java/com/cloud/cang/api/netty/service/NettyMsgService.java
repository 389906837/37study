...
import com.cloud.cang.vendstop.VendstopDefine;
...
public class NettyMsgService {
    ...
    public void openDoorSuccess(BaseResponseVo baseResponseVo) {
        ...
        if (null == openSource || (null != openSource && 20 == openSource)) {
            ...
            //发送消息给手机客户端
            asynSendMsgToSocketIo(NettyConst.EVENT_OPEN_DOOR, socketResponseVo, socketIOClient);

            //如果有小屏设备，发送消息给小屏
            sendMsgToAiFace(deviceId, userId, "大屏扫码开门成功");

            //开门成功，调用vendstop，通知其设备开门成功
            final SessionVo temp = sessionVo;
            ExecutorManager.getInstance().execute(new Runnable() {
                @Override
                public void run() {
                    logger.info("开门成功通知vendstop, sessionId="+temp.getApp_sessionId());
                    try {
                        RestServiceInvoker invoke = RestServiceInvokeBuilder.newBuilder().newInvoker(VendstopDefine.OPEN_SUCCESS);
                        invoke.setParameterizedTypeReference(new ParameterizedTypeReference<ResponseVo<String>>() {});
                        invoke.setRequestObj(temp.getApp_sessionId());
                        ResponseVo<String> resVO = (ResponseVo<String>) invoke.invoke();
                        logger.debug("开门成功通知vendstop , 返回结果:{}",JSON.toJSONString(resVO));
                    }catch (Exception e){
                        logger.debug("开门成功通知vendstop失败:{}",e);
                    }
                }
            });
    ...
    public void closeDoorInventorySuccess(BaseResponseVo baseResponseVo) {
        ...
        if (null == openSource || (null != openSource && 20 == openSource)) {
            ...
            try {
                ...
                asynSendMsgToSocketIo(NettyConst.EVENT_CLOSE_DOOR, socketResponseVo, socketIOClient);
                ...
                if (responseVo.isSuccess()) {
                    ...
                    if (inventoryResult.getItype().intValue() == 10 || inventoryResult.getItype().intValue() == 20) { //购物关门
                        ...
                        if (responseVo.getData().getItype().intValue() == 10) {//创建代扣支付订单成功
                            ResponseVo responseVo1 = createPayOrder(sessionVo, deviceId, userId, responseVo.getData().getMerchantCode(), responseVo.getData().getOrderRecords(), ctxMap);
                            if (responseVo1 != null && responseVo1.isSuccess()) {
                                FreePaymentResult result = (FreePaymentResult)responseVo1.getData();
                                asynSendMsgToSocketIo(NettyConst.EVENT_CLOSE_DOOR,
                                        SocketResponseVo.getSuccessResponse(result), socketIOClient);
                    ...
                    if (inventoryResult.getItype().intValue() == 10 || inventoryResult.getItype().intValue() == 20) { //购物关门
                        ...
                        //if (responseVo.getData().getItype().intValue() == 10) {//创建代扣支付订单成功
                            ...
                            if (responseVo1 != null && responseVo1.isSuccess()) {
                                FreePaymentResult result = (FreePaymentResult)responseVo1.getData();
                                asynSendMsgToSocketIo(NettyConst.EVENT_CLOSE_DOOR,
                                        SocketResponseVo.getSuccessResponse(result.getPushOrderParam()), socketIOClient);
                                logger.debug("扫码开门 -> 关门盘货 -> 创建支付代扣订单成功,设备ID：{}", deviceId);
                            }
                        //}
                        ...
                        //调用推送订单接口
                        FreePaymentDto paymentDto = new FreePaymentDto();
                        paymentDto.setSmerchantCode(clientVo.getMerchantCode());
                        paymentDto.setSmemberId(sessionVo.getUserId());
                        paymentDto.setSmemberName(sessionVo.getUserName());
                        paymentDto.setSmemberCode(sessionVo.getUserCode());
                        paymentDto.setIpayWay(BizTypeDefinitionEnum.PayWay.WITHHOLDING.getCode());//代扣
                        paymentDto.setIisIgnoreStatus(0);
                        paymentDto.setUserAuthToken(sessionVo.getApp_userAuthToken());
                        paymentDto.setApiKey(sessionVo.getApp_apiKey());
                        paymentDto.setSessionID(sessionVo.getApp_sessionId());
                        paymentDto.setDeviceCode(sessionVo.getDeviceCode());
                        paymentDto.setDeviceId(sessionVo.getDeviceId());
                        paymentDto.setAppDeviceId(sessionVo.getApp_deviceId());
                        paymentDto.setSremark(recordCode);//补货单号
                        invoke = RestServiceInvokeBuilder.newBuilder().newInvoker(FreeServicesDefine.VENDSTOP_PAYMENT);// 服务名称
                        invoke.setParameterizedTypeReference(new ParameterizedTypeReference<ResponseVo<FreePaymentResult>>() {
                        });
                        invoke.setRequestObj(paymentDto);
                        ResponseVo<FreePaymentResult> resVO = (ResponseVo<FreePaymentResult>) invoke.invoke();
                        logger.info("补货员关门：{}", resVO);
                        if (resVO.isSuccess()) {
                            FreePaymentResult result = (FreePaymentResult)resVO.getData();
                            asynSendMsgToSocketIo(NettyConst.EVENT_CLOSE_DOOR,
                                    SocketResponseVo.getSuccessResponse(result.getPushOrderParam()), socketIOClient);
                        }
                        // 给设备推送补货单
                        ...
                        socketResponseVo.setMsg("door is closed without picking any item.");
                        asynSendMsgToSocketIo(NettyConst.EVENT_CLOSE_DOOR, socketResponseVo, socketIOClient);
                        ...
                        // 给设备推送补货单
                        MsgToJsonUtils.asynSendNoCodeMsgByCtxMap(deviceId, ctxMap, true, CommonConstants.GENERATE_REPLENISHMENT_ORDERS, userId, TypeConstant.CLOSE_DOOR_REPLENISH_ORDER);
                        logger.debug("扫码开门 -> 关门盘货 -> 等待补货员确认补货单，设备ID：{}", deviceId);
                    }
                    //更新关门日志
                    LogUtils.updateOPLog(deviceCode, userId, closeType);
                    logger.info("设备关门盘货处理完成");
     ...
     */
    public ResponseVo createPayOrder(final SessionVo sessionVo, final String deviceId, final String userId, final String merchantCode, final List<CreatOrderResult> orderRecords, final ConcurrentMap<String, ChannelHandlerContext> ctxMap) {
//        ExecutorManager.getInstance().execute(new Runnable() {
//            @Override
//            public void run() {
                boolean flag = true;
                OrderRecord orderRecord = null;
                ResponseVo<FreePaymentResult> resVO = new ResponseVo<>();
                ...
                try {
                   ...
                   /* 微信代扣方式 10:签约免密  20:微信支付分 */
                    ...
                    for (CreatOrderResult orderResult : orderRecords) {
                        logger.info("创建免密支付订单,参数：{}", orderResult);
                        ...
                        try {
                            ...
                            if (null == clientConf/*
                                    || (orderRecord.getIpayType().intValue() == BizTypeDefinitionEnum.PayType.PAY_WECHAT.getCode() && clientConf.getIisConfWechat().intValue() == 0)
                            ...
                            resVO.setMsg("Merchant client configuration is abnormal");
                            return resVO;
                        }
                        ...
                        if (!merchantCode.equals(orderRecord.getSmerchantCode())) {
                            ...
                            resVO.setMsg("order is abnormal");
                            return resVO;
                        }
                        if (orderRecord.getIstatus().intValue() == 100) {//待支付  商户代扣
                            payFlag = true;
                            //创建支付
                            ...
                            }else if(orderRecord.getIpayType().intValue() == BizTypeDefinitionEnum.PayType.PAY_VENDSTOP.getCode()){
                                url = FreeServicesDefine.VENDSTOP_PAYMENT;
                                paymentDto.setUserAuthToken(sessionVo.getApp_userAuthToken());
                                paymentDto.setApiKey(sessionVo.getApp_apiKey());
                                paymentDto.setSessionID(sessionVo.getApp_sessionId());
                                paymentDto.setDeviceCode(sessionVo.getDeviceCode());
...
//            sessionVo = (SessionVo) iCached.get(IPUtils.getRemoteAddress(socketIOClient));
       ...
       /*     } else if ((TypeConstant.CLOSE_DOOR + "_CONFIRM").equals(methodType) && Integer.valueOf(200).equals(code)) { // 关门成功*/
                ...
                try {
                    ...
                    if (StringUtil.isBlank(token)) {
                        logger.info("设备:{},重力视觉柜关门生成订单重复处理", deviceId);
                        return;
                    }
            ...
            if (TypeConstant.OPEN_DOOR.equals(methodType)) {    //开门失败
                logger.debug("设备:{}发送开门失败消息到netty服务器", deviceId);
                WebSocketConstant.OPEN_REQUEST_FLAG = true;//更改请求开门状态
                //给用户發送通知
    ...
    private void sendOpenDoorFailedToCellPhone(BaseResponseVo baseResponseVo) {
        ...
        socketResponseVo.setSuccess(false);
        socketResponseVo.setMsg("open fail");
        ...
        try {
            socketIOClient.sendEvent(NettyConst.EVENT_OPEN_DOOR, JSON.toJSON(socketResponseVo));
        } catch (Exception e) {
 ...
 /*   private void layeredGeOrderDelWe(BigDecimal iele, DeviceInfo deviceInfo, BigDecimal subResult , String userId) throws Exception {
    ...
    public void localGravityLayeredCloseDoor(String deviceCode, String deviceId, BaseResponseVo baseResponseVo) {
        ...
        try {
            ...
            Map<String,String> vendstop_param = new HashMap<>();
            vendstop_param.put("appUserAuthToken",sessionVo.getApp_userAuthToken());
            vendstop_param.put("appSessionID",sessionVo.getApp_sessionId());
            vendstop_param.put("appDeviceId",sessionVo.getApp_deviceId());
            layeredInventoryDto.setSext(JSON.toJSONString(vendstop_param));
            ...
            ResponseVo<InventoryResult> responseVo = (ResponseVo<InventoryResult>) invoke.invoke();
            logger.info("====盘货结果：{}", JSON.toJSONString(responseVo));
     ...
     * @param merchantCode
     */
    private void cancelWechatPointOrder(final String userId, final String merchantCode, final String deviceId) {
        ExecutorManager.getInstance().execute(new Runnable() {
            @Override
            public void run() {
                try {
