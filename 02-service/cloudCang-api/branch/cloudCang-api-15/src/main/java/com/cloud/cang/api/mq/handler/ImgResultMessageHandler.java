...
public class ImgResultMessageHandler implements RmqMessageHandler<Mq_Cloud_ImgResul> {
    ...
    public HandlerEnum handler(RmqMessage<Mq_Cloud_ImgResul> rmqMessage) {
        logger.info("云端识别消费MQ消息：{}", rmqMessage);
            ...
            if (isSuccess) {
                ...
                if (10 == type) {
                    ...
                    if (null != responseVo && responseVo.isSuccess()) {
                        ...
                        logger.info("云端开门识别Mq消费消息时间：{}ms", closeTime - openTime);
                    ...
                    if (null != responseVo && responseVo.isSuccess()) {
                        Long closeTime = System.currentTimeMillis();
                        logger.info("云端实时识别Mq消费消息时间：{}ms", closeTime - openTime);
                        return HandlerEnum.ACK;
                    }
                } else if (30 == type) {
                    LogUtils.updateOPPicLog(message.getDeviceId(), message.getUserId(), message.getPicUrlList());
                    responseVo = imgCloudRecognition.cloudCloseDoor(message.getDeviceId(), message.getKey(), message.getUserId(), message.getOpenDoorType(), message.getImgResultDetail(), message.getUniquelyIdentifies());
                    logger.info("云端识别关门返回：{}", responseVo);
                    if (null != responseVo && responseVo.isSuccess()) {
                        Long closeTime = System.currentTimeMillis();
                        logger.info("云端关门识别Mq消费消息时间：{}ms", closeTime - openTime);
                        return HandlerEnum.ACK;
                    }
                } else {
