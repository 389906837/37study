...
public class LogUtils {
    ...
    static DeviceModelService deviceModelService;
...
//    static MerchantInfoService merchantInfoService;
    ...
    private static DeviceModelService getDeviceModelService() {
        if (deviceModelService == null) {
            deviceModelService = SpringContext.getBean(DeviceModelService.class);
        }
...
//    }
    ...
    public static void addOPLog(SessionVo sessionVo) {
        ...
        try {
            ...
            DeviceModel deviceModel = getDeviceModelService().selectByDeviceId(sessionVo.getDeviceId());
            /*rawstock 原始库存*/
            if (StringUtil.isBlank(picList) && "rawstock".equals(deviceModel.getScontrastMode())) {
                //取上一次关门图片为本次设备开门图片
                DeviceOper temp = getDeviceOperService().selectLastCloseDoorByDeviceCode(sessionVo.getDeviceCode());
                deviceOper.setSopenPicUrl(temp.getSclosePicUrl());
            } else {
                deviceOper.setSopenPicUrl(picList);
            }
            iCached.remove("cloudOpenPicUrl_" + sessionVo.getDeviceId() + "_" + sessionVo.getUserId());
        } catch (Exception e) {
     ...
     * @param userId   用户ID
     */
    public static void updateOPPicLog(final String deviceId, final String userId, String urlList) {
        final DeviceOper deviceOper = new DeviceOper();
        ...
        ExecutorManager.getInstance().execute(new Runnable() {
            ...
            public void run() {
                Map<String, String> map = new HashMap<>();
                map.put("sdeviceCode", deviceInfo.getScode());
                map.put("smemberId", userId);
                String id = getDeviceOperService().selectLastOpLog(map);
                if (StringUtils.isNotBlank(id)) {
                    deviceOper.setId(id);
