                    ...
                    for (CreatOrderResult orderResult : orderRecords) {
                        ...
                        if (orderRecord.getIstatus().intValue() == 100 && (orderRecord.getIchargebackWay().intValue() == 10
                            ...
                            if ((orderRecord.getIpayType().intValue() == BizTypeDefinitionEnum.PayType.PAY_WECHAT.getCode()
                                    && merchantConf.getIwechatWithholdType() == 10) || orderRecord.getIpayType().intValue() == BizTypeDefinitionEnum.PayType.PAY_ALIPAY.getCode()) {
                                payFlag = true;
                                //创建支付
                         ...
                         /*       String usedCouponId = orderRecord.getSusedCouponId();
                                if (StringUtil.isNotBlank(usedCouponId)) {
                                    ActivityConf activityConf = orderRecordService.selectACByUsedCouponId(orderRecord.getId());
                                    List<DiscountDto> discountDtos = null;
                                    if (activityConf != null) {
                                        discountDtos = new ArrayList<DiscountDto>();
