...
public class ImgWeightResultMessageHandler implements RmqMessageHandler<Mq_Cloud_Weight_ImgResul> {
    ...
    public HandlerEnum handler(RmqMessage<Mq_Cloud_Weight_ImgResul> rmqMessage) {
        logger.info("云端重力识别消费MQ消息：{}", rmqMessage);
            ...
            if (isSuccess) {
                ...
                if (10 == openDoorType) {
                    ...
                    if (10 == type) {
                        ...
                        baseResponseVo.setCloseDoorIden(message.getUniquelyIdentifies());
                        nettyMsgService.localGravityLayeredCloseDoor(message.getDeviceId(), baseResponseVo);
                    }
        ...
        logger.info("云端称重识别Mq消费消息时间：{}ms", closeTime - openTime);
