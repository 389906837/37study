...
import com.cloud.cang.api.hy.service.TrashAuthoriseService;
...
public class NettyMsgService {
    ...
    @Autowired
    TrashAuthoriseService trashAuthoriseService;
    ...
    public void openDoorSuccess(BaseResponseVo baseResponseVo) {
        ...
        if (null == openSource || (null != openSource && 20 == openSource)) {
            if (null == openSource) {
                logger.info("开门来源为空，手机端用户ID：{}", userId);
            }
            //手机端与服务器通信通道断开
            ...
            if (null != deviceInfo.getIsupportAi() && deviceInfo.getIsupportAi() == 1) {
    ...
    public void gravityOpenDoor(BaseResponseVo baseResponseVo) {
        logger.info("开始处理售货机设备发送过来的开门实时盘货结果");
    ...
    public void closeDoorInventorySuccess(BaseResponseVo baseResponseVo) {
        ...
        if (null == openSource || (null != openSource && 20 == openSource)) {
            ...
            try {
                ...
                logger.info("扫码开门 -> 关门盘货，调用关门盘货服务完成，调用结果：{}，设备ID：{}", responseVo.isSuccess(), deviceId);
                // 处理调用服务成功的结果
                if (responseVo.isSuccess()) {
                    logger.info("扫码开门调用关门盘货服务成功,设备ID：{}", deviceId);
                    ...
                    if (inventoryResult.getItype().intValue() == 10 || inventoryResult.getItype().intValue() == 20) { //购物关门
                        ...
                        logger.info("扫码开门 -> 关门盘货 -> 给大屏设备推送购物订单，设备ID：{}", deviceId);
                        ...
                        if (responseVo.getData().getItype().intValue() == 10) {//创建代扣支付订单成功
                            ...
                            logger.info("扫码开门 -> 关门盘货 -> 创建支付代扣订单成功,设备ID：{}", deviceId);
                        }
                        ...
                        logger.info("扫码开门 -> 关门盘货 -> 给设备推送补货单，设备ID：{}", deviceId);
                        ...
                        if (openDoorType == 10) {//购物开门
                            ...
                            logger.info("扫码开门 -> 关门盘货 -> 游客类型，给设备推送消息，设备ID：{}", deviceId);
                            ...
                            logger.info("扫码开门 -> 关门盘货 -> 补货员类型，给设备推送消息，设备ID：{}", deviceId);
                        }
                        asynSendMsgToSocketIo(NettyConst.SOCKETIO_EVENT, socketResponseVo, socketIOClient);
            ...
            if (null != deviceInfo.getIsupportAi() && deviceInfo.getIsupportAi() == 1) {
                ...
                if (StringUtils.isNotBlank(aiId)) {
                    logger.info("大屏设备带有小屏设备，扫码关门给小屏推送关门消息，大屏ID：{}，小屏ID：{}", deviceId, aiId);
            ...
            logger.info("刷脸开门 -> 关门盘货，设备ID：{}", deviceId);
            ...
            try {
                ...
                if (null == faceDeviceVo) {
                    removeUserOpenDoorKey(userId);
                    logger.error("从redis中获取AI设备信息失败，处理刷脸开门,关门盘货消息失败，设备编号:{}", deviceCode);
                    return;
                }
            } catch (Exception e) {
                removeUserOpenDoorKey(userId);
            ...
            RestServiceInvoker invoke = null;
            try {
                logger.info("刷脸开门 -> 关门盘货，开始调用关门盘货服务，设备ID：{}", deviceId);
                invoke = RestServiceInvokeBuilder.newBuilder().newInvoker(InventoryServicesDefine.INVENTORY_DEALWITH_SERVICE);
                ...
                ResponseVo<InventoryResult> responseVo = (ResponseVo<InventoryResult>) invoke.invoke();
                logger.info("刷脸开门 -> 关门盘货，调用关门盘货服务完成，调用结果：{}，设备ID：{}", responseVo.isSuccess(), deviceId);
                // 处理调用服务成功的结果
                if (responseVo.isSuccess()) {
                    InventoryResult inventoryResult = responseVo.getData();
                    //发送消息给终端
    ...
    public void closeDoorConfirmInventorySuccess(BaseResponseVo baseResponseVo) {
        logger.info("开始处理关门盘货成功消息消息,设备ID：{}，用户ID：{}", baseResponseVo.getDeviceId(), baseResponseVo.getUserId());
        String deviceId = baseResponseVo.getDeviceId();
        String userId = baseResponseVo.getUserId();
        String goodsString = baseResponseVo.getData();
        ...
        if (null == openSource || (null != openSource && 20 == openSource)) {
            ...
            if (null != openDoorType && 10 == openDoorType) {
                try {
                    iCached.put(baseResponseVo.getCloseDoorIden() + "_" + deviceId, userId);
                } catch (Exception e) {
                    logger.error("关门盘货唯一标识存入redis异常：{}", e);
                }
            }
            // 将商品对象由字符串转化为对象
            try {
            ...
            sourceClientType = sessionVo.getIsourceClientType();                //来源客户端类型
            clientIp = sessionVo.getSip();
            //openDoorType = sessionVo.getTypes();
            if (null == openDoorType) {
                logger.info("开门类型为空");
            } else if (10 == openDoorType) {
                logger.info("购物开门");
            ...
            try {
                ...
                if (responseVo.isSuccess()) {
                    ...
                    //发送消息给手机客户端
                    if (inventoryResult.getItype().intValue() == 10 || inventoryResult.getItype().intValue() == 20) { //购物关门
                        iCached.remove("closeDoorOvertime_" + baseResponseVo.getUserId() + "_" + baseResponseVo.getDeviceId());
                        socketResponseVo.setTypes(30);
                        StringBuffer sb = new StringBuffer();
                        List<CreatOrderResult> orderRecords = inventoryResult.getOrderRecords();
                        for (CreatOrderResult orderResult : orderRecords) {
                        ...
                        //  asynSendShoppingOrderByCtxMap(deviceId, userId, responseVo.getData().getItype(), orderRecords, ctxMap, 10);
                        logger.info("扫码开门 -> 关门盘货 -> 给大屏设备推送购物订单，设备ID：{}", deviceId);
                        ResponseVo<List<CreatOrderResult>> res = deductionPoint(deviceId, userId, responseVo.getData().getOrderRecords());
                        if (null == res || !res.isSuccess()) {
                            ...
                            // 给设备推送订单
                            asynSendShoppingOrderByCtxMap(deviceId, userId, responseVo.getData().getItype(), res.getData(), ctxMap, 20);
                        }
                    } else if (inventoryResult.getItype().intValue() == 30) { //补货关门
                        socketResponseVo.setTypes(40);
                        String recordCode = inventoryResult.getReplenishmentResult().getReplenishmentRecord().getScode();
                    ...
                    } else if (inventoryResult.getItype().intValue() == 70) { // 等待补货员确认补货单
                        socketResponseVo.setTypes(90);
                        iCached.put("CONFIRM_" + deviceCode, JSON.toJSONString(inventoryResult.getReplenishmentConfirmResult()), 60 * 60 * 2);
                        socketResponseVo.setData(deviceCode);
                        asynSendMsgToSocketIo(NettyConst.SOCKETIO_EVENT, socketResponseVo, socketIOClient);
                        // 给设备推送补货单
                        MsgToJsonUtils.asynSendNoCodeMsgByCtxMap(deviceId, ctxMap, true, CommonConstants.GENERATE_REPLENISHMENT_ORDERS, userId, TypeConstant.CLOSE_DOOR_REPLENISH_ORDER);
                        logger.info("扫码开门 -> 关门盘货 -> 等待补货员确认补货单，设备ID：{}", deviceId);
                    }
...
//            sessionVo = (SessionVo) iCached.get(IPUtils.getRemoteAddress(socketIOClient));
    ...
    public void handlerMessage(BaseResponseVo baseResponseVo) {
        ...
        if (StringUtils.isBlank(methodType)) {  //方法名为空直接返回
            logger.error("设备编号：" + deviceCode + "发送消息的方法名为空！");
            return;
        }
        ...
        if (BooleanUtils.isTrue(success)) {
            if (TypeConstant.OPEN_DOOR.equals(methodType) && Integer.valueOf(200).equals(code)) {    //开门成功
                logger.info("设备编号:{}发送开门成功消息到netty服务器", deviceCode);
          ...
          /*  } else if ((TypeConstant.CLOSE_DOOR + "_" + TypeConstant.INVENTORY).equals(methodType) && Integer.valueOf(200).equals(code)) { // 关门成功
                logger.info("设备编号:{}发送关门盘货成功消息到netty服务器", deviceId);
                closeDoorInventorySuccess(baseResponseVo);*/
       /*     } else if ((TypeConstant.CLOSE_DOOR + "_CONFIRM").equals(methodType) && Integer.valueOf(200).equals(code)) { // 关门成功*/
            } else if ((TypeConstant.CLOSE_DOOR + "_" + TypeConstant.INVENTORY).equals(methodType) && Integer.valueOf(200).equals(code)) { // 关门成功
                logger.info("设备编号:{}发送关门盘货成功消息到netty服务器", deviceCode);
                closeDoorConfirmInventorySuccess(baseResponseVo);
            } else if (TypeConstant.OPEN_DOOR_INVENTORY.equals(methodType) && Integer.valueOf(200).equals(code)) {  //开门状态下盘货结果
                logger.info("设备编号:{}发送开门状态实时盘货结果消息到netty服务器", deviceCode);
                openDoorInventory(baseResponseVo);
            } else if (TypeConstant.IS_ALIVE.equals(methodType) && Integer.valueOf(200).equals(code)) { // 设备是否在线
                logger.info("设备编号:{}发送在线消息到netty服务器", deviceCode);
                ...
                logger.info("设备编号:{}，收货机接收到苏宁的上货信息", deviceCode);
                ...
                logger.info("设备编号：{}，接受到第三方接口调用记录", deviceCode);
                ...
                logger.info("设备编号：{}，发送本地视觉识别关门盘货消息", deviceCode);
                ...
                logger.info("设备编号：{}，发送本地视觉识别开门前盘货消息", deviceCode);
                ...
                logger.info("设备编号：{}，发送本地视觉识别开门实时盘货消息", deviceCode);
                ...
                logger.info("设备编号：{}，发送mgr后台主动盘货结果", deviceCode);
                ...
                logger.info("设备编号：{}，云端识别实时盘货跳转页面结果", deviceCode);
                ...
                logger.info("设备编号:{},设备开门前的校验", deviceCode);
                gravityOpenDoor(baseResponseVo);
            } else if (TypeConstant.WEIGHT_LAYERED_OPEN_DOOR.equals(methodType) && Integer.valueOf(200).equals(code)) {    //设备开门前的校验
                logger.info("设备编号:{},设备开门前的校验", deviceCode);
                gravityLayeredOpenDoor(baseResponseVo);
            } else if (TypeConstant.WEIGHT_LAYERED_REPLEN_OPEN_DOOR.equals(methodType) && Integer.valueOf(200).equals(code)) {    //设备补货开门前的校验
                logger.info("设备编号:{},设备补货开门前的校验", deviceCode);
                ...
                logger.info("设备编号：{}，重力识别分层补货实时盘货跳转页面结果", deviceCode);
                ...
                logger.info("设备编号：{}，重力识别实时盘货跳转页面结果", deviceCode);
                ...
                logger.info("设备编号：{}，重力识别分层实时盘货跳转页面结果", deviceCode);
                ...
                logger.info("设备编号:{},重力视觉柜关门生成订单开始", deviceCode);
                try {
                    iCached.remove("closeDoorOvertime_" + baseResponseVo.getUserId() + "_" + baseResponseVo.getDeviceId());
                    String token = (String) iCached.get("shopping_" + baseResponseVo.getUserId());
                    if (StringUtil.isBlank(token)) {
                        logger.info("设备编号:{},重力视觉柜关门生成订单重复处理", deviceCode);
                        return;
                    }
                    iCached.remove("shopping_" + baseResponseVo.getUserId());
                } catch (Exception e) {
                    logger.info("重力视觉柜关门生成订单重复处理异常", e);
                    return;
                }
                ...
                logger.info("设备编号:{},重力视觉柜分层关门生成订单开始", deviceCode);
                localGravityLayeredCloseDoor(deviceId, baseResponseVo);
            }

        } else {
            if (TypeConstant.OPEN_DOOR.equals(methodType)) {    //开门失败
                logger.info("设备编号:{}发送开门失败消息到netty服务器", deviceCode);
                WebSocketConstant.OPEN_REQUEST_FLAG = true;//更改请求开门状态
                //给用户發送通知
            ...
            if (TypeConstant.QR_CODE.equals(methodType)) {   // 设备请求发送二维码
                ...
                logger.info("设备编号:{}发送Apk升级失败消息到netty服务器", deviceCode);
                upgradeApkResult(baseResponseVo);
            } else if (TypeConstant.UPGRADE_VOICE.equals(methodType)) {
                logger.info("设备编号:{}发送语音升级失败消息到netty服务器", deviceCode);
                upgradeVoiceResult(baseResponseVo);
            }
            return;
        }
    ...
    private boolean isOnLine(String deviceId) {
        try {
            ...
            //门处于关闭状态
            if (null != doorStatus && Integer.valueOf(10).equals(doorStatus)) {
                logger.info("设备门已经关闭");
                return true;
            }
        } catch (Exception e) {
            logger.info("实时订单判断设备门是否关闭异常：{}", e);
        }
        return false;
    }
     ...
     * @param baseResponseVo
     */
    private void sendOpenDoorFailedToCellPhone(BaseResponseVo baseResponseVo) {
        String deviceId = baseResponseVo.getDeviceId();
        String userId = baseResponseVo.getUserId();
    ...
    public void handlerTestMessage(BaseResponseVo baseResponseVo, ChannelHandlerContext ctx) {
        ...
        if (BooleanUtils.isTrue(success)) {
            if (TypeConstant.TEST_CONNECTION.equals(methodType) && Integer.valueOf(200).equals(code)) {    //连接测试
                logger.info("设备编号:{}发送测试连消息到netty服务器", deviceId);
    ...
    private void cloudRealtimeJump(String deviceId, String deviceCode, BaseResponseVo baseResponseVo) {
        ...
        try {
            ...
            asynSendMsgToSocketIo(NettyConst.SOCKETIO_EVENT, socketResponseVo, socketIOClient);
            logger.info("云端识别实时盘货跳转页面结果完成，用户ID:{}", userId);
            return;
        }
        asynSendMsgToSocketIo(NettyConst.SOCKETIO_EVENT, socketResponseVo, socketIOClient);
        logger.info("云端识别实时盘货跳转页面结果完成，用户ID:{}", userId);
            ...
            if (NettyConstant.SHOPPING.equals(openDoorType)) {   // 购物开门
                ...
                if (responseVo.isSuccess()) {
                    ...
                    if (null != realTimeOrderResult) {
                        ...
                        logger.info("商品数量没有变化，没有购物开门实时盘货订单生成");
                    }
                } else {
    ...
    private void localGravityOpenDoorInventory(String deviceId, String deviceCode, BaseResponseVo baseResponseVo) {
        logger.info("开始处理售货机设备发送过来的开门实时盘货结果：{}", deviceCode);
        Boolean flag = isOnLine(deviceId);
        if (flag) {
            logger.info("设备门已关闭,结束购物实时订单：{}", deviceCode);
            return;
        }
        String dataString = baseResponseVo.getData();                           // 商品盘货结果
        ...
        try {
            if (NettyConstant.SHOPPING.equals(openDoorType)) {   // 购物开门
                ...
                if (subTemp.abs().compareTo(deviceInfo.getIelectronicFloat()) <= 0) {
                    ...
                    logger.info("购物实时订单-->调用购物开门实时盘货服务成功");
                    ...
                    if (subResult.subtract(subTotal).abs().compareTo(floatResult.add(iele)) <= 0) {
                       ...
                       /* String json = (String) iCached.get("localGravityOpenDoorInventory_" + deviceId);
                        if (StringUtil.isBlank(json)) {
                            //发送空订单
                            logger.info("实时购物重力不匹配,缓存中未查询到上次的订单,发送空订单：{}", deviceCode);
                            MsgToJsonUtils.asynSendNoCodeMsgByCtxMap(deviceId, ctxMap, true, null, userId, TypeConstant.OPEN_DOOR_INVENTORY);
                        } else {
                            OrderModel orderModel1 = JSON.parseObject(json, OrderModel.class);
                            logger.info("实时购物重力不匹配,发送缓存中订单：{}", JSON.toJSONString(orderModel1));
     ...
     * @param baseResponseVo 消息体
     */
    public void localGravityLayeredOpenDoorInventory(String deviceId, String deviceCode, BaseResponseVo baseResponseVo) {
        logger.info("开始处理售货机设备发送过来的开门实时盘货结果：{}", deviceCode);
        Boolean flag = isOnLine(deviceId);
        if (flag) {
            logger.info("设备门已关闭,结束购物实时订单：{}", deviceCode);
        ...
        try {
            if (NettyConstant.SHOPPING.equals(openDoorType)) {   // 购物开门
                ...
                if (subTemp.abs().compareTo(ieleTotal) <= 0 || ieleTotal.compareTo(BigDecimal.ZERO) == 0) {
                    //重力不变,不出订单
                    ...
                    logger.info("分层购物实时订单-->调用购物开门实时盘货服务成功");
                 ...
                 /*   List<LayeredGoods> realGoods = (List<LayeredGoods>) map.get("layeredGoodsList");
                    ...
                    if (subResult.subtract(subTotal).abs().compareTo(floatResult.add(iele)) <= 0) {
                        ...
                        if (StringUtil.isBlank(json)) {
                            ...
                            OrderModel orderModel = JSON.parseObject(json, OrderModel.class);
                            logger.info("实时购物重力不匹配,发送缓存中订单：{}", JSON.toJSONString(orderModel));
                            MsgToJsonUtils.asynSendNoCodeMsgByCtxMap(deviceId, ctxMap, true, orderModel, userId, TypeConstant.OPEN_DOOR_INVENTORY);
                        }*/
     ...
     * @param baseResponseVo 消息体
     */
    public void localGravityLayeredReplenOpenDoorInventory(String deviceId, String deviceCode, BaseResponseVo baseResponseVo) {
        logger.info("补货员补货,开始处理售货机设备发送过来的开门实时盘货结果：{}", deviceCode);
        Boolean flag = isOnLine(deviceId);
        if (flag) {
            logger.info("设备门已关闭,结束补货实时订单：{}", deviceCode);
            return;
        }
        String dataString = baseResponseVo.getData();                           // 商品盘货结果
        ShopLayeredGoods shopLayeredGoods = new ShopLayeredGoods();
 ...
 /*   private void layeredGeOrderDelWe(BigDecimal iele, DeviceInfo deviceInfo, BigDecimal subResult , String userId) throws Exception {
    ...
    private void geOrder(String userId, String deviceCode,
        ...
        orderModel.setGoodsList(goodModelList);                                             // 订单商品集合
        // 发送消息
       /* iCached.put("localGravityOpenDoorInventory_" + deviceId, JSON.toJSONString(orderModel), 60 * 60 * 2);*/
        MsgToJsonUtils.asynSendNoCodeMsgByCtxMap(deviceId, ctxMap, true, orderModel, userId, TypeConstant.OPEN_DOOR_INVENTORY);
    }
    ...
    private List<LayeredReplenGoodModel> calcLayeredReplenCommodityDiffData(List<LayeredGoods> realGoods, List<LayeredGoods> openGoods, DeviceInfo deviceInfo) {
        ...
        for (int x = 0; x < realGoods.size(); x++) {
            ...
            for (Map.Entry<String, Integer> openDoor : openDoorMap.entrySet()) {
                ...
                layeredReplenGoods.setGoodName(commodityInfo.getSbrandName() + commodityInfo.getSname() + commodityInfo.getStaste() + commodityInfo.getIspecWeight() + commodityInfo.getSspecUnit() + "/" + commodityInfo.getSpackageUnit() + "(" + commodityInfo.getIweigth() + "g)");
    ...
    public void localGravityCloseDoor(String deviceCode, String deviceId, BaseResponseVo baseResponseVo) {
        ...
        try {
            ...
            SocketResponseVo<String> socketResponseVo = SocketResponseVo.getSuccessResponse();
            logger.info("扫码开门 -> 关门盘货差，调用关门盘货服务完成，调用结果：{}，设备编号：{}", responseVo.isSuccess(), getDeviceCodeByDeviceId(deviceId));
            // 处理调用服务成功的结果
            if (responseVo.isSuccess()) {
                logger.info("扫码开门调用关门盘货服务成功,设备ID：{}", deviceId);
                InventoryResult inventoryResult = responseVo.getData();
     ...
     * @param baseResponseVo
     */
    public void localGravityLayeredCloseDoor(String deviceId, BaseResponseVo baseResponseVo) {
        String deviceCode = getDeviceCodeByDeviceId(deviceId);
        logger.info("开始处理本地重力视觉识别关门盘货成功消息消息,设备编号：{}，用户ID：{}", deviceCode, baseResponseVo.getUserId());
        try {
            iCached.remove("closeDoorOvertime_" + baseResponseVo.getUserId() + "_" + baseResponseVo.getDeviceId());
            String token = (String) iCached.get("shopping_" + baseResponseVo.getUserId());
            if (StringUtil.isBlank(token)) {
                logger.info("设备:{},重力视觉柜分层关门生成订单重复处理", deviceId);
                return;
            }
            iCached.remove("shopping_" + baseResponseVo.getUserId());
        } catch (Exception e) {
            logger.info("重力视觉柜分层关门生成订单重复处理异常", e);
            return;
        }
        String userId = baseResponseVo.getUserId();
        String goodsString = baseResponseVo.getData();
        ...
        try {
            ...
            logger.info("购物关门生成订单-->重力视觉柜开关门对比，开门商品数据,{}", openMap);
            List<LayeredGoods> openGoods = JSON.parseObject(openMap, new TypeReference<List<LayeredGoods>>() {
            });
            ...
            logger.info("扫码开门 -> 关门盘货差，调用关门盘货服务完成，调用结果：{}，设备编号：{}", responseVo.isSuccess(), getDeviceCodeByDeviceId(deviceId));
            // 处理调用服务成功的结果
            //  String jsonUrL = (String) iCached.get("cloudClosePicUrl_" + deviceId + userId);
            if (responseVo.isSuccess()) {
                logger.info("扫码开门调用关门盘货服务成功,设备ID：{}", deviceId);
                InventoryResult inventoryResult = responseVo.getData();
                //发送消息给终端
     ...
     * @return
     */
    public ResponseVo<List<CreatOrderResult>> deductionPoint(String deviceId, String userId, List<CreatOrderResult> orderRecords) {
        try {
            DeviceInfo deviceInfo = deviceInfoService.selectByPrimaryKey(deviceId);
            MemberInfo memberInfo = memberInfoService.selectByPrimaryKey(userId);
            if (null != deviceInfo.getIisBindTrash() && deviceInfo.getIisBindTrash() == 1) {
                ...
                map.put("strashCode", deviceInfo.getStrashCode());
                map.put("strashBrand", deviceInfo.getStrashBrand());
                map.put("istatus", 10);
                TrashAuthorise trashAuthorise = trashAuthoriseService.selectByUserIdAndTrash(map);
                if (trashAuthorise != null) {
                    List<CreatOrderResult> creatOrderResults = new ArrayList<>();
                    CreatOrderResult creatOrderResult = null;
                    for (CreatOrderResult orderResult : orderRecords) {
                        OrderRecord orderRecord = orderResult.getOrderRecord();
                        if (orderRecord.getIstatus().intValue() == 100) {
                            DeductionPointDto deductionPointDto = new DeductionPointDto();
                            deductionPointDto.setOrderId(orderRecord.getId());
                            deductionPointDto.setMobile(memberInfo.getSmobile());
                            RestServiceInvoker invoke = RestServiceInvokeBuilder.newBuilder().newInvoker(PayServicesDefine.DEDUCTION_POINT);
                            invoke.setRequestObj(deductionPointDto);
                            // 返回对象中含有泛型，则需要设置返回类型，否则无需设置
                            invoke.setParameterizedTypeReference(new ParameterizedTypeReference<ResponseVo<CreatOrderResult>>() {
                            });
                            ResponseVo<CreatOrderResult> responseVo = (ResponseVo<CreatOrderResult>) invoke.invoke();
                            if (responseVo != null && responseVo.isSuccess()) {
                                creatOrderResult = responseVo.getData();
                            } else {
                                creatOrderResult = orderResult;
                            }
                            creatOrderResults.add(creatOrderResult);
                        }
                    }
                    return ResponseVo.getSuccessResponse(creatOrderResults);
                }
            }
        } catch (Exception e) {
            logger.error("调用第三方接口异常：{}", e);
        }
        return ErrorCodeEnum.ERROR_COMMON_SAVE.getResponseVo("积分抵扣异常");
    }
     ...
     * @param type
     */
    public void openDoor(String deviceId, String userId, Integer type) {
        MsgToJsonUtils.asynSendNoCodeMsgByCtxMap(deviceId, ctxMap, true, true, userId, TypeConstant.WEIGHT_OPEN_DOOR);
    }
    ...
    private void comapreLastCloseDoorStock(List<LocalGVGoodModel> localGVGoodModelList, String deviceCode, String deviceId) {
        ...
        try {
            ...
            ResponseVo<InventoryResult> responseVo = (ResponseVo<InventoryResult>) invoke.invoke();
            logger.info("调用开门前盘货对比上次关门时库存服务完成，调用结果：{}，设备编号：{}", responseVo.isSuccess(), getDeviceCodeByDeviceId(deviceId));
            // 处理调用服务成功的结果
            if (responseVo.isSuccess()) {
                logger.info("调用开门前盘货对比上次关门时库存服务成功,设备编号：{}", getDeviceCodeByDeviceId(deviceId));
            } else {
                logger.info("调用开门前盘货对比上次关门时库存服务失败,设备编号：{}", getDeviceCodeByDeviceId(deviceId));
            }
        } catch (Exception e) {
            logger.info("调用开门前盘货对比上次关门时库存服务失败,设备编号：{}，异常原因：{}", getDeviceCodeByDeviceId(deviceId), e);
        }
    }
    ...
    private void insertThirdInterfaceTransferLog(String deviceId, String deviceCode, BaseResponseVo baseResponseVo) {
        logger.info("开始处理记录第三方接口调用日志信息，设备编号:{}", deviceId);
        String data = baseResponseVo.getData();
        logger.info("模型json字符串：{}", data);
        ...
        try {
            if (StringUtil.isNotBlank(data) && !data.equals("[]")) {
                ...
                logger.info("记录第三方接口调用日志对象json转化完成，设备ID：{}", deviceId);
            }
        } catch (Exception e) {
    ...
    private void upgradeVoiceResult(BaseResponseVo baseResponseVo) {
        ExecutorManager.getInstance().execute(new Runnable() {
            @Override
            public void run() {
                logger.info("开始处理记录语音升级结果，设备ID:{}", baseResponseVo.getDeviceId());
                String deviceId = baseResponseVo.getDeviceId();
                ...
                try {
                    ...
                    logger.error("处理售货机设备编号:{}发送过来的语音升级结果，json转换出现异常:{}", deviceId, e);
                    return;
                }
                ...
                if (null != upgradeResultModel) {
                    ...
                    logger.info("开始向数据库中添加记录语音升级结果，明细表ID:{}，开始升级时间：{}，升级结束时间：{}，结果：{}，设备ID：{}", updateId, startTime, endTime, success.equals("1") ? "成功" : "失败", deviceId);
                    if (StringUtils.isNotBlank(updateId) && StringUtils.isNotBlank(startTime) && StringUtils.isNotBlank(endTime)
                        ...
                        if ("1".equals(success)) {
                            ...
                            logger.info("语音升级结果:成功，设备ID：{}", upgradeResultModel.getUpgradeId());
                        } else if ("0".equals(success)) {
                            deviceUpgradeDetails.setIstatus(30);
                            deviceUpgradeDetails.setSexceptionDesc(reason);                                     //  升级失败原因
                            logger.info("语音升级结果：失败，原因:{}，设备ID：{}", reason, upgradeResultModel.getUpgradeId());
                        }
                        ...
                        logger.info("记录语音升级结果完成，明细表ID:{}，设备ID：{}", upgradeResultModel.getUpgradeId());
                    }
                }
            }
        });
    ...
    private void suningReplenishment(BaseResponseVo baseResponseVo) {
        ExecutorManager.getInstance().execute(new Runnable() {
            ...
            public void run() {
                ...
                logger.info("开始记录苏宁设备的上货信息，设备ID:{}", deviceId);
                DeviceInfo deviceInfo = deviceInfoService.selectByPrimaryKey(deviceId);
                if (null == deviceInfo) {
                ...
                logger.info("开始处理苏宁接口传输的上货信息，设备编号:{}", deviceId);
                String data = baseResponseVo.getData();
                logger.info("模型json字符串：{}", data);
                ...
                try {
                    if (StringUtil.isNotBlank(data) && !data.equals("[]")) {
                        ...
                        logger.info("苏宁上货对象json转化完成，设备ID：{}", deviceId);
                    }
                } catch (Exception e) {
                ...
                if (null != suNingGoods) {
                    ...
                    logger.info("不存在商品模型，设备ID：{}", deviceId);
                }
    ...
    private void suningInventory(BaseResponseVo baseResponseVo) {
        ExecutorManager.getInstance().execute(new Runnable() {
            ...
            public void run() {
                ...
                Integer userType = null;    // 用户类型
                List<SuNingGoodChangeModel> goodChangeModelList = null; // 变化商品集合
                logger.info("开始记录苏宁设备的盘货信息，设备ID:{}", deviceId);
                DeviceInfo deviceInfo = deviceInfoService.selectByPrimaryKey(deviceId);
                if (null == deviceInfo) {
                    logger.error("设备不存在，设备ID：{}", deviceId);
                    return;
                ...
                String deviceCode = deviceInfo.getScode();
                logger.info("开始处理苏宁接口传输的盘货信息，设备编号:{}", deviceId);
                String data = baseResponseVo.getData();
                logger.info("模型json字符串：{}", data);
                SuNingGoods suNingGoods = null;
                // 将字符串转化为对象
                try {
                    if (StringUtil.isNotBlank(data) && !data.equals("[]")) {
                        suNingGoods = JSON.parseObject(data, SuNingGoods.class);
                        logger.info("苏宁盘货对象json转化完成，设备ID：{}", deviceId);
                    }
                } catch (Exception e) {
        ...
        ExecutorManager.getInstance().execute(new Runnable() {
            ...
            public void run() {
                ...
                if (StringUtils.isBlank(userId)) {
                    ...
                    logger.info("从Redis中获取开门的用户ID：{}，设备ID：{}", userId, deviceId);
                }
                Integer userType = null;
                List<SuNingGoodChangeModel> goodChangeModelList = null;
                logger.info("开始记录苏宁设备的关门信息，设备ID:{}", deviceId);
                DeviceInfo deviceInfo = deviceInfoService.selectByPrimaryKey(deviceId);
                if (null == deviceInfo) {
                    logger.error("设备不存在，设备ID：{}", deviceId);
                    return;
                ...
                String deviceCode = deviceInfo.getScode();
                logger.info("开始处理苏宁接口传输的关门信息，设备编号:{}", deviceId);
                String data = baseResponseVo.getData();
                logger.info("模型json字符串：{}", data);
                SuNingGoods suNingGoods = null;
                // 将字符串转化为对象
                try {
                    if (StringUtil.isNotBlank(data) && !data.equals("[]")) {
                        suNingGoods = JSON.parseObject(data, SuNingGoods.class);
                        logger.info("苏宁关门对象json转化完成，设备ID：{}", deviceId);
                    }
                } catch (Exception e) {
...
//                            thirdDeviceSkuVo.setIisLowerShelf(0);
                        ...
                        // 插入关门操作记录
                        logger.info("开始插入关门操作记录，设备ID：{}", deviceId);
                        ...
                        logger.info("插入关门操作记录完成，设备ID：{}，记录表ID：{}", deviceId, operateDeviceRecord.getId());

                        // 插入操作记录明细
                        logger.info("开始插入关门操作记录明细,设备ID：{}", deviceId);
                        ...
                        if (CollectionUtils.isNotEmpty(goodChangeModelList)) {
                            for (SuNingGoodChangeModel s : goodChangeModelList) {
                                ...
                                logger.info("商品增加数量：", addNumBd);
                                logger.info("商品减少数量：", subNumBd);
                    ...
                    if (StringUtils.isNotBlank(userId)) {
                        logger.info("开始查询该用户最后一次开门日志，设备ID:{},用户：{}", deviceId, userId);
                        ...
                        logger.info("开始插入设备关门日志，设备ID：{}，用户ID：{}", deviceId, userId);
                        ...
                        logger.info("修改用户设备关门日志完成，设备ID：{}，用户ID:{}", deviceId, userId);
                    } else {
    ...
    private void suningOpenDoor(BaseResponseVo baseResponseVo) {
        ExecutorManager.getInstance().execute(new Runnable() {
            ...
            public void run() {
                ...
                String userId = baseResponseVo.getUserId();
                String ip = "";
                logger.info("开始记录苏宁设备的开门信息，设备ID:{}", deviceId, userId);
                DeviceInfo deviceInfo = deviceInfoService.selectByPrimaryKey(deviceId);
                if (null == deviceInfo) {
                    logger.error("设备不存在，设备ID：{}", deviceId);
                    return;
                ...
                String deviceCode = deviceInfo.getScode();
                logger.info("开始处理苏宁接口传输的开门信息，设备编号:{}", deviceId);
                String data = baseResponseVo.getData();
                logger.info("模型json字符串：{}", data);
                SuNingGoods suNingGoods = null;
                // 将字符串转化为棚格图对象
                try {
                    if (StringUtil.isNotBlank(data) && !data.equals("[]")) {
                        ...
                        logger.info("苏宁开门对象json转化完成，设备ID：{}", deviceId);
                    }
                } catch (Exception e) {
                ...
                try {
                    ...
                    // 设备开门操作记录
                    if (StringUtils.isNotBlank(userId)) {
                        logger.info("开始插入设备开门日志，设备ID：{}，用户ID：{}", deviceId, userId);
                        DeviceOper deviceOper = new DeviceOper();
    ...
    private void suningShedInfo(BaseResponseVo baseResponseVo) {
        ExecutorManager.getInstance().execute(new Runnable() {
            ...
            public void run() {
                ...
                String deviceCode = deviceInfo.getScode();
                logger.info("开始处理苏宁接口传输的棚格图信息，设备编号:{}", deviceId);
                String data = baseResponseVo.getData();
                logger.info("模型json字符串：{}", data);
                ...
                try {
                    if (StringUtil.isNotBlank(data) && !data.equals("[]")) {
                        ...
                        logger.info("苏宁棚格图对象json转化完成，设备ID：{}", deviceId);
                    }
                } catch (Exception e) {
                ...
                if (CollectionUtils.isEmpty(suNingShedModelList)) {
                    logger.info("从棚格中获取苏宁商品集合为空,设备ID：{}", deviceId);
                }
                // 存放设备新的sku
    ...
    private void selfTestToNormal(BaseResponseVo baseResponseVo) {
        ...
        if (null != deviceInfo && 30 == deviceInfo.getIstatus()) {
            ...
            logger.info("接收到设备自检成功消息，将设备变为正常状态，设备ID：{}", baseResponseVo.getDeviceId());
        }
    }
        ...
        ExecutorManager.getInstance().execute(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    logger.error("处理售货机设备编号:{}发送过来的apk升级成功结果，json转换出现异常:{}", deviceId, e);
                    return;
                }
    ...
    private void closeDoorSuccess(BaseResponseVo baseResponseVo) {
        try {
            ...
            if (10 == openDoorType) {
                closeDoorOvertime(baseResponseVo);
            }
            String deviceId = baseResponseVo.getDeviceId();
            DeviceInfo deviceInfo = deviceInfoService.selectByPrimaryKey(deviceId);
            //更新关门日志
            LogUtils.updateOPLog(deviceInfo.getScode(), baseResponseVo.getUserId(), openDoorType);
            iCached.remove("cloudClosePicUrl_" + deviceId + "_" + userId);
     ...
     * @param baseResponseVo
     */
    private void closeDoorOvertime(final BaseResponseVo baseResponseVo) {
        logger.info("开始处理关门超时消息");
        ExecutorManager.getInstance().execute(new Runnable() {
            @Override
            public void run() {
                try {
                    String userId = baseResponseVo.getUserId();
                    String deviceId = baseResponseVo.getDeviceId();
                    String closeDoorIden = baseResponseVo.getCloseDoorIden();
                    Thread.sleep(1000 * 60 * 5); // 当前线程休眠5min
                    String tokenTemp = (String) iCached.get(closeDoorIden + "_" + deviceId);
                    //不为空表示已经调用关门盘货方法
                    if (StringUtil.isNotBlank(tokenTemp)) {
                        iCached.remove(closeDoorIden + "_" + deviceId);
                    } else {
                       /* String token = (String) iCached.get("closeDoorOvertime_" + baseResponseVo.getUserId() + "_" + baseResponseVo.getDeviceId());
                        ...
                        logger.debug("关门超时生成异常订单成功：{}", deviceInfo.getScode());
                       /* }*/
                    }
                } catch (Exception e) {
    ...
    private ResponseVo<String> generateExceptionOrder(DeviceInfo deviceInfo, SessionVo sessionVo) {
        try {
            ...
            exceptionOrderDto.setItype(50);//开门超时异常
    ...
    private void handlerActiveInventory(String deviceId, String deviceCode, BaseResponseVo baseResponseVo) {
        ...
        try {
            if (StringUtil.isNotBlank(goodsString) && !goodsString.equals("[]")) {
                ...
                logger.info("设备编号：{},接受到主动盘货商品信息：{},存入Redis中", deviceCode, goodsString);
...
//                        goodModel.setDiscountType();    // 优惠类型
     ...
     * @param userId   用户ID
     * @param infoLog  发送消息类型
     */
    private void sendMsgToAiFace(String deviceId, String userId, String infoLog) {
        String aiId = getAiIdByDeviceId(deviceId, infoLog);
        if (StringUtils.isNotBlank(aiId)) {
            logger.info("设备支持AI设备，设备ID：{}，AI设备ID：{}", deviceId, aiId);
            ChannelHandlerContext faceCtx = faceMap.get(aiId);
            if (null != faceCtx) {
                logger.info("向小屏设备推送消息：{}", infoLog);
                FaceMsgToJsonUtils.SendFaceCommonNoCodeMsgByCtx(deviceId, faceCtx, true, "open door success", userId, FaceType.OPEN_DOOR);
            } else {
                logger.info("售货机设备附带的AI设备不在线，不用向小屏设备推送消息，设备ID：{}，AI设备ID：{}", deviceId, aiId);
            }
        }
    }
    ...
    private String getAiIdByDeviceId(String deviceId, String log) {
        FaceDeviceVo faceDeviceVo = null;
        String aiId = "";
        logger.info("售货机设备ID：{}，消息内容：{}", deviceId, log);
        try {
            faceDeviceVo = (FaceDeviceVo) iCached.get(NettyConst.FACE_DEVICE_VO + deviceId);
            if (null == faceDeviceVo) {
