...
public class ImgCloudService {
     ...
     * @param closeDoorIden   关门唯一标识
     * @return
     */
    @RequestMapping("/cloudCloseDoor")
    @ResponseBody
    public ResponseVo<String> cloudCloseDoor(String deviceId, String key, String userId, Integer openDoorType, String imgResultDetail, String closeDoorIden) {
        String deviceCode = getDeviceCodeByDeviceId(deviceId);
        logger.debug("云识别接口设备关门结算开始，设备编号:{},通信密钥:{}，用户ID:{}，开门类型:{}", deviceCode, key, userId, openDoorType);
        ...
        try {
            ...
            if (10 == openDoorType) {
                try {
                    iCached.put(closeDoorIden + "_" + deviceId, userId);
                } catch (Exception e) {
    ...
    public ResponseVo<String> cloudException(String deviceId, String key, String userId, Integer openDoorType, Integer methodType, String errorCode) {
        String deviceCode = getDeviceCodeByDeviceId(deviceId);
        logger.debug("开关门异常处理，设备编号:{},通信密钥:{}，用户ID:{}，开门类型:{}", deviceCode, key, userId, openDoorType);
        ResponseVo<String> responseVo = ErrorCodeEnum.ERROR_APP_PARAM_EXCEPTION.getResponseVo();
        try {
            String tempDeviceId = (String) iCached.get("cloud_open_door_" + userId);
            if (StringUtil.isBlank(tempDeviceId)) {
    ...
    public ResponseVo<String> cloudOpenDoor(String deviceId, String key, String imgResultDetail) {
        String deviceCode = getDeviceCodeByDeviceId(deviceId);
        logger.debug("云识别接口设备开门记录信息开始，设备编号:{},通信密钥:{}", deviceCode, key);
        ResponseVo<String> responseVo = ErrorCodeEnum.ERROR_APP_PARAM_EXCEPTION.getResponseVo();
        try {
            //校验基础参数
            if (StringUtils.isBlank(deviceId)) {
    ...
    public ResponseVo<String> cloudRealtime(String deviceId, String key, String userId, Integer openDoorType, String imgResultDetail) {
        String deviceCode = getDeviceCodeByDeviceId(deviceId);
        logger.debug("云识别接口设备开门实时记录信息开始，设备编号:{},通信密钥:{}", deviceCode, key);
        ResponseVo<String> responseVo = ErrorCodeEnum.ERROR_APP_PARAM_EXCEPTION.getResponseVo();
        try {
            //校验基础参数
            if (StringUtils.isBlank(deviceId)) {
            ...
            Boolean flag = cloudRecognitionService.isOnLine(deviceId);
            if(flag){
                responseVo.setMsg("设备门已关闭");
                logger.debug("云识别系统设备开门实时计算接口调用失败：{}", responseVo.getMsg());
                return responseVo;
            }
            if (StringUtils.isBlank(key)) {
                responseVo.setMsg("设备通信密钥不能为空");
        ...
        logger.debug("云识别接口设备开门实时计算结束，设备编号：{}", deviceCode);
        return responseVo;
    }
