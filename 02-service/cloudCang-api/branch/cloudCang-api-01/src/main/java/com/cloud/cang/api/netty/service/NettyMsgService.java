        ...
        ExecutorManager.getInstance().execute(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    logger.error("插入开门操作数据出现异常，设备ID：{}，异常原因：{}", deviceId, e.getMessage());
                    return;
                }
            }
        });
...
//                        vrId = commodityInfo.getSvrCommodityId();
//                    }
                    // 插入新数据
...
//                    thirdDeviceSku.setSvrId(vrId);                                  // 本地视觉库视觉ID
...
//                        goodModel.setDiscountType();    // 优惠类型
    ...
    public void assemblyDeviceInfoVo(String deviceId, DeviceInfo deviceInfo) {
        try {
            ...
            logger.error("设备ID：{} 从redis中获取或者存入数据异常：{}", deviceId, e.getMessage());
        }
    }
