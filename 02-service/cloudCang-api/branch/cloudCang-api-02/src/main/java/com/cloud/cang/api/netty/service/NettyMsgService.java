...
import com.cloud.cang.core.common.utils.*;
            ...
            try {
                ...
                if (responseVo.isSuccess()) {
                    ...
                    if (inventoryResult.getItype().intValue() == 10 || inventoryResult.getItype().intValue() == 20) { //购物关门
                        ...
                        // 给设备推送订单
                        asynSendShoppingOrderByCtxMap(deviceId, userId, responseVo.getData().getItype(), orderRecords, ctxMap);
...
//            sessionVo = (SessionVo) iCached.get(IPUtils.getRemoteAddress(socketIOClient));
    ...
    public void handlerMessage(BaseResponseVo baseResponseVo) {
        ...
        if (BooleanUtils.isTrue(success)) {
            ...
            } else if (TypeConstant.VOLUME.equals(methodType) && Integer.valueOf(200).equals(code)) {
                logger.debug("设备:{}发送设备音量大小调节成功消息到netty服务器", deviceId);
                adjustVolumeResult(baseResponseVo);
            ...
            if (TypeConstant.QR_CODE.equals(methodType)) {   // 设备请求发送二维码
                ...
                upgradeVoiceResult(baseResponseVo);
            } else if (TypeConstant.VOLUME.equals(methodType)) {
                logger.debug("设备:{}发送音量调节失败消息到netty服务器", deviceId);
                adjustVolumeResult(baseResponseVo);
            }
            return;
        }
     ...
     * @param baseResponseVo
     */
    private void adjustVolumeResult(BaseResponseVo baseResponseVo) {
        ExecutorManager.getInstance().execute(new Runnable() {
            @Override
            public void run() {
                logger.debug("开始处理音量大小调节结果，设备ID:{}", baseResponseVo.getDeviceId());
                String deviceId = baseResponseVo.getDeviceId();
                ...
                try {
                    ...
                    logger.error("处理售货机设备:{}发送过来的音量大小调节结果，json转换出现异常:{}", deviceId, e);
                    return;
                }

                if (null != upgradeResultModel) {
                    String updateId = upgradeResultModel.getUpgradeId();        // 升级明细表ID
                    String startTime = upgradeResultModel.getStartTime();       // 升级开始时间
                    String endTime = upgradeResultModel.getEndTime();           // 升级结束时间
                    String success = upgradeResultModel.getSuccess();           // 升级成功/失败
                    String version = upgradeResultModel.getVersion();           // 升级版本号
                    String reason = upgradeResultModel.getReason();             // 升级失败原因
                    logger.debug("开始向数据库中添加记录音量大小调节结果，明细表ID:{}，开始升级时间：{}，升级结束时间：{}，结果：{}，设备ID：{}", updateId, startTime, endTime, success.equals("1") ? "成功" : "失败", deviceId);
                    if (StringUtils.isNotBlank(updateId) && StringUtils.isNotBlank(startTime) && StringUtils.isNotBlank(endTime)
                        ...
                        if ("1".equals(success)) {
                            ...
                            logger.debug("音量大小调节结果:成功，设备ID：{}", upgradeResultModel.getUpgradeId());
                        } else if ("0".equals(success)) {
                            deviceUpgradeDetails.setIstatus(30);
                            deviceUpgradeDetails.setSexceptionDesc(reason);                                     //  升级失败原因
                            logger.debug("音量大小调节结果：失败，原因:{}，设备ID：{}", reason, upgradeResultModel.getUpgradeId());
                        }
                        ...
                        logger.debug("记录音量大小调节结果完成，明细表ID:{}，设备ID：{}", upgradeResultModel.getUpgradeId());
                    }
                }
            }
        });
...
//                    List<ReplenishRealTimeCommodityResult> results = replenishRealTimeOrderResult.getResults();
    ...
    private List<CommodityVo> assemblyCommodityVo(List<CommodityVo> commodityVoList, List<TagModel> goodsList) {
        ...
        for (TagModel tagModel : goodsList) {
            ...
            commodityVo.setCommodityNum(commodityVo.getCommodityNum() + tagModel.getNumber());
...
//    }
     ...
     * @param ctxMap
     */
    private void asynSendShoppingOrderByCtxMap(String deviceId, String userId, Integer itype, List<CreatOrderResult> orderRecords, ConcurrentMap<String, ChannelHandlerContext> ctxMap) {
