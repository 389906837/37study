...
public class CloudRecognitionService {
     ...
     * @return
     */
    public ResponseVo<String> closeDoorSettle(String deviceId, String key, String userId, Integer openDoorType, List<ImageDetail> imgDetail, HttpServletRequest request) {
        ...
        response = OpenSdkApi.recognition(deviceId, IdGenerator.randomUUID(), "关门时记录商品信息", imgDetail);
     ...
     * @param openDoorImgDetails 开门图片
     * @param request
     * @return
     */
    public ResponseVo<String> realtimeOrder(String deviceId, String key, String userId, Integer openDoorType, List<ImageDetail> openDoorImgDetails, HttpServletRequest request) {
        ...
        List<ImgResultDetail> imgResultDetailList = new ArrayList<>();
        Map<String, Integer> goodMapTemp = new HashMap<>();  // 临时存放从云端返回的图片中商品集合
        response = OpenSdkApi.recognition(deviceId, IdGenerator.randomUUID(), "开门实时盘货商品信息", openDoorImgDetails);
        if (BooleanUtils.isTrue(response.isSuccess())) {
            imgResultDetailList = response.getImgResultDetail();
            if (CollectionUtils.isNotEmpty(imgResultDetailList)) {
                for (ImgResultDetail img : imgResultDetailList) {   // 遍历所有图片
