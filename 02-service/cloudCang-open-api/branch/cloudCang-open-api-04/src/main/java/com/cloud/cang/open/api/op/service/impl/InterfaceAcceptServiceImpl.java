...
public class InterfaceAcceptServiceImpl extends GenericServiceImpl<InterfaceAccept, String> implements
     ...
     */
    @Override
    public void syncAddInterfaceAcceptRecord(String batchNo, Map<String, String> params, AppManage app, InterfaceInfo interfaceInfo, String bizContent, String clientIp) {
        ExecutorManager.getInstance().execute(new Runnable() {
            @Override
            public void run() {
                logger.info("新增接口业务受理信息，业务编号：{}", batchNo);
                InterfaceAccept accept = new InterfaceAccept();
                accept.setScode(batchNo);
                accept.setSinterfaceCode(interfaceInfo.getScode());
                accept.setSappId(app.getSappId());
                accept.setSappCode(app.getScode());
                accept.setSuserCode(app.getSuserCode());
                accept.setSuserId(app.getSuserId());
                accept.setSrequestData(bizContent);
                accept.setSoperIp(clientIp);

                if (StringUtil.isNotBlank(params.get("timestamp"))) {
                    DateFormat df = new SimpleDateFormat("yyyyMMddHHmmss");
                    df.setTimeZone(TimeZone.getTimeZone("GMT+8"));
                    try {
                        accept.setSrequestTime(df.parse(params.get("timestamp")));
                ...
                if (StringUtil.isNotBlank(params.get("notifyUrl")) || interfaceInfo.getItype().intValue()
                ...
                try {
                    iCached.put(batchNo + app.getSappId() + "_interface_accept", accept, 2 * 60 * 60);
                } catch (Exception e) {
     ...
     */
    @Override
    public void updateInterfaceAcceptByFailed(String batchNo, AppManage app, InterfaceInfo interfaceInfo, SubCodeEnum subCodeEnum, Map<String, String> params) {
        ExecutorManager.getInstance().execute(new Runnable() {
            @Override
            public void run() {
                ...
                try {
                    ...
                    if (null != accept) {
                        ...
                        if (null != interfaceInfo.getItype() && interfaceInfo.getItype().intValue() != BizTypeDefinitionEnum.InterfacType.ASYNCHRONOUS.getCode()) {
                            ...
                            tempMap.put("methodName", EncryptUtils.decryptStringUnZip(params.get("methodName")));
                            //1、参数签名字段
     ...
     */
    @Override
    public void updateInterfaceAcceptBySuccess(String outBatchNo, String requestData, Integer imgSize, Integer successNum, String batchNo, String backBody, String encryptBackBody, String callbackBody, String encryptCallbackBody, AppManage app, Map<String, String> params) {
        ExecutorManager.getInstance().execute(new Runnable() {
            @Override
            public void run() {
                ...
                try {
                    ...
                    if (null != accept) {
                        ...
                        updateAccept.setTrequestFinishTime(new Date());
                        updateAccept.setIisDealwith(1);
                        updateAccept.setScallbackAddress(params.get("notifyUrl"));
                        updateAccept.setUpdateTime(new Date());
     ...
     * @throws Exception
     */
    private InterfaceAccept getInterfaceAccept(String batchNo, String sappId) throws Exception {
        InterfaceAccept accept = (InterfaceAccept) iCached.get(batchNo + sappId + "_interface_accept");
        if (null == accept) {
            Map<String, Object> paramMap = new HashMap<String, Object>();
