 ...
 **/
@Component
public class ImgResultSynchronizeMessageHandler implements RmqMessageHandler<Mq_ImgResult> {
    private static final Logger logger = LoggerFactory.getLogger(ImgResultSynchronizeMessageHandler.class);
    @Autowired
    ...
    public HandlerEnum handler(RmqMessage<Mq_ImgResult> rmqMessage) {
        ...
        // 业务完成之后删除缓存
        try {
            iCached.remove(com.cloud.cang.openapi.APIConstant.RedisKey.IMG_RECOGNITIONVO_SYNCHRONIZE + imgResult.getBatchNo());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public boolean matchHandler(String queueName) {
        if(queueName.contains(QueueEnum.IMG_RESULT_SYNCHRONIZE.getName())) {
            return true;
        }
        return false;
    }
        ...
        try {
            ...
            transferLogService.addTransferLog(
                    ...
                    StrFormatter.format("图片视觉识别接口调用成功，业务编号：{}, 成功识别图片张数：{}",
                            imgResult.getBatchNo(),
                            imgResult.getMsg().size()));
            logger.info("===========第三方接口同步识别结果处理成功==============");
            iCached.put(com.cloud.cang.openapi.APIConstant.RedisKey.IMG_RECOGNITIONVO_THRID_SYNCHRONIZE_RESULT + recongitionVo.getBatchNo(), encryptBackBody,60*60*2);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private RecongitionVo<ImgRecognition> getRecongitionVo(Mq_ImgResult imgResult) {
        try {
            Object object = iCached.get(com.cloud.cang.openapi.APIConstant.RedisKey.IMG_RECOGNITIONVO_SYNCHRONIZE + imgResult.getBatchNo());
            if (object != null) {
    ...
    private String buildBackBody(Mq_ImgResult imgResult,RecongitionVo<ImgRecognition> recongitionVo) throws Exception {
        ...
        tempMap.put("cloud_api_recognition_bitmain_sync_response", signContentTemp);
        ...
        imgResult.getMsg().forEach(o->{
            ...
            if(CollUtil.isNotEmpty(o.getGoods())){
                o.getGoods().forEach(goods -> {
                    ...
                    // 2020/3/12 qzg 带坐标
                    goodDetail.setProb(goods.getProb());
                    goodDetail.setPosx(goods.getPosx());
                    goodDetail.setPosy(goods.getPosy());
                    goodDetail.setPosh(goods.getPosh());
                    goodDetail.setPosw(goods.getPosw());
