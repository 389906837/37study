...
public class BaseService {
    ...
    public RecongitionVo verifyCommonParam(String batchNo, HttpServletRequest request, HttpServletResponse response) throws ServiceException, Exception {
        ...
        iCached.put("platform_key_" + batchNo, app.getSplatformKey(), 5 * 60);
        iCached.put("app_manage_" + batchNo, app, 5 * 60);

        String bizContent = BaseEncrypt.decryptContent(commonParam.getBizContent(),
                commonParam.getEncryptType(),
                app.getSappSecretKey(), charset);

        //解密数据放入CommonParam
        commonParam.setBizContent(bizContent);
        iCached.put("common_param_" + batchNo, commonParam, 5 * 60);

        if (APIConstant.Recognition.RECOGNITION.equals(methodName)
                || APIConstant.Recognition.RECOGNITION_SYNCHRONIZE.equals(methodName)
                || APIConstant.Recognition.RECOGNITION_CALLBACK.equals(methodName)
                || APIConstant.Recognition.RECOGNITION_YOLOV4_CALLBACK.equals(methodName)) {
        ...
        iCached.put("interface_info_" + batchNo, interfaceInfo, 5 * 60);
     ...
     * @param response @throws Exception
     */
    public void realTimeBackcallError(String batchNo, SubCodeEnum codeEnum, HttpServletResponse response) {
        try {
            String platformPrivateKey = (String) iCached.get("platform_key_" + batchNo);
            CommonParam commonParam = (CommonParam) iCached.get("common_param_" + batchNo);
            if (StringUtil.isBlank(platformPrivateKey)) {
                response.getWriter().write("failure");
                return;
            }
    ...
    public void failOperation(String batchNo, String clientIp, SubCodeEnum subCodeEnum) {
        try {
            CommonParam commonParam = (CommonParam) iCached.get("common_param_" + batchNo);
            AppManage app = (AppManage) iCached.get("app_manage_" + batchNo);
            InterfaceInfo interfaceInfo = (InterfaceInfo) iCached.get("interface_info_" + batchNo);
            if (null != app && null != interfaceInfo && null != commonParam) {
                iCached.remove("common_param_" + batchNo);
                iCached.remove("app_manage_" + batchNo);
                iCached.remove("interface_info_" + batchNo);
                //新增接口调用错误日志
                String operType = "";
                String methodName = commonParam.getMethodName();
                switch (methodName) {
                    case APIConstant.Recognition.RECOGNITION:
                    case APIConstant.Recognition.RECOGNITION_SYNCHRONIZE:
                    case APIConstant.Recognition.RECOGNITION_CALLBACK:
                ...
                iCached.remove(batchNo + app.getSappId() + APIConstant.RedisKey.INTERFACE_ACCEPT);
