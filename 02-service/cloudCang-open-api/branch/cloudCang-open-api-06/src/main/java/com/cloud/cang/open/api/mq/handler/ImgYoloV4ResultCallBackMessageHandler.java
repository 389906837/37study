 ...
 **/
@Component
public class ImgYoloV4ResultCallBackMessageHandler implements RmqMessageHandler<Mq_ImgResult> {
    private static final Logger logger = LoggerFactory.getLogger(ImgYoloV4ResultCallBackMessageHandler.class);
    @Autowired
    ...
    public HandlerEnum handler(RmqMessage<Mq_ImgResult> rmqMessage) {
        ...
        // 业务完成之后删除缓存
        try {
            iCached.remove(com.cloud.cang.openapi.APIConstant.RedisKey.IMG_RECOGNITIONVO_YOLOV4_CALLBACK + imgResult.getBatchNo());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public boolean matchHandler(String queueName) {
        if (queueName.contains(QueueEnum.IMG_RESULT_YOLOV4_CALLBACK.getName())) {
            return true;
        }
        return false;
    }
    ...
    private RecongitionVo<ImgRecognition> getRecongitionVo(Mq_ImgResult imgResult) {
        try {
            Object object = iCached.get(com.cloud.cang.openapi.APIConstant.RedisKey.IMG_RECOGNITIONVO_YOLOV4_CALLBACK + imgResult.getBatchNo());
            if (object != null) {
