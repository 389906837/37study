...
public class EyeOpenGatewayService extends BaseService {
    ...
    public void gateway(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            // 校验请求参数
            recongitionVo = this.verifyCommonParam(request, response);
            if (null == recongitionVo){//老版请求跳转到老版服务
                return;
            }
            // 执行业务处理
            this.processBiz(recongitionVo,response);
    ...
    private  void processBiz(RecongitionVo recongitionVo, HttpServletResponse response) throws ServiceException,Exception {
            ...
            if (flag) {
                ...
                ExecutorManager.getInstance().execute(new Runnable() {
                    ...
                    public void run() {
                        ...
                        do {
                            try {
                                encryptBackBody = (String) iCached.get(com.cloud.cang.openapi.APIConstant.RedisKey.IMG_RECOGNITIONVO_THRID_SYNCHRONIZE_RESULT + recongitionVo.getBatchNo());
                                if (StringUtil.isNotBlank(encryptBackBody) || time >= timeOut) {
                                    flag = false;
                                    //删除同步方法识别结果
                                    iCached.remove(com.cloud.cang.openapi.APIConstant.RedisKey.IMG_RECOGNITIONVO_THRID_SYNCHRONIZE_RESULT + recongitionVo.getBatchNo());
                                    continue;
                                }
                                time++;
                                Thread.sleep(200l);
