...
public class PayApplyServiceImpl extends GenericServiceImpl<PayApply, String> implements
     ...
     */
    @Override
    public ResponseVo<CreatOrderResult> deductionPoint(DeductionPointDto deductionPointDto) throws Exception {
        //    ResponseVo<PayBackDto> responseVo = ResponseVo.getSuccessResponse();

        //循环订单信息
        OrderRecord orderRecord = orderRecordService.selectByPrimaryKey(deductionPointDto.getOrderId());
        if (orderRecord == null) {
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("订单信息不能为空");
        }/* else if (!orderRecord.getSmemberId().equals(payApplyDto.getSmemberId())) {
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("订单异常");
        ...
        //新增订单付款信息
        OrderPay orderPay = orderPayService.insertOrderPay(orderRecord, 30, 0);
        //新增付款申请
        PayApply payApply = orderRecordService.insertPayApply(orderRecord, deductionPointDto.getSremark());
        logger.info("付款申请插入成功：{}", payApply);

        //调用积分变更服务
        //调用第三方变更积分接口
        String error_msg = "调用第三方扣除积分服务失败";
        ...
        if (null == parameterGroupDetail || StringUtil.isBlank(parameterGroupDetail.getSvalue()) || StringUtil.isBlank(parameterGroupDetail.getSremark())) {
            ...
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("调用第三方接口用户不存在");
        }
        String key = parameterGroupDetail.getSvalue();
        String pwd = parameterGroupDetail.getSremark();
        Map<String, Object> temp = new HashMap();
        temp.put("mobile", deductionPointDto.getMobile());
        temp.put("appKey", key);
        BigDecimal pointTemp = orderRecord.getFactualPayAmount().multiply(new BigDecimal(100));
        Integer point = (int) pointTemp.negate().doubleValue();
        temp.put("point", point);
        String token = DigestUtils.md5Hex(key + pwd + deductionPointDto.getMobile() + point);
        temp.put("token", token);
        String jsonStr = HttpClientUtils.sendPost(url, temp);
        if (StringUtil.isBlank(jsonStr)) {
            logger.info("调用第三方扣除积分接口返回异常：{}", orderRecord.getSorderCode());
            return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("调用第三方扣除积分接口返回异常");
        }
        ...
        if (null != status && status == 200) {
            logger.info("调用第三方扣除积分接口成功：{}", orderRecord.getSorderCode());
            //调用第三方接口成功 修改订单支付方式 抵扣积分
            Map<String, Object> map = new HashMap<>();
            map.put("sorderId", orderRecord.getId());
            List<OrderCommodity> orderCommodityList = orderCommodityService.selectByMapWhere(map);
            return changePointUpOrder(orderRecord, orderCommodityList, payApply.getId(), orderPay.getId(), (int) pointTemp.doubleValue());
        } else {
            logger.info("调用第三方扣除积分接口失败：{}", orderRecord.getSorderCode());
            changePointFailUpOrder(orderRecord, payApply.getId(), orderPay.getId(), String.valueOf(status), ErrorCodeEnum.getNameByCode(status));
        }
        error_msg = error_msg + ErrorCodeEnum.getNameByCode(status);
        return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo(error_msg);
    }
     ...
     */
    private ResponseVo<CreatOrderResult> changePointUpOrder(OrderRecord orderRecord, List<OrderCommodity> orderCommodities, String payApplyId, String orderPayId, Integer point) {
        OrderRecord temp = new OrderRecord();
        temp.setId(orderRecord.getId());
        if (orderRecord.getIpoint() == null) {
            orderRecord.setIpoint(BigDecimal.ZERO);
        }
        temp.setIpoint(orderRecord.getIpoint().add(new BigDecimal(point)));
        if (orderRecord.getFpointDiscountAmount() == null) {
            orderRecord.setFpointDiscountAmount(BigDecimal.ZERO);
        }
        temp.setFpointDiscountAmount(orderRecord.getFpointDiscountAmount().add(orderRecord.getFactualPayAmount()));
        if (orderRecord.getFdiscountAmount() == null) {
            orderRecord.setFdiscountAmount(BigDecimal.ZERO);
        }
        temp.setFdiscountAmount(orderRecord.getFdiscountAmount().add(orderRecord.getFactualPayAmount()));
        temp.setFactualPayAmount(BigDecimal.ZERO);
        temp.setIstatus(10);
        temp.setTpayCompleteTime(DateUtils.getCurrentDateTime());
        temp.setIpayType(BizTypeDefinitionEnum.PayType.PAY_POINT.getCode());
        temp.setSpayApplyId(payApplyId);
        temp.setIpayWay(BizTypeDefinitionEnum.PayWay.POINT_ALL_DEDUCTION.getCode());
        orderRecordService.updateBySelective(temp);
        List<OrderCommodity> orderCommodityArrayList = new ArrayList<>();
        for (OrderCommodity orderCommodity : orderCommodities) {
            OrderCommodity orderCommodityTemp = new OrderCommodity();
            orderCommodityTemp.setId(orderCommodity.getId());
            if (orderCommodity.getIpoint() == null) {
                orderCommodity.setIpoint(BigDecimal.ZERO);
            }
            orderCommodityTemp.setIpoint(orderCommodity.getIpoint().add(orderCommodity.getFactualAmount().multiply(new BigDecimal(100))));
            if (orderCommodity.getFpointDiscountAmount() == null) {
                orderCommodity.setFpointDiscountAmount(BigDecimal.ZERO);
            }
            orderCommodityTemp.setFpointDiscountAmount(orderCommodity.getFpointDiscountAmount().add(orderCommodity.getFactualAmount()));
            if (orderCommodity.getFdiscountAmount() == null) {
                orderCommodity.setFdiscountAmount(BigDecimal.ZERO);
            }
            orderCommodityTemp.setFactualAmount(BigDecimal.ZERO);
            orderCommodityTemp.setFdiscountAmount(orderCommodity.getFdiscountAmount().add(orderCommodity.getFactualAmount()));
            orderCommodityService.updateBySelective(orderCommodityTemp);
            OrderCommodity orderCommodityReturn = orderCommodityService.selectByPrimaryKey(orderCommodity.getId());
            orderCommodityArrayList.add(orderCommodityReturn);
        }
        //更新付款申请
        PayApply payApply = new PayApply();
        payApply.setId(payApplyId);
        payApply.setIstatus(20);
        payApply.setTfinishDatetime(DateUtils.getCurrentDateTime());
        this.updateBySelective(payApply);
        //更新订单付款数据
        OrderPay orderPay = new OrderPay();
        orderPay.setId(orderPayId);
        orderPay.setIstatus(20);
        orderPay.setTcompleteTime(DateUtils.getCurrentDateTime());
        orderPayService.updateBySelective(orderPay);
        logger.info("订单积分抵扣成功处理完成");
        CreatOrderResult creatOrderResult = new CreatOrderResult();
        OrderRecord reOrder = orderRecordService.selectByPrimaryKey(orderRecord.getId());
        creatOrderResult.setOrderRecord(reOrder);
        creatOrderResult.setOrderCommodityList(orderCommodityArrayList);
        return ResponseVo.getSuccessResponse(creatOrderResult);
    }
     ...
     * @param
     */
    private ResponseVo changePointFailUpOrder(OrderRecord orderRecord, String payApplyId, String orderPayId, String errorCode, String errorDesc) {
        OrderRecord temp = new OrderRecord();
        temp.setId(orderRecord.getId());
        temp.setIstatus(20);
        //   temp.setIpayType(BizTypeDefinitionEnum.PayType.PAY_POINT.getCode());
        ...
        if (StringUtil.isNotBlank(orderRecord.getSpayFailureReason())) {
            temp.setSpayFailureReason(orderRecord.getSpayFailureReason() + ",{" + errorCode + ":" + errorDesc + "}");
        } else {
            temp.setSpayFailureReason("{" + errorCode + ":" + errorDesc + "}");
        }
        temp.setSpayApplyId(payApplyId);
        orderRecordService.updateBySelective(temp);
        //更新付款申请数据
        ...
        updateApply.setTcloseTime(DateUtils.getCurrentDateTime());
        updateApply.setId(payApplyId);
        updateApply.setIstatus(30);
        updateApply.setTfinishDatetime(DateUtils.getCurrentDateTime());
        this.updateBySelective(updateApply);
        //更新订单付款数据
        OrderPay orderPay = new OrderPay();
        orderPay.setId(orderPayId);
        orderPay.setIstatus(30);
        orderPay.setTcompleteTime(DateUtils.getCurrentDateTime());
        orderPay.setSerrorCode(errorCode);
        orderPay.setSerrorDesc(errorDesc);
        orderPay.setTcompleteTime(DateUtils.getCurrentDateTime());
        orderPayService.updateBySelective(orderPay);
        logger.info("订单积分抵扣失败处理完成");
    ...
    public ResponseVo<HashMap<String, Object>> payWechatRepairProcess(RepairProcessDto repairProcessDto) throws Exception {
        ...
        if (repairProcessDto.getItype().intValue() == 10 && (orderRecord.getIchargebackWay().intValue() == 10 || orderRecord.getIchargebackWay().intValue() == 30 || orderRecord.getIchargebackWay() == 40)) {
            ...
            if (orderRecord.getIchargebackWay().intValue() == 10 || orderRecord.getIchargebackWay().intValue() == 30) {
            ...
            } else if (orderRecord.getIchargebackWay() == 40) {
