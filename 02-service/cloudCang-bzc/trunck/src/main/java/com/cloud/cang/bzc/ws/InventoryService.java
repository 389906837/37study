...
public class InventoryService {
     ...
     * 验证底层盘点服务 参数
     *
     * @param layeredInventoryDto 盘点参数
     * @return
     */
    private ResponseVo<Integer> validateParamLayered(LayeredInventoryDto layeredInventoryDto) {
        logger.info("底层盘点服务校验参数开始.....参数：{}", layeredInventoryDto);
        if (StringUtils.isBlank(layeredInventoryDto.getDeviceId())) {
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("盘点设备信息不能为空");
        }
        if (null == layeredInventoryDto.getInventoryType()) {
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("盘点类型不能为空");
        }
        if (layeredInventoryDto.getInventoryType().intValue() == 10 && null == layeredInventoryDto.getCloseType()) {
            return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo("盘点关门类型不能为空");
        }
    ...
    private ResponseVo<Integer> validateParam(InventoryDiffDto inventoryDiffDto) {
        logger.info("底层盘点服务校验参数开始.....参数：{}", JSON.toJSONString(inventoryDiffDto));
     ...
     */
    @SuppressWarnings("unchecked")
    @RequestMapping(value = "/localGravityCloseService", method = RequestMethod.POST)
    public ResponseVo<InventoryResult> localGravityCloseService(@RequestBody InventoryDto inventoryDto) {
        logger.debug("重力视觉柜关门生成订单...");
        try {
            // 校验基础参数
            ResponseVo<Integer> validateResult = validateParam(inventoryDto);
            if (!validateResult.isSuccess()) {
            ...
            // 根据商品数量差生成订单
            ResponseVo<InventoryResult> responseVo = deviceStockService.closeGravityCommodityDeal(inventoryDto);
            if (!responseVo.isSuccess()) {
                logger.info("根据商品数量差生成订单服务失败：{}", JSON.toJSONString(responseVo));
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(responseVo.getMsg());
            }
     ...
     */
    @SuppressWarnings("unchecked")
    @RequestMapping(value = "/localGravityLayeredCloseService", method = RequestMethod.POST)
    public ResponseVo<InventoryResult> localGravityLayeredCloseService(@RequestBody LayeredInventoryDto layeredInventoryDto) {
        logger.debug("重力视觉柜关门生成订单...");
        try {
            // 校验基础参数
            ResponseVo<Integer> validateResult = validateParamLayered(layeredInventoryDto);
            if (!validateResult.isSuccess()) {
                logger.info("根据商品数量差生成订单参数校验失败：{}", validateResult.getMsg());
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(validateResult.getMsg());
            }
            // 根据商品数量差生成订单
            ResponseVo<InventoryResult> responseVo = deviceStockService.closeGravityLayeredCommodityDeal(layeredInventoryDto);
            if (!responseVo.isSuccess()) {
                logger.info("根据商品数量差生成订单服务失败：{}", JSON.toJSONString(responseVo));
                return ErrorCodeEnum.ERROR_COMMON_PARAM.getResponseVo(responseVo.getMsg());
            }
