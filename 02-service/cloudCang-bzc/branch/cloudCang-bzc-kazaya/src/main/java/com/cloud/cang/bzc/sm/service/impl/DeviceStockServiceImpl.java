...
public class DeviceStockServiceImpl extends GenericServiceImpl<DeviceStock, String> implements
    ...
    public ResponseVo<InventoryResult> dealwithInventory(InventoryDto inventoryDto) throws Exception {
        ...
        List<CommodityDiffVo> addVoList = null;
        List<CommodityDiffVo> subVoList = null;
        if (deviceInfo.getItype().intValue() == BizTypeDefinitionEnum.DeviceType.VISION.getCode()){
            ...
            map = this.calcRFIDCommodityDiffByType(inventoryDto,deviceInfo, memberInfo);
        }
        addVoList = (List<CommodityDiffVo>) map.get("addVoList");
        subVoList = (List<CommodityDiffVo>) map.get("subVoList");
                ...
                if (null != subVoList && subVoList.size() > 0) {
                    ...
                    if (null != resVO) {
                        ...
                        if (resVO.isSuccess()) {
                            ...
                            if (orderRecord.getIchargebackWay().intValue() == 10) {
                                result.setItype(10);
                            } else {
                                result.setItype(20);
                            }
     ...
     * 组装rfid商品差
     *
     * @param lablesType   标签集合类型  10：新增标签集合  20：减少标签集合
     * @param commodityVo 商品信息
     * @param deviceInfo  设备信息
     */
    private CommodityDiffVo assemblyRFID(Integer lablesType,CommodityVo commodityVo, DeviceInfo deviceInfo) {
        CommodityDiffVo tempDiffVo = new CommodityDiffVo();
        tempDiffVo.setSvrCode(commodityVo.getSvrCode());
        CommodityInfo commodityInfo = commodityInfoService.selectByScode(commodityVo.getCommodityCode(), deviceInfo.getSmerchantId());
        if (null != commodityInfo) {
            tempDiffVo.setScommodityCode(commodityInfo.getScode());
            tempDiffVo.setScommodityId(commodityInfo.getId());
            tempDiffVo.setScommodityFullName(commodityInfo.getSbrandName() + commodityInfo.getSname() + commodityInfo.getStaste() + commodityInfo.getIspecWeight() + commodityInfo.getSspecUnit() + "/" + commodityInfo.getSpackageUnit());
        ...
        int stockTemp = 0;
        DeviceStock deviceStock = this.selectDeviceStockByCommodityId(deviceInfo.getId(), commodityInfo.getId());
        if(deviceStock!=null && deviceStock.getIstock()!=null && deviceStock.getIstock().intValue() >0){
            stockTemp = deviceStock.getIstock().intValue();
        }
        // 10：新增标签集合
        if (lablesType == 10) {
            tempDiffVo.setItype(10);
            tempDiffVo.setNumber(commodityVo.getCommodityNum());
            tempDiffVo.setFcommodityPrice(commodityInfo.getFsalePrice());
            tempDiffVo.setCurrStock(stockTemp);
        }
        // 20：减少标签集合
        if (lablesType == 20) {
            tempDiffVo.setItype(20);
    ...
    /**
     * 计算设备库存商品差
     * @param inventoryDto 盘点商品集合
     * @param deviceInfo   设备数据
     * @param memberInfo   会员数据
     */
    @Override
    public Map<String, Object> calcRFIDCommodityDiffByType(InventoryDto inventoryDto, DeviceInfo deviceInfo, MemberInfo memberInfo) {
        // 缺少rfid标签
        Set<String> lossLables = inventoryDto.getLossLables();
        // 新增rfid标签
        Set<String> newLables = inventoryDto.getNewLables();
        Map<String, Object> map = new HashMap<String, Object>();
        List<CommodityDiffVo> subVoList = new ArrayList<CommodityDiffVo>();
        List<CommodityDiffVo> addVoList = new ArrayList<CommodityDiffVo>();
        if(CollectionUtils.isNotEmpty(lossLables)){
            List<CommodityVo> lossCommoditys = commodityRfidService.selectCommodityVoGruopByCommodityCode(lossLables);
            Integer diffNum = lossCommoditys.size();
            for (CommodityVo lossCommodity : lossCommoditys) {
                CommodityDiffVo diffVo = assemblyRFID(20,lossCommodity,deviceInfo);
                diffVo.setRfidList(lossCommodity.getLables());
                subVoList.add(diffVo);
            }
        }
        if(CollectionUtils.isNotEmpty(newLables)){
            List<CommodityVo> newCommoditys = commodityRfidService.selectCommodityVoGruopByCommodityCode(newLables);
            for (CommodityVo newCommodity : newCommoditys) {
                CommodityDiffVo diffVo = assemblyRFID(10,newCommodity,deviceInfo);
                diffVo.setRfidList(newCommodity.getLables());
                addVoList.add(diffVo);
            }
        }
        map.put("subVoList", subVoList);
        map.put("addVoList", addVoList);
        ...
        if (subTemp.abs().compareTo(ieleTotal) <= 0) {  //库存重力等于盘货重力    -->重力不变
            ...
            if (subResult.subtract(subTotal).abs().compareTo(floatResult.add(iele)) <= 0) {
                ...
                if (null != subVoList & !subVoList.isEmpty()) {
                    ...
                    //return layeredGeOrder(iele, deviceInfo, memberInfo, subVoList, layeredInventoryDto, subResult, subTotal, 10, floatResult, visionList);
                    ...
                    // return layeredGeOrder(iele, deviceInfo, memberInfo, subVoList, layeredInventoryDto, subResult, null, 20, null, visionList);
    ...
    private ResponseVo<InventoryResult> layeredGeOrder2(BigDecimal iele, DeviceInfo deviceInfo, MemberInfo
        ...
        if (10 == type && !flagTemp) {
            ...
            logger.info("重量减少大于视觉减少,根据重力视觉差匹配商品,匹配单个商品：{}", diffData);
            ...
            diffStocksTemp = this.selectGravityStockByDeviceId(deviceInfo.getId(), subResult.add(iele));
            stockVos = new ArrayList<>();
            if (diffStocksTemp != null && !diffStocksTemp.isEmpty()) {
                for (StockVo stockVo : diffStocksTemp) {
                    if (subResult.subtract(stockVo.getIweigth()).abs().compareTo(iele.add(stockVo.getIcommodityFloat())) <= 0) {
                        stockVos.add(stockVo);
                    }
                }
            }
          ...
          /*      Collections.sort(stockVos, new Comparator<StockVo>() {
                ...
                logger.info("重量减少大于视觉减少,根据重力视觉差匹配商品,匹配单个商品,可以匹配出商品：{}", stockVo);
                return;*/
                logger.info("重量减少大于视觉减少,根据重力视觉差匹配商品,匹配单个商品,可以匹配出多个结果：{}", stockVos);
            }
           ...
           /*     Collections.sort(stockVos, new Comparator<StockVo>() {
                ...
                subVoList = getSubList(stockVo, deviceInfo, flagTemp,
                        subVoList, new BigDecimal(1));
                logger.info("重量减少小于视觉减少,根据重力匹配,匹配单个商品,匹配出商品：{}", stockVo);
                return geOrder(deviceInfo, memberInfo, subVoList, inventoryDto);*/
                logger.info("重量减少大于视觉减少,根据重力视觉差匹配商品,匹配单个商品,可以匹配出多个商品：{}", stockVos.size());
               /* generateExceptionOrder(deviceInfo, memberInfo, subVoList, inventoryDto, 40);
                return ErrorCodeEnum.ERROR_COMMON_SYSTEM.getResponseVo("创建订单失败,生成异常订单");*/
    ...
    private List<ContrastGoodsVo> calcLayeredCommodityDiffData(List<LayeredGoodsVo> realGoods, List<LayeredGoodsVo> openGoods, DeviceInfo deviceInfo) {
        logger.info("重力视觉柜计算开关门商品差异：{}", deviceInfo.getScode());
        // 遍历云端返回response
        logger.info("重力视觉柜计算开关门对比差异，关门商品数据,{}", realGoods);
        // 开关门前后都有商品，计算增加与减少的商品信息
