...
public class OrderRecordServiceImpl extends GenericServiceImpl<OrderRecord, String> implements
    ...
    public /*ResponseVo<List<CreatOrderResult>>*/  Map generateOrder(OrderCommDto orderCommDto) throws Exception {
        ...
        //计算订单优惠，订单详情Dto
        List<CommodityDiscountDto> commodityDiscountDtoList = null;
        try {
            commodityDiscountDtoList = assembleOrderDiscount(orderCommDto);
        }catch (Exception e){
            logger.error("组装订单优惠参数异常:{}",e);
        }
        //订单表Dto
        ...
        }else if(BizTypeDefinitionEnum.ClientType.VENDSTOP.getCode() == isourceClientType){
            responseVo = insertOrder(orderDto, commodityDiscountDtoList, BizTypeDefinitionEnum.ClientType.VENDSTOP.getCode());
        }
        Map map = new HashMap();
       ...
       /* 查看商户客户端配置支持支付类型
        ...
        if (30 == payWay || (10 == payWay && 0 == isOpen)) {
            ...
            //如果订单总价小于等于限制金额，则直接生成订单
                if (10 == merchantConfType && (null != merchantConfTemp.getIwechatWithholdType() && 20 == merchantConfTemp.getIwechatWithholdType())) {
                    orderDto.setIchargebackWay(BizTypeDefinitionEnum.ChargebackWay.WECHAT_PAY_POINT_WITHHOLDING.getCode());
                } else {
    ...
    private ResponseVo<OrderRecord> insertOrderRecord(OrderRecord orderRecord, OrderDto orderDto, Integer isDismantling, String sdismantlingCode) {
        ...
        try {
            ...
            if (null != orderRecord.getIsourceClientType()) {
                ...
                } else if(BizTypeDefinitionEnum.ClientType.VENDSTOP.getCode() == orderRecord.getIsourceClientType()){
                    orderRecord.setIpayType(BizTypeDefinitionEnum.PayType.PAY_VENDSTOP.getCode());
                }
            }
            //如果订单为商户代扣,则支付类型为来源客户端类型
            if (BizTypeDefinitionEnum.ChargebackWay.WITHHOLDING.getCode() == orderRecord.getIchargebackWay()
                    || BizTypeDefinitionEnum.ChargebackWay.ONE_WITHHOLDING.getCode() == orderRecord.getIchargebackWay()) {
                /*if (BizTypeDefinitionEnum.ClientType.WECHAT.getCode() == orderRecord.getIsourceClientType()) {
                    orderRecord.setIpayType(BizTypeDefinitionEnum.PayType.PAY_WECHAT.getCode());
                } else if (BizTypeDefinitionEnum.ClientType.ALIPAY.getCode() == orderRecord.getIsourceClientType()) {
                    orderRecord.setIpayType(BizTypeDefinitionEnum.PayType.PAY_ALIPAY.getCode());
            ...
            } else if (BizTypeDefinitionEnum.ChargebackWay.WECHAT_PAY_POINT_WITHHOLDING.getCode() == orderRecord.getIchargebackWay()) {
                orderRecord.setIpayWay(BizTypeDefinitionEnum.PayWay.WECHAT_PAY_POINT_WITHHOLDING.getCode());//支付类型中的支付方式为代扣
            }
...
//        List<CommodityDiffVo> addVoList = (List<CommodityDiffVo>) map.get("addVoList");
    ...
    public ResponseVo generateExceptionOrder(ExceptionOrderDto exceptionOrderDto) {
        ...
        orderAudit.setSext(exceptionOrderDto.getSext());
