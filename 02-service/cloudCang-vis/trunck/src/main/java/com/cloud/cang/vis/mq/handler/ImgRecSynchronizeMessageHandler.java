...
public class ImgRecSynchronizeMessageHandler implements RmqMessageHandler<Mq_ImgRec> {
    ...
    public HandlerEnum handler(RmqMessage<Mq_ImgRec> rmqMessage) {
        ...
        try {
            ...
            logger.info("第三方同步返回识别参数: {}", JSONObject.toJSONString(mq_imgRec));

            String result = "";
            if(mq_imgRec.getResponseType() == 10){
                result = new DetectorLibrary().recogitionImageByFileName(rearms);
                logger.info("recogitionImageByFileName第三方同步返回识别结果: {}", result);
            } else {
                result = new DetectorLibrary().recogitionImageByFileNamePos(rearms);
                logger.info("recogitionImageByFileNamePos第三方同步返回识别结果: {}", result);
            }
