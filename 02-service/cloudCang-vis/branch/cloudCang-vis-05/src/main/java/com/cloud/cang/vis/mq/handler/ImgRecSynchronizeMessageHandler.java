...
/**
 * @program: 37cang-云平台
 * @description: 消息处理类：处理图像识别消息（第三方同步回调）
 * @author: qzg
 * @create: 2019-11-15 11:23
 **/
@Component
@DependsOn("initServer")
public class ImgRecSynchronizeMessageHandler implements RmqMessageHandler<Mq_ImgRec> {
    private static final Logger logger = LoggerFactory.getLogger(ImgRecSynchronizeMessageHandler.class);
    @Autowired
    ...
    public HandlerEnum handler(RmqMessage<Mq_ImgRec> rmqMessage) {
        ...
        try {
            ...
            if(mq_imgRec.getResponseType() == 10){
                logger.info("第三方同步方法调用recognitionImageByFileName识别参数: {}", rearms);
                result = new DetectorLibrary().recogitionImage(rearms);
                logger.info("第三方同步方法调用recogitionImageByFileName识别结果: {}", result.toString());
            }else{
                logger.info("第三方同步方法调用recogitionImageByFileNamePos识别参数: {}", rearms);
                result = new DetectorLibrary().recogitionImagePos(rearms);
                logger.info("第三方同步方法调用recogitionImageByFileNamePos识别结果: {}", result.toString());
            }

            imgResult.setMsg(result);
            imgResult.setStatus(200);
            ...
            // 识别失败
                ...
                DingTalkUtils.sendText("第三方同步方法vis调用识别失败,消息ID:"+rmqMessage.getId());
            }
        } catch (Exception e) {
            logger.info("第三方同步方法调用recognitionImageByFileName识别异常: {}", e.getMessage());
            // 钉钉预警
            DingTalkUtils.sendText("第三方同步方法vis调用识别失败,消息ID:"+rmqMessage.getId());
            // 识别失败，发送消息给api
            this.sendImgErrorToOpenApi(mq_imgRec,imgResult,rmqMessage);
    ...
    public boolean matchHandler(String queueName) {
        if(queueName.contains(QueueEnum.IMG_MODEL_SYNCHRONIZE.getName())){
            return true;
        }
        return false;
    }
    ...
    private void sendImgErrorToOpenApi(Mq_ImgRec mq_imgRec,
        ...
        RmqMessage build = RmqMessage.<Mq_ImgResult>builder()
                .queueName(QueueEnum.IMG_RESULT_SYNCHRONIZE.getName())
                .message(imgResult)
