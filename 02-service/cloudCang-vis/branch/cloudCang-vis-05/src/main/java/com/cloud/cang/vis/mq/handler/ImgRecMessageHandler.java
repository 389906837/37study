...
public class ImgRecMessageHandler implements RmqMessageHandler<Mq_ImgRec> {
    ...
    public HandlerEnum handler(RmqMessage<Mq_ImgRec> rmqMessage) {
        ...
        try {
            ...
            //String result = new DetectorLibrary().recogitionImage(rearms);
            List<Mq_ImgResult.ImgResultItem> result = new DetectorLibrary().recogitionImage(rearms);
            logger.info("调用recognitionImageByFileName识别结果: {}", result.toString());
