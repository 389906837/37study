...
public class ImgRecCallBackV4MessageHandler implements RmqMessageHandler<Mq_ImgRec> {
    private static final Logger logger = LoggerFactory.getLogger(ImgRecCallBackV4MessageHandler.class);
    @Autowired
    ...
    public HandlerEnum handler(RmqMessage<Mq_ImgRec> rmqMessage) {
        ...
        try {
            ...
            if(mq_imgRec.getResponseType() == 10){
                ...
                logger.info("第三方调用recogitionImageByFileName识别结果: {}", result.toString());
            }else{
                logger.info("第三方调用recogitionImageByFileNamePos识别参数: {}", rearms);
                result = new DetectorLibrary().recogitionImagePos(rearms);
                logger.info("第三方调用recogitionImageByFileNamePos识别结果: {}", result.toString());
            }
    ...
    public boolean matchHandler(String queueName) {
        if(queueName.contains(QueueEnum.IMG_MODEL_YOLOV4_CALLBACK.getName())){
            return true;
        }
        return false;
    }
    ...
    private void sendImgErrorToOpenApi(Mq_ImgRec mq_imgRec,
        ...
        RmqMessage build = RmqMessage.<Mq_ImgResult>builder()
                .queueName(QueueEnum.IMG_RESULT_YOLOV4_CALLBACK.getName())
                .message(imgResult)
