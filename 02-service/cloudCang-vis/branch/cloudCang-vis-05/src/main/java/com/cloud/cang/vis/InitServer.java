...
public class InitServer implements InitializingBean {
    ...
    public void loadVisMoel(boolean isReload){
        ...
        ModelParamsDto initServerDto = ModelParamsDto.builder()
                .weightPath(smodelAddress + VisContant.MODE_FIEL)
                .configPath(smodelAddress + VisContant.CONFIG_FILE)
                .classesPath(smodelAddress + VisContant.NAMES_FILE)
                .classes(model.getIcategoryNum())
                .gpuIndex(server.getSgpuIndex()!=null?Integer.parseInt(server.getSgpuIndex()):0)
        ...
        if(status != 200){
            logger.info("模型初始化失败,vis服务{}:{},模型编号:{}",host,port,modelCode);
